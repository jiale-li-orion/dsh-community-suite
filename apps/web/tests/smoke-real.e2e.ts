...
describe('dsh web keyless CLI smoke', () => {
  ...
  it('DSH_TOOLS_MODE=code collapses the provider wire tools to run_code with the SDK prompt section', async () => {
    ...
    const provider = createServer((request, response) => {
      ...
      request.on('data', (chunk: string) => { body += chunk })
      request.on('end', () => {
        const parsed = JSON.parse(body) as CodeModeProviderRequest
        // Title generation can race the main turn and carries no tool surface.
        // Verify the first model-facing tool request rather than whichever
        // provider request happens to arrive first.
