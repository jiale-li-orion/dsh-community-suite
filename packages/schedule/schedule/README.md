...
Replay rejects unknown versions, extra fields, reused ids, mismatched one-shot or Every dispatch shapes, and delete or dispatch transitions against inactive records. Normal Sessions incrementally fold indexed `schedule/change` events from the complete logical log. A fork starts that fold at `session.header.seedLength ?? 0`, so it does not inherit its parent's reminders. The package's `./invariant` companion applies the same policy to existing logs and candidate events.
