...
/** Make one attached Session read a same-length hostile persistence result. */
function replaceSessionLog(test: ToolHarness, events: readonly SessionEvent[]): void {
  const hostileLog = Session.create(test.agent.id, events).readLog()
  test.agent.session.append('turn/start', { turn: 1 })
  test.agent.session.append('turn/end', { turn: 1, reason: { kind: 'completed' } })
  expect(test.agent.session.seq).toBe(hostileLog.length)
  Object.defineProperty(test.agent.session, 'readLog', {
    configurable: true,
    value: () => hostileLog,
  })
}
...
describe('Schedule persistence failure boundaries', () => {
  ...
  it('maps corrupt and unreadable folds for create, list, and delete', async () => {
    ...
    unreadable.agent.session.append('turn/start', { turn: 1 })
    unreadable.agent.session.append('turn/end', { turn: 1, reason: { kind: 'completed' } })
    Object.defineProperty(unreadable.agent.session, 'readLog', {
      configurable: true,
