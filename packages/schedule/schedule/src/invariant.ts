...
import { foldScheduleSession, ScheduleLogError, validateScheduleCandidate } from './domain.ts'
...
/** Validate the current exact-session stream under its fork suffix policy. */
function validate(session: Session, fail: InvariantFailure): void {
  try {
    foldScheduleSession(session)
  } catch (error: unknown) {
...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  ...
  ctx.on('internal/dispatch', (_mode, eventName, args) => {
    ...
    try {
      validateScheduleCandidate(session, event)
    } catch (error: unknown) {
      /* v8 ignore next -- candidate validation normalizes rejected transitions. */
      if (!(error instanceof ScheduleLogError)) throw error
      fail(error.message)
    }
  }, { global: true })
