...
function foldForTool(agent: Agent): ReturnType<typeof foldScheduleSession> | ScheduleToolError {
  try {
    return foldScheduleSession(agent.session)
  } catch (error: unknown) {
...
function isToolError(
  value: ReturnType<typeof foldScheduleSession> | ScheduleToolError,
