...
interface MutableScheduleFold {
  readonly seedLength: number
  readonly active: Map<ScheduleIdType, ScheduleRecord>
  readonly seen: Set<ScheduleIdType>
}
...
/** Apply one owned schedule event after all fallible decoding for that transition. */
function applyScheduleEvent(state: MutableScheduleFold, event: SessionEvent): void {
  if (event.seq < state.seedLength || event.type !== 'schedule/change') return
...
/** Detach the public immutable view from mutable replay maps. */
function scheduleFoldView(state: MutableScheduleFold): FoldedSchedules {
  return Object.freeze({
...
const liveScheduleFolds = new SessionEventFold<MutableScheduleFold>(
  (session) => {
    const seedLength = session.header.seedLength ?? 0
    if (!Number.isSafeInteger(seedLength) || seedLength < 0 || seedLength > session.seq) {
      throw new ScheduleLogError('schedule seedLength must be within the supplied event log')
    }
    return { seedLength, active: new Map(), seen: new Set() }
  },
  applyScheduleEvent,
  ['schedule/change'],
)
...
/**
 * Incrementally fold one live Session without materializing its complete log.
 * @param session - Session whose owned schedule suffix is folded.
 * @returns detached active records and used ids.
 */
export function foldScheduleSession(session: Session): FoldedSchedules {
  return scheduleFoldView(liveScheduleFolds.read(session))
}

/**
 * Whether one live Session owns any schedule record, including a corrupt one.
 * @param session - Session whose schedule suffix is inspected.
 * @returns whether the suffix contains or attempts to contain a schedule record.
 */
export function sessionHasScheduleEvents(session: Session): boolean {
  try {
    const state = liveScheduleFolds.read(session)
    return state.seen.size > 0
  } catch {
    // A corrupt owned event still needs the runtime drive that reports the fault.
    return true
  }
}

/**
 * Validate one uncommitted schedule event against the current live fold.
 * @param session - Session supplying the committed schedule state.
 * @param event - candidate event at the Session's next seq.
 */
export function validateScheduleCandidate(session: Session, event: SessionEvent): void {
  const current = liveScheduleFolds.read(session)
  applyScheduleEvent({
    seedLength: current.seedLength,
    active: new Map(current.active),
    seen: new Set(current.seen),
  }, event)
}
...
export function foldScheduleEvents(
  ...
  const state: MutableScheduleFold = {
    seedLength,
    active: new Map(),
    seen: new Set(),
  }
  for (let seq = seedLength; seq < events.length; seq += 1) {
    // Range validation above proves every index in this loop exists.
    // oxlint-disable-next-line typescript/no-non-null-assertion
    applyScheduleEvent(state, events[seq]!)
  }
  return scheduleFoldView(state)
}
