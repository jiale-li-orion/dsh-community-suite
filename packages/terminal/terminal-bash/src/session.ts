  ...
  private async pollReadiness(operation: LocalSendOperation): Promise<void> {
    ...
    try {
      ...
      const foreground = await this.terminal.inspectForeground({
        maxObservationAgeMs: this.config.pollIntervalMs,
      })
      if (this.active !== operation || this.closing || this.interrupting === operation) return
      ...
      // A shared, fast process-table snapshot can observe the controlled shell
      // back in read(2) before node-pty delivers its prompt bytes. Give that
      // same foreground group the normal handoff grace; a child group keeps
      // the exact stdin-wait fast path.
      const shellWaitNeedsDrain = foreground?.processGroupId === this.shellPgid
        && idleFor < this.config.handoffGraceMs
      if (elapsed >= this.config.exactProbeAfterMs && acceptsStdinWait && !shellWaitNeedsDrain) {
        this.settleActive('stdin_read')
        return
      }
