...
import type {
  ...
  SubprocessTerminalInspectOptions,
...
class FakeTerminal implements SubprocessTerminalHandle {
  ...
  readonly inspectionOptions: Array<SubprocessTerminalInspectOptions | undefined> = []
  ...
  async inspectForeground(options?: SubprocessTerminalInspectOptions) {
    this.inspectionOptions.push(options)
...
describe('LocalPtySession readiness and output', () => {
  it('lets queued terminal output run before the first post-write readiness poll', async () => {
    ...
    let inspections = 0
    terminal.inspectionOptions.length = 0
    terminal.inspectForeground = async (options) => {
      inspections += 1
    ...
    expect(terminal.inspectionOptions).toEqual([undefined])

    await vi.advanceTimersByTimeAsync(0)
    expect(inspections).toBe(1)
    terminal.emitData('\x1b]133;D;0\x07dsh> ')
    await vi.advanceTimersByTimeAsync(10)
    expect(terminal.inspectionOptions.at(-1)).toEqual({ maxObservationAgeMs: 10 })
  ...
  it('lets a controlled-shell prompt overtake an early same-group stdin wait', async () => {
    vi.useFakeTimers()
    const terminal = new FakeTerminal()
    const inspector = new FakeInspector()
    const session = makeSession(terminal, inspector, config({ handoffGraceMs: 100 }))
    await initialize(session, terminal)

    const operation = session.startSend({ text: 'exec bash --noprofile --norc -i', submit: true })
    let settled = false
    void operation.done.then(() => { settled = true })
    inspector.waiting = false
    await vi.advanceTimersByTimeAsync(19)
    terminal.emitData('exec bash --noprofile --norc -i\r\n')
    inspector.waiting = true
    ...
    expect(await operation.done).toMatchObject({
      ...
      viewport: 'exec bash --noprofile --norc -i\ndsh> ',
    })
  })
