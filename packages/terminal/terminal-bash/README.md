...
Periodic readiness polls permit foreground observations up to `pollIntervalMs` old so one provider snapshot can serve concurrent terminals; the pre-write observation and every signal omit that allowance and remain exact.
