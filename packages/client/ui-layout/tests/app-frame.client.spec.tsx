...
describe('AppFrame — workbench column', () => {
  it('renders the workbench slot collapsed at zero width by default', () => {
    ...
    expect(slotCalls.find(c => c.key === 'workbench')!.props).toEqual({ collapsed: true, width: 0 })
  })

  it('openWorkbench opens the column at the contract default and adds its handle', () => {
    ...
    expect(slotCalls.filter(c => c.key === 'workbench').at(-1)!.props)
      .toEqual({ collapsed: false, width: 560 })
  })

  it('workbench drag widens leftward (negative dx grows the panel)', () => {
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.openWorkbench() })
    const handles = frame.querySelectorAll('[class*="handle"]')
    // With details closed the workbench handle sits at viewport - workbench.
    drag(handles[1]!, 1360, 1300)
    expect(tracks(frame)).toEqual([280, 620, 0])
  })

  it('workbench and details keep independent columns with both handles present', () => {
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.openWorkbench() })
    act(() => { instance.actions.openDetails() })
    expect(tracks(frame)).toEqual([280, 560, 360])
    expect(frame.querySelectorAll('[class*="handle"]')).toHaveLength(3)
  })

  it('the workbench concedes before the center when the viewport shrinks', () => {
    frameWidth = 1300
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.openWorkbench() })
    act(() => { instance.actions.openDetails() })
