...
describe('open', () => {
  ...
  it('installs the tail page: cold → loading → open with window and nodes in place', async () => {
    ...
    expect(snapshot.surfaceRevision).toBe(13)
  })
...
describe('live event path', () => {
  ...
  it('changes surfaceRevision only for surface-bearing history or live events', async () => {
    const { session } = await opened()
    expect(session.getSnapshot().surfaceRevision).toBe(3)

    session.handleMuxEnvelope('log-only' as never, {
      type: 'session/event',
      sessionId: SID,
      event: ev.commandRun(6, 'context-log-only', 'noop'),
    })
    await Promise.resolve()
    expect(session.getSnapshot().surfaceRevision).toBe(3)

    session.handleMuxEnvelope('surface' as never, {
      type: 'session/event',
      sessionId: SID,
      event: ev.user(7, 'new context'),
    })
    await Promise.resolve()
    expect(session.getSnapshot().surfaceRevision).toBe(7)
  })
...
describe('paging', () => {
  ...
  it('pages from the first durable seq represented by a compact history entry', async () => {
    const older = plainTurn(0, 0, '旧问', '旧答')
    const { api, session } = makeSession()
    api.onHistory = payload => payload.beforeSeq === undefined
      ? Promise.resolve(ok({
        events: [{ firstSeq: 6, event: ev.assistant(9, 1, '压缩回复') }] as never[],
        hasMore: true,
      }))
      : histResponse(older, false)
    await session.open()
    ...
    expect(session.getSnapshot().nodes.map(node => node.seq)).toEqual([1, 3, 9])
  })
