...
import { resolvedClientDevice } from '../device.ts'
...
export class Session implements SessionFace {
  ...
  /** Wire views aligned with the history-projected `events` by index. */
  ...
  /** Session-owned business Context engine over history projections plus the raw live tail. */
  ...
  /** Latest surface-bearing event seq in the installed history plus raw live tail. */
  private surfaceRevision: number | null = null
  ...
  async prompt(content: PromptContentPart[], mode: 'queue' | 'steer'): Promise<RpcResult<{ accepted: true }>> {
    ...
    try {
      if (this.address === undefined) {
        const clientDevice = resolvedClientDevice()
  ...
  async loadOlder(): Promise<void> {
    ...
    try {
      ...
      this.baseSeq = older[0] === undefined ? this.baseSeq : historyEntryFirstSeq(older[0])
      this.hasMore = result.value.hasMore
  ...
  /** Install the history window + stitch the liveBuffer (event.seq is the sole dedup key).
  ...
  private installWindow(entries: HistoryEntry[], hasMore: boolean, projections?: ProjectionsBaseline): void {
    ...
    this.surfaceRevision = latestSurfaceRevision(this.events)
    this.baseSeq = entries[0] === undefined ? 0 : historyEntryFirstSeq(entries[0])
  ...
  private appendLive(event: SessionEvent, view?: ToolEventView): ConversationPublication {
    ...
    const surfaceChanged = isSurfaceEvent(event)
    if (surfaceChanged) this.surfaceRevision = event.seq
    if (event.type === 'turn/start') this.firstPromptPendingTurn = false
    const queueChanged = this.queueMirror.acceptDurable(event)
    const publication = this.conversation.append({ event, view })
    return queueChanged || surfaceChanged ? 'immediate' : publication
  }

  /** Land a live session/event (open/repair in flight -> buffer; overlapping seq -> drop;
   *  a seq gap -> buffer + tail-page repull instead of appending a hole (a gap is an
   *  expected reconnect-window artifact, repaired by refetch). History entries preserve their
   *  represented seq spans, so the projected window joins the raw live tail without a durable gap. */
...
/** Latest event that changed model-visible surface membership in one installed window. */
function latestSurfaceRevision(events: readonly SessionEvent[]): number | null {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index]
    if (event !== undefined && isSurfaceEvent(event)) return event.seq
  }
  return null
}
...
/** First durable seq represented by one history projection entry. */
function historyEntryFirstSeq(entry: HistoryEntry): number {
  return entry.firstSeq ?? entry.event.seq
}
