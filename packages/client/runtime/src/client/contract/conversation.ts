...
export interface ConversationNodeDefinition<State = unknown> {
   ...
   * History may coalesce consecutive Assistant deltas while preserving their
   * accumulated content and semantic timing boundaries; live-tail events are
   * the committed Session events. No Context or history access is available.
   * @param event - event value supplied by the current history or live projection.
