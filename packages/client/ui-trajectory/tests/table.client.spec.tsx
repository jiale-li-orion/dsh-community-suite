...
afterEach(async () => {
  const virtualizerMounted = document.querySelector('[data-virtual-position]') !== null
  cleanup()
  // TanStack Virtual's scroll-end fallback retains its 150 ms timeout after
  // observer disposal. Let virtualized cases settle while jsdom still owns window.
  if (virtualizerMounted) await new Promise(resolve => window.setTimeout(resolve, 175))
  vi.restoreAllMocks()
