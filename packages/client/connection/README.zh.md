...
- **`privilegedAuthority: 'trusted'` 放宽的是可达性，不是信任**：它允许已声明的权威调用 `host.pickDirectory`／`host.openPath`、整个设置与凭据面、agent preset 创作面以及 `llm.discoverModels`。这里假定对端都是操作者自己的设备；认证层仍不存在，因此共享或不可信的网络必须保持默认的 `'loopback'`。
