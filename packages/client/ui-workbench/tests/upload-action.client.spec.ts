...
describe('createUploadAction', () => {
  it('posts the file to the route the listing named and answers the written path', async () => {
    ...
    const path = await createUploadAction(listDir)(SESSION, file)
    expect(path).toBe('uploads/photo.jpg')
    expect(calls[0]?.url).toBe('/workbench/upload?sessionId=session-upload&name=photo.jpg')
  ...
  it('surfaces the server’s reason for a refusal', async () => {
    ...
    await expect(createUploadAction(listDir)(SESSION, new File(['x'], 'x.bin')))
