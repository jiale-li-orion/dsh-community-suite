...
export interface IConversation {
  ...
  /**
   * Select one registered conversation view in the caller's Session scope.
   * @param viewId - registration id from the conversation view ring.
   */
...
export class ConversationController extends Service implements IConversation {
  ...
  private readonly viewSelections = new Map<SessionId, (viewId: string) => void>()
  ...
  /**
   * Select one conversation view through the caller's Session scope.
   * @param viewId - registration id from the conversation view ring.
   */
  selectView(viewId: string): void {
    const sessionId = this.scopeId('selectView')
    const select = this.viewSelections.get(sessionId)
    if (select === undefined) {
      throw new Error(`conversation.selectView: session "${sessionId}" has no mounted view selector`)
    }
    select(viewId)
  }

  /**
   * Adopt the current per-session store action used by optional view plugins.
   * @param sessionId - Session whose view store owns the action.
   * @param select - bound action that updates the active registration id.
   */
  attachViewSelection(sessionId: SessionId, select: (viewId: string) => void): void {
    this.viewSelections.set(sessionId, select)
  }
