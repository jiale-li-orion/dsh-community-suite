...
function inspection(meta: SessionHeader, events: readonly SessionEvent[]): SessionInspection {
...
describe('API Remote Agent resolver races', () => {
  it('maps an inspected session without a cwd to session-not-found', async () => {
    ...
    provideSession(ctx, meta, () => Promise.resolve(inspection(
      { ...meta, cwd: undefined } as unknown as SessionHeader,
      [],
    )))
  ...
  it('passes the stable log cut to cold setup without materializing events', async () => {
    const ctx = await createContext()
    const sessionId = sid('cut-only-setup')
    const meta = header(sessionId)
    const log = sessionLogCutFromEvents([])
    const inspected: SessionInspection = {
      meta,
      log,
      get events(): readonly SessionEvent[] {
        throw new Error('complete event materialization is forbidden in cold setup')
      },
    }
    provideSession(ctx, meta, () => Promise.resolve(inspected))
    const setup = vi.fn((source: SessionInspection) => {
      expect(source.log).toBe(log)
      return () => {}
    })
    ...
    const result = await createApiRemoteAgentResolver(ctx, { setup })(sessionId)
  ...
  it('resumes through a concurrently attached ordinary Session without optional defaults', async () => {
    ...
    provideSession(ctx, meta, () => {
      ...
      return Promise.resolve(inspection(meta, []))
    })
  ...
  it('reclassifies failed resumes after a live or attached subagent wins publication', async () => {
    for (const winner of ['agent', 'session'] as const) {
      ...
      const meta = header(sessionId)
      provideSession(ctx, meta, () => Promise.resolve(inspection(meta, [])))
