...
export interface ApiRemoteAgentOptions {
   ...
   * log comes along because a session's own record of such a choice may be an
   * event rather than a header field.
   * @param session - the resumed session's persisted header and stable log cut.
   * @returns the Agent-scope setup to run before publication.
   */
  readonly setup?: (session: SessionInspection) => AgentSetup | Promise<AgentSetup>
}
 ...
 * @returns the immutable persistence inspection for a servable session.
