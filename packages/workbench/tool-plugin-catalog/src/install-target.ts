 ...
 * `github:owner/repo[#subpath]` reference, no `..`, no shell metacharacter —
 * because the target is the one field an attacker could try to widen.
...
/** A GitHub shorthand, optionally addressing a monorepo subpath. */
const GITHUB_SPEC = /^github:[a-z0-9-_.]+\/[a-z0-9-_.]+(?:#[a-z0-9-_.]+(?:\/[a-z0-9-_.]+)*)?$/i
...
export function parseInstallTarget(install: string): string {
  ...
  if (!NPM_SPEC.test(target) && !GITHUB_SPEC.test(target)) {
    throw new PluginInstallTargetError(`install target ${JSON.stringify(target)} is neither an npm specifier nor a github:owner/repo reference`)
  }
  return target
}
