...
class TestPersistence extends SessionPersistence {
  ...
  readonly logical = new Map<SessionId, { meta: SessionHeader; events: readonly SessionEvent[] }>()
  ...
  async load(id: SessionId): Promise<SessionInspection> {
    const stored = await this.readFrom(id, 0)
    return inspectionFromEvents(stored.meta, stored.events)
  }
  ...
  inspect(id: SessionId): Promise<SessionInspection> {
    ...
    if (explicit !== undefined) return Promise.resolve(inspectionFromEvents(explicit.meta, explicit.events))
    const live = this.ctx.sessions.get(id)
    if (live !== undefined) {
      const log = live.readLog()
      return Promise.resolve({ meta: live.header, log, events: log.materialize() })
    }
  ...
  async readFrom(
    ...
    return stored === undefined
      ? Promise.reject(new Error(`test persistence: session '${id}' not found`))
      : { meta: stored.meta, events: [...stored.log.values(fromSeq)] }
  }
  ...
  async listSnapshots(): Promise<SessionPersistenceSnapshot[]> {
    ...
    return [...this.durable.values()].map((value, index) => ({
      header: value.meta,
      revision: SessionPersistenceRevision(`test:${index}:${value.log.length}`),
    }))
  }

  persist(session: Session): void {
    const log = session.readLog()
    this.durable.set(session.id, { meta: session.header, log, events: log.materialize() })
  }

  setDurable(source: { meta: SessionHeader; events: readonly SessionEvent[] }): void {
    this.durable.set(source.meta.id, inspectionFromEvents(source.meta, source.events))
  }
}

/** Adapt one expanded test fixture to the production inspection contract. */
function inspectionFromEvents(
  meta: SessionHeader,
