...
import { sessionLogCutFromEvents } from '@deepseek-ai/dsh-session'
...
export class MessageFeedbackService extends TypertRemoteService {
  ...
  private hasFeedbackTarget(inspection: SessionInspection, messageId: MessageFeedbackItem['messageId']): boolean {
    for (const event of inspection.log.valuesOf(['assistant/message'])) {
      if (!isAppendSurfaceEvent(event)) continue
      const message = deriveEventMessage(event)
      if (message?.role === 'assistant' && message.id === messageId) return true
    }
    return false
  }
  ...
  private async ensureTargetDurable(inspection: SessionInspection): Promise<SessionInspection> {
    ...
    if (live !== undefined && sameHeaderIdentity(live.header, inspection.meta)) {
      ...
      const durable = await this.ctx.sessionPersistence.readFrom(inspection.meta.id, 0)
      const log = sessionLogCutFromEvents(durable.events)
      return { meta: durable.meta, log, events: log.materialize() }
    }
    const durable = await this.ctx.sessionPersistence.readFrom(inspection.meta.id, 0)
    const log = sessionLogCutFromEvents(durable.events)
    return { meta: durable.meta, log, events: log.materialize() }
