...
export class LocalTerminalHandle implements SubprocessTerminalHandle {
  ...
  async inspectForeground(
    options?: SubprocessTerminalInspectOptions,
  ): Promise<SubprocessTerminalForeground | undefined> {
    const maxObservationAgeMs = options?.maxObservationAgeMs ?? 0
    this.descendants(maxObservationAgeMs)
    const processGroupId = this.inspector.foregroundPgid(this.pid)
    if (processGroupId === undefined) return undefined
    return {
      processGroupId,
      inputWaiting: this.inspector.isStdinWaiting(processGroupId, maxObservationAgeMs),
    }
  }
  ...
  private descendants(maxObservationAgeMs = 0): ProcessIdentity[] {
    ...
    const tree = this.inspector.processTree(this.pid, maxObservationAgeMs)
    ...
    const rootVerified = this.rootIdentity !== undefined
      ...
      && this.inspector.isAlive(this.rootIdentity)
    this.trackedDescendants = this.survivors(this.unionMembers(
      this.trackedDescendants,
      ...rootVerified ? [tree, this.inspector.processSession(this.pid, maxObservationAgeMs)] : [],
