...
/** Injectable OS process operations shared by local PTY handles. */
export interface ProcessInspector {
  foregroundPgid(shellPid: number): number | undefined
  isStdinWaiting(pgid: number, maxObservationAgeMs?: number): boolean
  /** Return the root and its current transitive descendants, children first. */
  processTree(rootPid: number, maxObservationAgeMs?: number): ProcessIdentity[]
  /** Return current members of one POSIX process session when the platform exposes them. */
  processSession(sessionId: number, maxObservationAgeMs?: number): ProcessIdentity[]
...
abstract class PosixProcessInspector implements ProcessInspector {
  ...
  abstract isStdinWaiting(pgid: number, maxObservationAgeMs?: number): boolean
  abstract processTree(rootPid: number, maxObservationAgeMs?: number): ProcessIdentity[]
  abstract processSession(sessionId: number, maxObservationAgeMs?: number): ProcessIdentity[]
...
interface LinuxProcessTable {
  capturedAt: number
  byPid: ReadonlyMap<number, ProcStat>
  byParent: ReadonlyMap<number, readonly ProcStat[]>
  byProcessGroup: ReadonlyMap<number, readonly ProcStat[]>
  bySession: ReadonlyMap<number, readonly ProcStat[]>
}

function indexedProcessTable(capturedAt: number, entries: readonly ProcStat[]): LinuxProcessTable {
  const byPid = new Map<number, ProcStat>()
  const byParent = new Map<number, ProcStat[]>()
  const byProcessGroup = new Map<number, ProcStat[]>()
  const bySession = new Map<number, ProcStat[]>()
  const append = (index: Map<number, ProcStat[]>, key: number, entry: ProcStat): void => {
  ...
  for (const entry of entries) {
    ...
    append(byParent, entry.parentPid, entry)
    append(byProcessGroup, entry.pgrp, entry)
    append(bySession, entry.session, entry)
  }
  return { capturedAt, byPid, byParent, byProcessGroup, bySession }
}

function indexedProcessTree(table: LinuxProcessTable, rootPid: number): ProcessIdentity[] {
  const root = table.byPid.get(rootPid)
  if (root === undefined) return []
  const visited = new Set<number>()
  const result: ProcessIdentity[] = []
  const visit = (entry: ProcStat): void => {
    if (visited.has(entry.pid)) return
    visited.add(entry.pid)
    for (const child of table.byParent.get(entry.pid) ?? []) visit(child)
    result.push({ pid: entry.pid, started: entry.started })
  }
  visit(root)
  return result
...
class LinuxProcessInspector extends PosixProcessInspector {
  private processTableSnapshot: LinuxProcessTable | undefined
  ...
  isStdinWaiting(pgid: number, maxObservationAgeMs = 0): boolean {
    const table = SYSCALLS[this.arch]
    if (table === undefined) return false
    const observation = this.processTable(maxObservationAgeMs)
    for (const observed of observation.table.byProcessGroup.get(pgid) ?? []) {
      const current = observation.cached ? readLinuxStat(this.internals, observed.pid) : observed
      if (current?.started !== observed.started || current.pgrp !== pgid) continue
      for (const tid of numericEntries(this.internals, `/proc/${observed.pid}/task`)) {
        const syscall = readSyscall(this.internals, observed.pid, tid)
        if (syscall !== undefined
          && syscallWaitsOnStdin(this.internals, observed.pid, syscall, table)) return true
      }
    }
    return false
  }

  processTree(rootPid: number, maxObservationAgeMs = 0): ProcessIdentity[] {
    return indexedProcessTree(this.processTable(maxObservationAgeMs).table, rootPid)
  }

  processSession(sessionId: number, maxObservationAgeMs = 0): ProcessIdentity[] {
    const observation = this.processTable(maxObservationAgeMs)
    return (observation.table.bySession.get(sessionId) ?? []).flatMap((observed) => {
      const current = observation.cached ? readLinuxStat(this.internals, observed.pid) : observed
      return current?.started === observed.started && current.session === sessionId
        ? [{ pid: observed.pid, started: observed.started }]
        : []
    })
  }
  ...
  private processTable(maxObservationAgeMs: number): { table: LinuxProcessTable; cached: boolean } {
    const capturedAt = this.internals.now()
    const previous = this.processTableSnapshot
    const age = previous === undefined ? Number.POSITIVE_INFINITY : capturedAt - previous.capturedAt
    if (previous !== undefined && maxObservationAgeMs > 0 && age >= 0 && age <= maxObservationAgeMs) {
      return { table: previous, cached: true }
    }
    ...
    const table = indexedProcessTable(capturedAt, entries)
    this.processTableSnapshot = table
...
class MacProcessInspector extends PosixProcessInspector {
  ...
  isStdinWaiting(_pgid: number, _maxObservationAgeMs = 0): boolean {
    return false
  }

  processTree(rootPid: number, _maxObservationAgeMs = 0): ProcessIdentity[] {
    return processTree(macProcessTable(this.internals), rootPid)
  }

  processSession(_sessionId: number, _maxObservationAgeMs = 0): ProcessIdentity[] {
    return []
  }
