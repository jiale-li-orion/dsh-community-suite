...
import type { SubprocessTerminalInspectOptions, SubprocessTerminalSignal } from '@deepseek-ai/dsh-subprocess'
...
class FakeInspector implements ProcessInspector {
  ...
  readonly observationAges: Array<number | undefined> = []
  throwGroup = false
  throwProcess = false
  removeOnSignal = true

  foregroundPgid() { return this.pgid }
  isStdinWaiting(_pgid: number, maxObservationAgeMs?: number) {
    this.observationAges.push(maxObservationAgeMs)
    return this.waiting
  }
  processTree(_rootPid: number, maxObservationAgeMs?: number) {
    this.observationAges.push(maxObservationAgeMs)
    return this.root === undefined ? this.members : [this.root, ...this.members]
  }
  processSession(_sessionId: number, maxObservationAgeMs?: number) {
    this.observationAges.push(maxObservationAgeMs)
    return this.sessionMembers
  }
  isAlive(identity: ProcessIdentity) {
    return (identity.pid === this.root?.pid && identity.started === this.root.started)
      || this.alive.has(identity.pid)
  }
...
describe('LocalTerminalHandle', () => {
  ...
  it('bridges terminal bytes, foreground control, and signalled exit facts', async () => {
    ...
    inspector.observationAges.length = 0
    const options: SubprocessTerminalInspectOptions = { maxObservationAgeMs: 50 }
    expect(await handle.inspectForeground(options)).toEqual({ processGroupId: 456, inputWaiting: true })
    expect(inspector.observationAges).toEqual([50, 50, 50])
    inspector.observationAges.length = 0
    expect(await handle.signalForeground('SIGINT')).toBe(456)
    expect(inspector.observationAges).toEqual([0, 0, 0])
