...
function fakeInternals() {
  ...
  return {
    ...
    directoryReadCount(path: string) { return directoryReads.get(path) ?? 0 },
  }
}
...
describe('Linux process inspector', () => {
  ...
  it('parses stat safely, captures only the rooted process tree, and signals identities', () => {
    ...
    fake.files.set('/proc/10/stat', stat(10, 20, 30, 40, '500', 13))
  ...
  it('reuses one bounded process-table snapshot and refreshes on expiry or exact reads', () => {
    const fake = fakeInternals()
    fake.dirs.set('/proc', ['10', '11'])
    fake.files.set('/proc/10/stat', stat(10, 77, 10, 77, '1'))
    fake.files.set('/proc/11/stat', stat(11, 88, 10, 77, '2', 10))
    fake.dirs.set('/proc/10/task', ['10'])
    fake.files.set('/proc/10/task/10/syscall', syscall(0, 0))
    const inspector = createProcessInspector('linux', 'x64', fake.internals)

    expect(inspector.processTree(10, 50)).toEqual([
      { pid: 11, started: '2' },
      { pid: 10, started: '1' },
    ])
    expect(inspector.processSession(10, 50)).toEqual([
      { pid: 10, started: '1' },
      { pid: 11, started: '2' },
    ])
    expect(inspector.isStdinWaiting(77, 50)).toBe(true)
    expect(fake.directoryReadCount('/proc')).toBe(1)

    fake.advance(50)
    expect(inspector.processTree(10, 50)).toHaveLength(2)
    expect(fake.directoryReadCount('/proc')).toBe(1)
    fake.advance(1)
    expect(inspector.processTree(10, 50)).toHaveLength(2)
    expect(fake.directoryReadCount('/proc')).toBe(2)
    expect(inspector.processTree(10)).toHaveLength(2)
    expect(fake.directoryReadCount('/proc')).toBe(3)
  })

  it('revalidates cached process-group candidates before reporting an input wait', () => {
    const fake = fakeInternals()
    ...
    expect(inspector.isStdinWaiting(77, 50)).toBe(true)
    fake.files.set('/proc/10/stat', stat(10, 88, 10, 88, '1'))
