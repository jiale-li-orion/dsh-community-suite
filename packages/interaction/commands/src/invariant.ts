...
const COMMAND_REPLAY_TYPES = ['command/run', 'command/done'] as const
...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  ...
  const validateEvent = (session: Session, event: SessionEvent): void => {
    ...
    if (source !== undefined) {
      const validIndex = Number.isSafeInteger(source) && source >= 0 && source < event.seq
      const sourceEvent = validIndex ? session.readLog().at(source) : undefined
  ...
  for (const session of ctx.sessions.list()) {
    for (const event of session.readLog().valuesOf(COMMAND_REPLAY_TYPES)) validateEvent(session, event)
  }
