...
/** Incremental policy and turn fold for one Session object. */
...
export class ApprovalService extends Service {
  ...
  private readonly fold = new SessionEventFold<ApprovalSessionState>(
    () => ({ override: undefined, openTurn: false }),
    (state, event) => {
      if (event.type === 'approval/policy') state.override = event.data.policy
      else if (event.type === 'turn/start') state.openTurn = true
      else if (event.type === 'turn/end') state.openTurn = false
    },
    ['approval/policy', 'turn/start', 'turn/end'],
  )
  ...
  async request(req: ApprovalRequest): Promise<ApprovalOutcome> {
    ...
    if (!this.fold.read(session).openTurn) {
      throw new Error(
  ...
  overrideOf(session: Session): ApprovalPolicy | undefined {
    return this.fold.read(session).override
  }
