 ...
 * and `agent.session.readLog()`. Seeded inside an open turn by default (request()'s
...
function fakeAgent(seed: Array<{ type: string }> = [{ type: 'turn/start' }, { type: 'user/message' }]): { agent: Agent; appended: Array<{ type: string; data: Record<string, unknown> }> } {
  ...
  const agent = {
    session: {
      ...
      readLog: () => ({
        length: seed.length,
        values: (from = 0, to = seed.length) => seed.slice(from, to),
        valuesOf: (types: readonly string[], from = 0, to = seed.length) =>
          seed.slice(from, to).filter(event => types.includes(event.type)),
      }),
  ...
  it('propagates an append failure that prevented audit log growth', async () => {
    ...
    const agent = {
      session: {
        ...
        readLog: () => ({
          length: 1,
          values: () => [{ type: 'turn/start' }],
          valuesOf: (types: readonly string[]) => types.includes('turn/start')
            ? [{ type: 'turn/start' }]
            : [],
        }),
