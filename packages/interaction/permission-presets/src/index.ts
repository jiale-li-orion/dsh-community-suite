...
/** Incremental live knob fold plus the durable seed marker. */
interface LivePermissionState {
  knobs: KnobState
  seeded: boolean
}
...
export class PermissionPresetService extends Service {
  ...
  private readonly fold = new SessionEventFold<LivePermissionState>(
    () => ({ knobs: { ...EMPTY_KNOBS }, seeded: false }),
    (state, event) => {
      state.knobs = applyKnobEvent(state.knobs, event)
      if (event.type === 'session/end-seed') state.seeded = true
    },
  ...
  constructor(ctx: Context, config: Config) {
    ...
    ctx.on('session/event', (session, event) => {
      this.fold.advance(session, event)
    })
    ...
    ctx.inject(['sessionProjections'], (projectionCtx) => {
      projectionCtx.sessionProjections.register<'permissions', KnobState>({
        ...
        schema: selectSchema,
        init: () => EMPTY_KNOBS,
        eventTypes: ['permission/preset', 'sandbox/mode', 'approval/policy'],
    ...
    ctx.inject(['commands'], (commandCtx) => {
      commandCtx.commands.register({
        ...
        handler: ({ agent, rawInput }) => {
          ...
          if (name === '') {
            return { kind: 'success', text: `current preset ${this.currentSession(agent.session)} (available: ${this.names.join(', ')})` }
          }
  ...
  /** Resolve one live Session without materializing its complete log. */
  private currentSession(session: Session): string {
    return this.derive(this.fold.read(session).knobs)
  }
  ...
  private apply(session: Session, name: string, setApproval: (policy: ApprovalPolicy) => void): void {
    const spec = this.resolve(name)
    const state = this.fold.read(session)
    if (this.derive(state.knobs) !== name) {
      session.append('permission/preset', { preset: name })
    }
    if (spec.sandbox !== (state.knobs.sandbox ?? this.ctx.shell.sandboxMode)) {
      setSandboxMode(session, spec.sandbox)
    }
    if (spec.approval !== (state.knobs.approval ?? this.ctx.approval.config.policy ?? 'ask')) {
  ...
  private pinInitialPermission(session: Session): void {
    const live = this.fold.read(session)
    const selected = live.knobs.preset ?? undefined
    const sandbox = live.knobs.sandbox ?? undefined
    const approval = live.knobs.approval ?? undefined
    const seeded = live.seeded
