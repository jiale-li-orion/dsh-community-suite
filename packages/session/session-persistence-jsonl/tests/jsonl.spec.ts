...
import SessionStore, { SessionId, storageRecordsValues } from '@deepseek-ai/dsh-session'
...
interface PlaintextReaderInternals {
  readPlaintextPrefix(buffer: Buffer, signal?: AbortSignal): Promise<unknown>
}
...
describe('JsonlSessionPersistence: durability and crash semantics', () => {
  ...
  it('yields while scanning one oversized plaintext artifact', async () => {
    const header = meta('cancel-plaintext-large-artifact')
    const event = {
      type: 'turn/start',
      seq: 0,
      time: 1,
      data: { turn: 1, padding: 'x'.repeat(1_100_000) },
    }
    ...
    const reason = new Error('cancel between plaintext JSONL scan slices')
    const reader = ctx.sessionPersistence as unknown as PlaintextReaderInternals
    vi.spyOn(performance, 'now').mockReturnValueOnce(0).mockReturnValue(501)
    const pending = reader.readPlaintextPrefix(buffer, controller.signal)
  ...
  it('retries a full-prefix read when the file revision changes during the read', async () => {
    ...
    const stored = await persistence.loadStored(m.id)
    expect(stored?.log.kind).toBe('packed')
    if (stored?.log.kind !== 'packed') throw new Error('JSONL must retain packed restore records')
    expect(stored.log.eventCount).toBe(oneTurnLog().length)
    expect([...storageRecordsValues(stored.log.records)]).toEqual(oneTurnLog())
    expect(statRace.reads).toBe(4)
  })
...
describe('JsonlSessionPersistence: default packed chunk rows', () => {
  ...
  it('scanLog: a packed row advances the seq cursor by its whole run', () => {
    ...
    expect(scanned.eventCount).toBe(5)
