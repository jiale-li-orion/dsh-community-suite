...
interface ZstdReaderInternals {
  readZstdPrefix(buffer: Buffer, signal?: AbortSignal): Promise<{
    log: { kind: 'packed'; records: StorageRecord[]; eventCount: number }
  }>
}
...
describe('JsonlSessionPersistence: default Zstandard encoding', () => {
  ...
  it('yields while scanning one oversized plaintext frame', async () => {
    const root = await freshRoot()
    const ctx = await mount(root)
    const header = meta('cancel-zstd-large-frame')
    const headerFrame = await compressZstdFrame(`${JSON.stringify(toHeaderLine(header))}\n`)
    const event = {
      type: 'turn/start',
      seq: 0,
    ...
    const eventFrame = await compressZstdFrame(`${JSON.stringify(event)}\n`)
    const controller = new AbortController()
    const reason = new Error('cancel between JSONL scan slices')
    const reader = ctx.sessionPersistence as unknown as ZstdReaderInternals
    vi.spyOn(performance, 'now').mockReturnValueOnce(0).mockReturnValue(501)
    const pending = reader.readZstdPrefix(Buffer.concat([headerFrame, eventFrame]), controller.signal)
    queueMicrotask(() => { controller.abort(reason) })
  ...
  it('continues decoding every frame after a slice deadline yields', async () => {
    ...
    expect(prefix.log.eventCount).toBe(events.length)
    expect([...storageRecordsValues(prefix.log.records)]).toEqual(events)
  })
