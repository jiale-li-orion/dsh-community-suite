...
import {
  ...
  type PackedStoredLog, type PersistenceBackend, type SessionLocation, type SessionPersistenceSnapshot,
  type SessionInspection, type SessionPersistenceRevision as PersistenceRevision, type SessionRawArtifact,
  type StoredPrefix,
} from '@deepseek-ai/dsh-session-persistence'
import { storageRecordsValues } from '@deepseek-ai/dsh-session'
import type { SessionEvent, SessionId, SessionHeader, SessionPreparation, StorageRecord } from '@deepseek-ai/dsh-session'
import {
  encodeSegment, eventLines, logPath, logSuffix, parseHeaderMeta, projectDir, sessionDir,
...
/**
 * Internal scheduling constants, not deployment configuration: bound cold-read
 * host-thread occupancy while keeping decoder and `setImmediate` overhead low.
 * Native decoding of one frame remains indivisible; JSONL scanning does not.
 */
const READ_YIELD_INTERVAL_MS = 10
const JSONL_SCAN_SLICE_BYTES = 1024 * 1024

interface JsonlScanSchedule {
  yieldDeadline: number
}

/** Feed decoded JSONL bytes through bounded host-thread slices. */
async function scanJsonlSlices(
  scanner: SessionLogScanner,
  plaintext: Buffer,
  schedule: JsonlScanSchedule,
  hasMoreInput: boolean,
  signal?: AbortSignal,
): Promise<void> {
  for (let offset = 0; offset < plaintext.length;) {
    const end = Math.min(plaintext.length, offset + JSONL_SCAN_SLICE_BYTES)
    scanner.write(plaintext.subarray(offset, end))
    offset = end
    if ((offset < plaintext.length || hasMoreInput)
      && performance.now() >= schedule.yieldDeadline) {
      await scheduler.yield()
      signal?.throwIfAborted()
      schedule.yieldDeadline = performance.now() + READ_YIELD_INTERVAL_MS
    }
  }
}
...
interface JsonlTornMarker {
  truncateTo: number
  recoveredRecords: StorageRecord[]
}

interface JsonlReadPrefix {
  meta: SessionHeader
  log: PackedStoredLog
...
export class JsonlSessionPersistence extends SessionPersistence implements PersistenceBackend<JsonlTornMarker> {
  ...
  private async readPrefix(
    ...
    let prefix: JsonlReadPrefix
    try {
      if (this.compression === 'zstd') {
        prefix = await this.readZstdPrefix(buffer, signal)
      } else {
        prefix = await this.readPlaintextPrefix(buffer, signal)
      }
    } catch (error: unknown) {
  ...
  /** Scan one plaintext artifact cooperatively with the compressed read path. */
  private async readPlaintextPrefix(
    buffer: Buffer,
    signal?: AbortSignal,
  ): Promise<JsonlReadPrefix> {
    signal?.throwIfAborted()
    ...
    const schedule = { yieldDeadline: performance.now() + READ_YIELD_INTERVAL_MS }
    await scanJsonlSlices(scanner, buffer.subarray(headerEnd + 1), schedule, false, signal)
    signal?.throwIfAborted()
    const scanned = scanner.finish()
    return {
      meta: scanned.meta,
      log: { kind: 'packed', records: scanned.records, eventCount: scanned.eventCount },
      ...scanned.committedBytes < buffer.byteLength
        ? { tornMarker: { truncateTo: scanned.committedBytes, recoveredRecords: [] } }
        : {},
    }
  }
  ...
  private async readZstdPrefix(
    ...
    try {
      ...
      for (const plaintext of decodedFrames) {
        signal?.throwIfAborted()
        remainingFrames -= 1
        await scanJsonlSlices(scanner, plaintext, schedule, remainingFrames > 0, signal)
      }
      signal?.throwIfAborted()
      ...
      if (tornStart === undefined) {
        ...
        return {
          meta: prefix.meta,
          log: { kind: 'packed', records: prefix.records, eventCount: prefix.eventCount },
        }
      }
      ...
      signal?.throwIfAborted()
      await scanJsonlSlices(scanner, recoveredPlaintext, schedule, false, signal)
      ...
      return {
        ...
        log: {
          kind: 'packed',
          records: recoveredPrefix.records,
          eventCount: recoveredPrefix.eventCount,
        },
        tornMarker: {
          truncateTo: tornStart,
          recoveredRecords: structuredClone(recoveredPrefix.records.slice(complete.recordCount)),
        },
      }
    } catch (error) {
      /* v8 ignore next -- decoder failure plus concurrent abort is timing-dependent */
  ...
  async commitRepair(
    ...
    const recoveredEvents = tornMarker === undefined
      ? []
      : [...storageRecordsValues(tornMarker.recoveredRecords)]
