 ...
 * either way: {@link scanLog} validates both forms and exposes the same logical
 * event stream while retaining eligible rows for packed Session restoration.
 * The switch therefore changes only newly written bytes.
...
class SessionLogScan {
  ...
  /** Explicit compatibility view; durable restore paths consume {@link records}. */
  get events(): SessionEvent[] {
    return [...storageRecordsValues(this.records)]
  }
}
...
export class SessionLogScanner {
   ...
   * @returns byte, committed-prefix, storage-record, and logical-event cursors.
   */
  checkpoint(): { inputBytes: number; committedBytes: number; recordCount: number; eventCount: number } {
    return {
  ...
  finish(): SessionLogScan {
    this.finished = true
    return new SessionLogScan(this.meta, this.records, this.eventCount, this.committedBytes)
  }
  ...
  private consumeEventLine(line: Buffer, endByte: number): void {
    ...
    try {
      record = adoptStorageRecord(JSON.parse(line.toString('utf8')))
    } catch {
      this.issue ??= new Error(`corrupt session log: unparsable committed event at line ${this.eventLine}`)
      return
    }

    const closesTurn = !isChunkRow(record) && record.type === 'turn/end'
    if (this.issue !== undefined) {
      if (closesTurn) throw this.issue
      return
    }

    const start = isChunkRow(record)
      ? storageRecordStart(record)
      : (record as { seq?: unknown } | null)?.seq
    if (start !== this.eventCount) {
      this.issue = new Error(
        `corrupt session log: seq gap in committed region at line ${this.eventLine} `
        + `(expected ${this.eventCount}, got ${String(start)})`,
      )
      if (closesTurn) throw this.issue
      return
    }
    this.records.push(record)
    this.eventCount += storageRecordLength(record)
