...
Cold reads scan plaintext JSONL in bounded byte slices and yield after a short CPU interval; compressed reads apply the same schedule between decoded frame output. One native frame decode and one `JSON.parse` record remain indivisible. Parsed packed rows transfer directly into the unpublished Session, and the parser releases each accepted array slot during adoption, so restore does not retain a second expanded event log while preserving cancellation and host I/O progress.
...
- **Non-mutating inspection.** `inspect()` returns an immutable balanced `SessionInspection`. Its stable `log` cut preserves packed retention and bounded reads; the lazy `events` getter materializes the complete logical array only when explicitly accessed. Inspection may synthesize recovery closers in memory without truncating an incomplete tail or changing the lightweight revision.
