...
冷读取会按有界字节切片扫描明文 JSONL，并在短 CPU 区间后让出执行权；压缩读取会在解码 frame 输出之间应用同一调度。单次原生 frame 解码和单条记录的 `JSON.parse` 仍不可拆分。解析出的打包行会直接移交给未发布 Session，解析器在接纳过程中逐项释放数组槽位，因此恢复过程不会同时保留第二份展开后的事件日志，同时仍可观察取消并让宿主 I/O 取得进展。
...
- **非修改式检查。**`inspect()` 返回不可变、平衡的 `SessionInspection`。其稳定 `log` 切片保留打包存储和有界读取；惰性 `events` getter 只在被显式访问时实体化完整逻辑数组。检查可以在内存中合成恢复 closer，但不会截断不完整尾部或更改轻量修订。
