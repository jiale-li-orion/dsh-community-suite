 ...
 */
interface StoredPrefixBase<TornMarker> {
  meta: SessionHeader
...
/** Compact logical event source retained by a backend parser. */
export interface PackedStoredLog {
  readonly kind: 'packed'
  /** Fresh validated storage records whose ownership transfers to Session. */
  records: StorageRecord[]
  /** Logical event count represented by `records`. */
  eventCount: number
}

/** Ordinary detached event array returned by row-oriented backends. */
export interface ExpandedStoredLog {
  readonly kind: 'events'
  events: SessionEvent[]
}

/** Exclusively owned logical log representation returned by a backend. */
export type StoredLog = ExpandedStoredLog | PackedStoredLog

/**
 * A backend may return ordinary events or a validated packed representation.
 * The packed path is coordinator-private: public inspections expose one
 * logical {@link SessionLogCut}, with complete array materialization available
 * only through its explicit compatibility view.
 */
export type StoredPrefix<TornMarker = unknown> = StoredPrefixBase<TornMarker> & { log: StoredLog }
...
interface PreparedSessionSource<TornMarker> {
  ...
  /** Logical durable cursor after in-memory recovery closers. */
  ...
  /** Created only when a caller explicitly requests the complete inspection. */
  inspection?: SessionInspection
}

/** Materialize and cache the public complete-log view on explicit inspection. */
function preparedInspection<TornMarker>(source: PreparedSessionSource<TornMarker>): SessionInspection {
  source.inspection ??= sessionInspection(source.meta, prefixLogCut(source.session.readLog(), source.cursor))
  return source.inspection
}

/** Validate one half-open range against a bounded inspection cut. */
function inspectionRange(length: number, from = 0, to = length): readonly [number, number] {
  if (!Number.isSafeInteger(from) || !Number.isSafeInteger(to)
    || from < 0 || from > to || to > length) {
    throw new RangeError(`inspection log range [${String(from)}, ${String(to)}) is outside [0, ${length})`)
  }
  return [from, to]
}

/** Restrict a stable Session cut to the logical prefix represented by persistence. */
function prefixLogCut(log: SessionLogCut, length: number): SessionLogCut {
  if (length === log.length) return log
  return Object.freeze({
    length,
    at(seq: number): SessionEvent | undefined {
      if (!Number.isSafeInteger(seq) || seq < 0) {
        throw new RangeError(`inspection log seq must be a non-negative safe integer, got ${String(seq)}`)
      }
      return seq < length ? log.at(seq) : undefined
    },
    values(from?: number, to?: number): Iterable<SessionEvent> {
      const [start, end] = inspectionRange(length, from, to)
      return log.values(start, end)
    },
    reverseValues(from?: number, to?: number): Iterable<SessionEvent> {
      const [start, end] = inspectionRange(length, from, to)
      return log.reverseValues(start, end)
    },
    valuesOf<T extends SessionEventType>(
      ...
      const [start, end] = inspectionRange(length, from, to)
      return log.valuesOf(types, start, end)
    },
    reverseValuesOf<T extends SessionEventType>(
      ...
      const [start, end] = inspectionRange(length, from, to)
      return log.reverseValuesOf(types, start, end)
    },
    chunkRuns(from?: number, to?: number) {
      const [start, end] = inspectionRange(length, from, to)
      return log.chunkRuns(start, end)
    },
    materialize(from?: number, to?: number): readonly SessionEvent[] {
      const [start, end] = inspectionRange(length, from, to)
      return log.materialize(start, end)
    },
  })
}

/** Build an immutable inspection whose complete event-array compatibility view is lazy. */
function sessionInspection(meta: SessionHeader, log: SessionLogCut): SessionInspection {
  let events: readonly SessionEvent[] | undefined
  return Object.freeze({
    meta,
    log,
    get events(): readonly SessionEvent[] {
      events ??= log.materialize()
      return events
    },
  })
}
...
function assertSupportedEvent(event: SessionEvent, id: SessionId): void {
...
/** Reject retired event vocabulary in one logical event stream. */
function assertSupportedEvents(events: Iterable<SessionEvent>, id: SessionId): void {
  for (const event of events) assertSupportedEvent(event, id)
}
...
/** Stream upgraded, validated snapshots while retaining only legacy identity state. */
function* snapshotStoredEventValues(
  events: Iterable<SessionEvent>,
  id: SessionId,
): Generator<SessionEvent> {
  const messageIds = new Map<number, PersistedMessageId>()
  for (const event of events) {
    assertSupportedEvent(event, id)
...
/** Materialize one explicit complete stored-event read. */
function snapshotStoredEvents(events: Iterable<SessionEvent>, id: SessionId): SessionEvent[] {
  return [...snapshotStoredEventValues(events, id)]
}

/** Upgrade an exclusively owned backend result without expanding packed rows. */
function adoptStoredRecords<TornMarker>(
  stored: StoredPrefix<TornMarker>,
  id: SessionId,
): { records: StorageRecord[]; eventCount: number } {
  const records: StorageRecord[] = stored.log.kind === 'packed'
    ? stored.log.records
    : stored.log.events
  const declaredCount = stored.log.kind === 'packed' ? stored.log.eventCount : records.length
  const messageIds = new Map<number, PersistedMessageId>()
  let eventCount = 0
  for (const [index, record] of records.entries()) {
    if (isChunkRow(record)) {
      eventCount += storageRecordLength(record)
      continue
    }
    assertSupportedEvent(record, id)
    const event = record
    const migratedStart = migrateLegacyTurnStartEvent(event, id)
    const migratedTurn = migrateLegacyTurnEndEvent(migratedStart, id)
    const migratedSteering = migrateLegacySteeringEvent(migratedTurn, id)
  ...
  if (eventCount !== declaredCount) {
    throw new Error(`session "${id}" packed event count ${declaredCount} does not match ${eventCount} logical events`)
  }
  return { records, eventCount }
}

/** Iterate one backend result as logical events, expanding rows only on demand. */
function storedEventValues<TornMarker>(stored: StoredPrefix<TornMarker>): Iterable<SessionEvent> {
  return stored.log.kind === 'packed' ? storageRecordsValues(stored.log.records) : stored.log.events
}
...
export class PersistenceCoordinator<TornMarker = unknown> {
  ...
  async load(id: SessionId): Promise<SessionInspection> {
    for (;;) {
      ...
      this.preparations.discard(reservation)
      return preparedInspection(reservation.source)
    }
  }
  ...
  async inspect(id: SessionId, signal?: AbortSignal): Promise<SessionInspection> {
    for (;;) {
      ...
      try {
        ...
        if (current) return preparedInspection(source)
        if (this.preparations.discardReady(id, source) === 'retained') {
          return preparedInspection(source)
        }
      } catch (error: unknown) {
        signal?.throwIfAborted()
        const attached = this.ctx.sessions.get(id)
  ...
  private async readStoredPrefix(
    ...
    const events = snapshotStoredEvents(storedEventValues(stored), id)
  ...
  private async prepareCore(id: SessionId): Promise<PreparedSessionSource<TornMarker>> {
    ...
    if (stored === undefined) throw new Error(`session "${id}" not found`)
    try {
      const { meta, revision, tornMarker } = stored
      this.assertStoredId(id, meta)
      this.assertVersion(meta)
      const { records, eventCount } = adoptStoredRecords(stored, id)
      this.assertRecordsSupported(meta, records)

      // Preserve complete interrupted events and synthesize only missing closers.
      const closers = interruptedTurnClosersFromRecords(records).map(adoptSessionEvent)
      const cursor = eventCount + closers.length
      const session = await Session.fromRestoreRecordsAsync(id, records, closers, meta)
      return {
        meta: session.header,
        cursor,
        session,
  ...
  private async commitPrepared(
    ...
    if (source.tornMarker !== undefined || source.closers.length > 0) {
      await this.backend.commitRepair(source.meta, source.tornMarker, source.closers)
  ...
  private async isPreparedSourceCurrent(
    ...
    return await this.backend.readStoredRevision(source.meta.id, signal) === source.revision
  }
  ...
  private async loadLiveSnapshot(session: Session): Promise<SessionInspection> {
    ...
    if (log.length === 0) throw new Error(`session "${session.id}" not found`)
    if (interruptedTurnClosersFromLog(log).length > 0) {
   ...
   */
  private assertEventsSupported(meta: SessionHeader, events: Iterable<SessionEvent>): void {
    for (const event of events) {
  ...
  /** Refuse unknown required event records without decoding packed chunk rows. */
  private assertRecordsSupported(meta: SessionHeader, records: readonly StorageRecord[]): void {
    for (const record of records) {
      if (isChunkRow(record)) continue
      this.assertEventsSupported(meta, [record])
    }
  }
  ...
  private attachPrepared(
    ...
    const log = session.readLog()
    const suffix = state.cursor < log.length ? log.materialize(state.cursor) : []
  ...
  /**
   * Whether a live session's stable seed cut reproduces the first `cursor` persisted
   ...
   */
  private async seedMatchesPersisted(id: SessionId, seed: SessionLogCut, cursor: number): Promise<boolean> {
    ...
    this.assertStoredId(id, stored.meta)
    let index = 0
    for (const event of snapshotStoredEventValues(storedEventValues(stored), id)) {
      if (index >= cursor) break
      const seedEvent = seed.at(index)
      if (seedEvent === undefined || JSON.stringify(seedEvent) !== JSON.stringify(event)) return false
      index += 1
    }
    return index === cursor
  }
   ...
   */
  private async onCreated(session: Session, seed: SessionLogCut): Promise<void> {
    const id = session.header.id
    ...
    if (tracked !== undefined) {
      ...
      if (tracked.owner === undefined) {
        ...
        if (tracked.cursor < seed.length) await this.appendCore(id, seed.materialize(tracked.cursor))
        return
      }
    ...
    if (seed.length > 0) await this.appendCore(id, seed.materialize())
  }
   ...
   */
  private async adoptLivePrefix(session: Session, seed: SessionLogCut, stored: StoredPrefix<TornMarker>): Promise<void> {
    const { meta, tornMarker } = stored
    ...
    this.assertVersion(meta)
    let storedLength = 0
    for (const event of snapshotStoredEventValues(storedEventValues(stored), session.header.id)) {
      this.assertEventsSupported(meta, [event])
      const seedEvent = seed.at(storedLength)
      if (seedEvent === undefined || JSON.stringify(seedEvent) !== JSON.stringify(event)) {
    ...
    if (storedLength < seed.length) {
      await this.appendCore(session.header.id, seed.materialize(storedLength))
    }
  }
