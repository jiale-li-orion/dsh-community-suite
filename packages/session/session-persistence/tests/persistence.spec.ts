...
import type { SessionEvent, SessionHeader, SessionLogCut } from '@deepseek-ai/dsh-session'
import {
  DEFAULT_PREPARED_SESSION_CACHE_SIZE, DEFAULT_WRITE_BATCH_MAX_DELAY_MS, MAX_WRITE_BATCH_DELAY_MS,
  SessionPersistence, SessionPersistenceRevision, PersistenceCoordinator,
  type PersistenceBackend, type SessionInspection, type SessionPersistenceSnapshot, type StoredPrefix, type StoredSuffix,
} from '../src/index.ts'
...
class MemoryPersistence extends SessionPersistence implements PersistenceBackend<never> {
  ...
  async loadStored(id: SessionId): Promise<StoredPrefix<never> | undefined> {
    ...
    return {
      ...
      log: { kind: 'events', events: structuredClone(entry.events) },
...
describe('PersistenceCoordinator bounded writes', () => {
  ...
  it('adopts an HMR prefix without materializing the live seed', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    const id = SessionId('bounded-hmr-seed')
    ...
    backend.store.set(id, {
      ...
      events: structuredClone(base.materialize()) as SessionEvent[],
    })
