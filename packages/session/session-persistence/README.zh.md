...
| `load(id): Promise<SessionInspection>` | 转换同一格式版本中受支持的旧记录后，返回不可变、平衡的逻辑日志，并提交冷恢复。实时 load 先 flush 其稳定切片，并在轮次开放时拒绝；冷 load 保留中断的最终轮次，并用合成 `tool/result`/`step/end?`/`turn/end {interrupted}` 事件持久关闭它。只丢弃撕裂尾部碎片；已提交损坏和格式错误的记录以 `SessionPersistenceCorruptionError` 拒绝，不支持的格式 `version` 或本构建不认识且信封未带 `ignorable` 标记的事件类型以 `SessionFormatUnsupportedError` 拒绝，消息说明拒绝方向，并在后端为每个会话保留独立文件时给出原始日志路径。 |
| `inspect(id, signal?): Promise<SessionInspection>` | 返回已经升级、验证和深度冻结的逻辑视图，但不提交恢复或发布 Session。冷视图会获得仅存在于内存的合成恢复 closer，物理撕裂尾部保持不变；实时状态下的视图则是当前日志的稳定切片，可能包含开放的轮次。基于协调器的实现会在有界 LRU 中保留该冷状态下未发布的 Session 本身，供后续 `prepare` 使用，但已存储修订值变化后会丢弃并重新读取。同 id 检查共享进行中的读取。 |
...
`SessionInspection.log` 是主要的不可变读取接口。它支持有界读取和按类型读取，不要求构造完整事件数组，也可以继续保留 Session 私有的打包分片行。`SessionInspection.events` 仍是惰性兼容 getter：首次访问会显式实体化完整逻辑日志；历史分页、投影恢复和授权扫描等消费方则直接使用 `log`。
...
崩溃修复只适用于冷状态。对于已有活动会话的 id，`load(id)` 捕获权威内存日志，等待该切片持久，并只在平衡时返回；活动会话中开放的轮次会被拒绝，而不会收到合成中断 closer。对于冷 id，检查会把已验证存储记录移交给一个未发布 Session。打包分片行保持打包状态；记录被接纳后立即释放解析器自有槽位；构造过程会在有界 CPU 区间之间让出执行权。只有来源修订值仍然是当前值时，重复检查才会复用这个 Session。`prepare(id)` 在修复前执行相同校验，预留该 Session 本身，提交任何待处理的撕裂尾部或中断轮次修复，并将其返回用于发布。HMR（热模块替换）接管通过 `loadStored` 读取，应用协调器 cwd 检查，并绝不关闭活动轮次。
