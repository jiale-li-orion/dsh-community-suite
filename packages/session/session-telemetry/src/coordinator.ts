...
export class SessionTelemetryCoordinator {
  ...
  captureSession(session: Session, throughSeq?: number): void {
    ...
    const end = throughSeq === undefined
      ? log.length
      : Math.max(0, Math.min(log.length, throughSeq + 1))
    const trackedEnd = Math.max(0, Math.min(end, cursor + 1))
    const seen = this.seen(session)
    for (const run of log.chunkRuns(0, trackedEnd)) seen.add(`${run.turn}:${run.step}`)

    // Containment is PER EVENT: one rejected record is withheld fail-closed
    // while the rest of the unhanded suffix proceeds. Historical chunks at or
    // below the cursor were summarized above without decoding their payloads.
    const replayFrom = Math.max(0, cursor + 1)
    if (replayFrom >= end) return
    for (const event of log.values(replayFrom, end)) {
      this.contain(() => {
        this.captureEvent(session, event)
      })
    }
