...
import SessionStore, { SessionId, type Session, type SessionEvent, type SessionLogCut } from '@deepseek-ai/dsh-session'
...
describe('SessionTelemetryCoordinator adoption', () => {
  ...
  it('resume shape: a full-log seed exports only its own end-seed and rebuilds the chunk projection', async () => {
    ...
    const base = resumed.readLog()
    const values = vi.fn((from?: number, to?: number) => base.values(from, to))
    const chunkRuns = vi.fn((from?: number, to?: number) => base.chunkRuns(from, to))
    ...
    vi.spyOn(resumed, 'readLog').mockReturnValue(tracked)
    await ctx.plugin({
      name: 'fake-telemetry',
      inject: ['sessions'],
      apply: (inner: Context) => void new SessionTelemetryCoordinator(inner, backend),
    ...
    expect(chunkRuns).toHaveBeenCalledExactlyOnceWith(0, 2)
    expect(values).toHaveBeenCalledExactlyOnceWith(2, 3)
