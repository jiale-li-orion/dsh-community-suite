...
/** Project one eligible human prompt without retaining its full event. */
function titleUserMessage(event: SessionEvent): SessionTitleUserMessage | undefined {
  ...
  return deepFreeze({ seq: event.seq, text })
}
...
export function collectSessionTitleMessages(
  ...
  for (const event of events) {
    ...
    const message = titleUserMessage(event)
    if (message !== undefined) messages.push(message)
  }
  return messages
}

/** Build the detached public snapshot for one logged title event. */
function titleSnapshot(event: SessionEvent<'session/title'>): SessionTitleSnapshot {
  return deepFreeze({
...
/** Minimal replay state for synchronous title and prompt reads. */
interface SessionTitleReadState {
...
export class SessionTitleService extends Service {
  ...
  private readonly reads = new SessionEventFold<SessionTitleReadState>(
    () => ({ title: undefined, messages: [] }),
    (state, event) => {
      const message = titleUserMessage(event)
      if (message !== undefined) state.messages.push(message)
      if (event.type === 'session/title') state.title = titleSnapshot(event)
    },
    ['user/message', 'session/title'],
  )
  ...
  constructor(ctx: Context, config: Config) {
    ...
    ctx.on('session/event', (session, event) => {
      this.reads.advance(session, event)
      switch (event.type) {
        case 'user/message':
  ...
  get(session: Session): SessionTitleSnapshot | undefined {
    return this.readStateFor(session).title
  }
  ...
  async refresh(session: Session, signal?: AbortSignal): Promise<SessionTitleSnapshot | undefined> {
    ...
    const messages = this.messagesThrough(this.readStateFor(session))
  ...
  private onUserMessage(session: Session, event: Extract<SessionEvent, { type: 'user/message' }>): void {
    ...
    if (titleUserMessage(event) === undefined) return
    const read = this.readStateFor(session)
    // A user rename pins the title: no automatic revision may override it.
    if (read.title?.source.kind === 'user') return
    const registration = this.registration
    if (registration !== undefined && !registration.closing) {
      const messages = this.messagesThrough(read, event.seq)
      const shouldSchedule = registration.provider.automatic === 'all-prompts'
        || (session.header.parentSession === undefined && messages.length === 1 && read.title === undefined)
      if (shouldSchedule) {
        const state = this.stateFor(session)
  ...
  private onMainRequest(options: GenerateOptions): void {
    ...
    let boundary: SessionEvent<'step/start' | 'step/end'> | undefined
    for (const event of session.readLog().reverseValuesOf(['step/start', 'step/end'])) {
      boundary = event
      break
    }
  ...
  private async runProvider(
    ...
    try {
      ...
      await this.ensureFallback(session)
      this.assertCurrent(session, work)
      const messages = this.messagesThrough(this.readStateFor(session), work.throughSeq)
  ...
  /** Replay an unseen suffix and return synchronous title read state. */
  private readStateFor(session: Session): SessionTitleReadState {
    return this.reads.read(session)
  }

  /** Return a detached immutable prompt list through one optional event boundary. */
  private messagesThrough(
    state: SessionTitleReadState,
    throughSeq?: number,
  ): readonly SessionTitleUserMessage[] {
    const messages = throughSeq === undefined
      ? [...state.messages]
      : state.messages.filter(message => message.seq <= throughSeq)
  ...
  private async ensureFallback(session: Session): Promise<SessionTitleSnapshot | undefined> {
    ...
    const [first] = this.readStateFor(session).messages
