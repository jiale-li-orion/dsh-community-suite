...
- `ProjectionDefinition<K, S>`——`{ key, schema, init(), eventTypes, apply(state, event), view(state), stateVersion }`：由三个纯同步函数外加若干声明构成的状态驱动计算单元（state-driven computation unit），绝不是一个不透明的 getter。`eventTypes` 是被消费的事件判别值列表或 `'all'`。

## 约定

- **框架负责驱动，领域负责计算。** 注册表只订阅一次 `session/event`，并在每个已提交事件上推进所有单元的水位线，但只对单元声明的事件类型调用 `apply`；声明 `'all'` 的单元接收所有事件。惰性或冷回放会合并同一水位线上的单元声明，并使用稳定日志切面的类型索引，因此不会解码无关的打包分片。领域不持有任何订阅。cell（每会话每单元一份 `{state, observedSeq}`，以 WeakMap 为键）在单元或 Session 首次触达时惰性构建。
- **同引用即无下游工作。** 对声明范围内但不改变值的事件，`apply` 必须返回同一个状态引用；驱动以 `Object.is` 把守变更流。
...
- **实时分发仍会在每个事件上检查每个已注册单元**——`eventTypes` 避免无关的 `apply` 调用与冷解码，但分发成本仍与已注册单元数成正比。如果单元表显著增大，可以增加事件类型到注册项的索引，而无需改变公开约定。
