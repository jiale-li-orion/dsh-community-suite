 ...
 * drive relation (every committed `session/event` advances every unit and
 * declared types reach `apply`) would require re-running the drive to check —
 * duplicating the implementation rather than detecting drift — and the
 * served-value relation (every served key has a live registration) lives on
 * each carrier's wire path, which
