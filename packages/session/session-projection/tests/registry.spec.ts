...
describe('SessionProjectionRegistry drive', () => {
  ...
  it('initializes every missing selected-type unit through one indexed log scan', async () => {
    const { ctx, session } = await harness()
    mark(session, ['a'])
    session.append('turn/start', { turn: 1 })
    mark(session, ['b'])
    const base = session.readLog()
    const values = vi.fn((from?: number, to?: number) => base.values(from, to))
    const valuesOf = vi.fn()
    const tracked: SessionLogCut = {
      length: base.length,
      at: seq => base.at(seq),
      values,
      reverseValues: (from, to) => base.reverseValues(from, to),
      valuesOf: (types, from, to) => {
        valuesOf(types, from, to)
        return base.valuesOf(types, from, to)
      },
      reverseValuesOf: (types, from, to) => base.reverseValuesOf(types, from, to),
      chunkRuns: (from, to) => base.chunkRuns(from, to),
      materialize: (from, to) => base.materialize(from, to),
    }
    vi.spyOn(session, 'readLog').mockReturnValue(tracked)

    ctx.sessionProjections.register(marksUnit())
    ctx.sessionProjections.register({ ...countUnit(), eventTypes: ['test/mark'] })
    const snapshot = ctx.sessionProjections.snapshot(session)

    expect(snapshot.values['test/marks']).toEqual({ marks: ['b'] })
    expect(snapshot.values['test/count']).toBe(2)
    expect(valuesOf).toHaveBeenCalledTimes(1)
  ...
  it('restoreLog uses the type index when every registered unit names its inputs', async () => {
    const { ctx, session } = await harness()
    ctx.sessionProjections.register(marksUnit())
    mark(session, ['first'])
    session.append('turn/start', { turn: 1 })
    ...
    const values = vi.fn((from?: number, to?: number) => base.values(from, to))
    const valuesOf = vi.fn()
    const materialize = vi.fn((from?: number, to?: number) => base.materialize(from, to))
    const tracked: SessionLogCut = {
      length: base.length,
      at: seq => base.at(seq),
      values,
    ...
    const restored = ctx.sessionProjections.restoreLog({}, tracked)

    expect(restored.snapshot).toEqual({
      asOfSeq: 2,
      values: { 'test/marks': { marks: ['last'] } },
    })
    expect(valuesOf).toHaveBeenCalledExactlyOnceWith(['test/mark'], 0, 3)
    expect(values).not.toHaveBeenCalled()
    expect(materialize).not.toHaveBeenCalled()
  })

  it('restoreLog keeps each unit on its own watermark instead of widening an earlier scan', async () => {
    const { ctx, session } = await harness()
    ctx.sessionProjections.register(marksUnit())
    ctx.sessionProjections.register(countUnit())
    mark(session, ['first'])
    ...
    const restored = ctx.sessionProjections.restoreLog({
      // This row must refold from zero.
      'test/marks': { ver: 99, seq: 2, val: { marks: ['stale'] } },
      // The all-events unit is already current and must not make that refold
      // decode unrelated event types.
      'test/count': { ver: 1, seq: 2, val: 3 },
    }, tracked)

    expect(restored.snapshot.values).toEqual({
      'test/marks': { marks: ['last'] },
      'test/count': 3,
    })
    expect(valuesOf).toHaveBeenCalledExactlyOnceWith(['test/mark'], 0, 3)
    expect(values).not.toHaveBeenCalled()
  ...
  it('restoreSession publishes checkpoint-backed cells atomically for later live reads', async () => {
    const { ctx, session } = await harness()
    mark(session, ['first'])
    session.append('turn/start', { turn: 1 })
    mark(session, ['last'])
    ...
    const base = session.readLog()
    const valuesOf = vi.fn()
    vi.spyOn(session, 'readLog').mockReturnValue({
      length: base.length,
      at: seq => base.at(seq),
    ...
    ctx.sessionProjections.restoreSession(session, {
      'test/marks': { ver: 1, seq: 0, val: { marks: ['first'] } },
    })

    expect(valuesOf).toHaveBeenCalledExactlyOnceWith(['test/mark'], 1, 3)
    valuesOf.mockClear()
    expect(ctx.sessionProjections.snapshot(session).values['test/marks']).toEqual({ marks: ['last'] })
    expect(valuesOf).not.toHaveBeenCalled()
  })
