...
import type { Session, SessionEvent, SessionHeader, SessionId, SessionLogCut } from '@deepseek-ai/dsh-session'
...
export class SessionProjectionCache extends Service {
  ...
  snapshotLog(meta: SessionHeader, log: SessionLogCut): ProjectionSnapshot {
    const cached = this.recordFor(meta.id, identityOf(meta))?.rows ?? {}
    const restored = this.ctx.sessionProjections.restoreLog(cached, log)
    if (Object.keys(restored.checkpoint).length > 0) {
      void this.putSoft(meta.id, identityOf(meta), restored.checkpoint, 'detached-log write-back')
    }
  ...
  private installWritePath(): void {
    // Agent setup has committed every preset-owned projection registration by
    // session publication. Seed those cells synchronously before the resume
    // caller can append, while treating the cache as the optional shortcut it
    // is: an invalid row never vetoes session creation.
    this.ctx.on('session/created', (session: Session) => {
      this.restoreLiveSoft(session)
    })
    ...
    // HMR may mount the cache after Sessions are already live. Existing cells
    // remain authoritative; only absent cells are initialized.
    for (const session of this.ctx.sessions.list()) this.restoreLiveSoft(session)
  }

  /** Initialize one live Session from its lifecycle-matching stored rows. */
  private restoreLiveSoft(session: Session): void {
    const record = this.recordFor(session.id, identityOf(session.header))
    if (record === undefined) return
    try {
      this.ctx.sessionProjections.restoreSession(session, record.rows)
      const rows = this.ctx.sessionProjections.checkpoint(session)
      void this.putSoft(session.id, identityOf(session.header), rows, 'live restore write-back')
    } catch (error) {
      this.ctx.logger.warn(`session projection cache: live restore for "${session.id}" failed (registry will rebuild lazily): ${String(error)}`)
    }
  }
