...
describe('SessionProjectionCache cold read', () => {
  ...
  it('hydrates a published restored Session from its cached row before later live reads', async () => {
    const pool = new MemoryMediaPool()
    seedRow(pool, 'live-restore', { ver: 1, seq: 1, val: { marks: ['a'] } })
    const { ctx } = await harness({ pool })
    const id = SessionId('live-restore')
    const seed: SessionEvent[] = [
      { type: 'turn/start', seq: 0, time: 0, data: { turn: 1 } },
      { type: 'cache-test/mark', seq: 1, time: 1, data: { marks: ['a'] } },
      { type: 'cache-test/mark', seq: 2, time: 2, data: { marks: ['a', 'b'] } },
      { type: 'turn/end', seq: 3, time: 3, data: { turn: 1, reason: { kind: 'completed' } } },
    ]
    const session = ctx.sessions.prepare(id, { seed, meta: { createdAt: 0 } })
    ...
    try {
      ...
      expect(valuesOf).toHaveBeenCalledExactlyOnceWith(['cache-test/mark'], 2, 5)
      valuesOf.mockClear()
      expect(ctx.sessionProjections.snapshot(session).values['cache-test/marks'])
        .toEqual({ marks: ['a', 'b'] })
  ...
  it('restores an exact detached log cut from cache rows and writes the advanced cut back', async () => {
    const pool = new MemoryMediaPool()
    seedRow(pool, 'detached-log', { ver: 1, seq: 1, val: { marks: ['a'] } })
    const { ctx, cache } = await harness({ pool })
    const id = SessionId('detached-log')
    ...
    expect(cache.snapshotLog(session.header, session.readLog())).toEqual({
      asOfSeq: 4,
      values: { 'cache-test/marks': { marks: ['a', 'b'] } },
    })
    await settle()
    expect(storedRows(pool, id)?.['cache-test/marks'])
      .toEqual({ ver: 1, seq: 4, val: { marks: ['a', 'b'] } })
  })
