...
- `Session.fromRestoreRecords()` takes ownership of validated storage records and preserves packed chunk rows while rebuilding the logical log. `Session.fromRestoreRecordsAsync()` produces the same detached Session but yields between bounded restore batches so a cold load does not monopolize the host event loop.
...
- `session.surface` exposes the readonly `SessionSurface` view owned by the session's single incremental surface manager; `replaceGeneration` changes on every committed rewrite and `generationSeq` identifies the replacement that created the current generation.
- `session.readLog()` captures an immutable logical prefix without allocating a complete event array. It provides checked point/range/reverse reads, selected-type iterators backed by a compact type index, chunk-run summaries, and explicit bounded `materialize()`.
- `session.readSurface()` captures one immutable `SessionSurfaceCut` containing the current generation, frozen positional membership, and the exact stable log cut that contains every node. It copies only the surface seq list and is cached until append.
- `session.events` is the compatibility face for consumers that truly need the complete frozen array. It materializes lazily, caches that expansion until append, and never changes a previously returned snapshot.
...
The shared [storage codec](src/chunk-rows.ts) losslessly converts event sequences to compact rows and back. It preserves unrecognized events verbatim and rejects malformed encoded rows; persistence backends decide whether to enable packed writes. Session also uses the same row vocabulary privately: consecutive eligible chunk deltas stay packed after restore and may pack during live append, while every public read preserves the original logical event boundaries, seq values, timestamps, and deep immutability.

### Incremental event folds

`SessionEventFold<State>` centralizes lazy per-session state derived from a declared event-type set. An initialized fold consumes the delivered `session/event` directly, replays only a missed selected-type prefix, ignores notifications already consumed by an earlier read, and advances its watermark only after the reducer succeeds. Reducers that mutate state must remain retryable after an exception.
...
- Persistence plugins: subscribe to `session/event` (write-behind) and drain on `session/flush` (awaited) and fiber dispose. A durable backend transfers validated storage records through `Session.fromRestoreRecordsAsync()` so packed rows remain compact and long restore CPU work yields; the metadata contract (`SessionHeader`, `session.header`) is what such a backend stores beside the log.
...
- Compaction and context management: `dsh-compaction-basic` appends a `user/message` replacement for summary checkpoints, `dsh-compaction-tool-result-pruner` appends a content-only `tool/result` replacement, and `dsh-session-context` captures stable current generations for bounded inspection and rewrite. Tool-pairing policy belongs to the [`dsh-compaction` seam](../../compaction/compaction/README.md); this package owns ordered membership, replacement validation, `replaceGeneration`, and `generationSeq`.
