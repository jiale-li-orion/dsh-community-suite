...
import type { SessionEvent, StorageRecord, SurfaceEvent } from '../src/index.ts'
...
describe('interruptedTurnClosers', () => {
  ...
  it('uses a packed tail member as the closer sequence and timestamp anchor', () => {
    ...
    expect(interruptedTurnClosersFromRecords(records)).toEqual([{
      type: 'turn/end',
      seq: 4,
      time: 25,
  ...
  it('uses the final structural record after an intermediate packed run', () => {
    const records: StorageRecord[] = [
      { type: 'turn/start', seq: 0, time: 10, data: { turn: 1 } },
      {
        type: 'text-chunks',
      ...
      { type: 'step/start', seq: 3, time: 30, data: { turn: 1, step: 2 } },
    ]

    expect(interruptedTurnClosersFromRecords(records)).toEqual([
      { type: 'step/end', seq: 4, time: 30, data: { turn: 1, step: 2 } },
      { type: 'turn/end', seq: 5, time: 30, data: { turn: 1, reason: { kind: 'interrupted' } } },
    ])
  })

  it('uses indexed recovery events plus one final point read from a log cut', () => {
    const events: SessionEvent[] = [
      { type: 'turn/start', seq: 0, time: 10, data: { turn: 1 } },
      { type: 'assistant/chunk', seq: 1, time: 20, data: {
        turn: 1, step: 1, chunk: { type: 'text-delta', index: 0, text: 'a' },
      } },
      { type: 'assistant/chunk', seq: 2, time: 25, data: {
        turn: 1, step: 1, chunk: { type: 'text-delta', index: 0, text: 'b' },
      } },
    ]
    const base = sessionLogCutFromEvents(events)
    let selectedTypes: readonly string[] = []

    const closers = interruptedTurnClosersFromLog({
      length: base.length,
      at: seq => base.at(seq),
    ...
    expect(selectedTypes).not.toContain('assistant/chunk')
