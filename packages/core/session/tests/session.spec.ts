...
import type {
  CreateSessionOptions, SessionEventType, SessionHeader, SessionSurface, SessionSurfaceCut, StorageRecord, TodoItem,
} from '@deepseek-ai/dsh-session'
...
describe('Session', () => {
  ...
  it('restores packed storage records without changing logical event semantics', () => {
    const id = SessionId('packed-restore')
    ...
    const session = Session.fromRestoreRecords(id, records, [], {
      version: SESSION_FORMAT_VERSION,
      id,
      createdAt: 1,
    })

    expect(session.firstLiveSeq).toBe(5)
    expect(session.readLog().materialize(0, 5).map(event => event.seq)).toEqual([0, 1, 2, 3, 4])
    expect(session.readLog().at(2)).toEqual({
      type: 'assistant/chunk',
      seq: 2,
      time: 3,
      data: { turn: 1, step: 1, chunk: { type: 'text-delta', index: 0, text: 'b' } },
    })
    expect(session.readLog().at(5)).toMatchObject({ type: 'session/end-seed', seq: 5 })
    expect(Object.isFrozen(row)).toBe(true)
    expect(Object.isFrozen(row.data)).toBe(true)
    expect(Object.isFrozen(row.data.texts)).toBe(true)
  })

  it('cooperatively restores packed records with the same logical result', async () => {
    const id = SessionId('packed-restore-async')
    const records: StorageRecord[] = [
      { type: 'turn/start', seq: 0, time: 1, data: { turn: 1 } },
      {
        type: 'text-chunks',
    ...
    const now = vi.spyOn(performance, 'now')
      ...
      .mockReturnValue(11)
    let hostTurnAdvanced = false
    setImmediate(() => { hostTurnAdvanced = true })

    try {
      const session = await Session.fromRestoreRecordsAsync(id, records, [], {
        version: SESSION_FORMAT_VERSION,
        id,
        createdAt: 1,
      })

      expect(hostTurnAdvanced).toBe(true)
      expect(session.firstLiveSeq).toBe(4)
      expect(session.readLog().materialize(0, 4).map(event => event.seq)).toEqual([0, 1, 2, 3])
      expect(session.readLog().at(4)).toMatchObject({ type: 'session/end-seed', seq: 4 })
  ...
  it('rejects a packed restore record whose logical range is not contiguous', () => {
    const id = SessionId('packed-restore-gap')
    expect(() => Session.fromRestoreRecords(id, [{
      type: 'text-chunks',
      seq0: 1,
      time0: 1,
  ...
  it('captures immutable bounded log cuts with point and range reads', () => {
    const session = Session.create(SessionId('log-cut'))
    ...
    expect(cut.at(0)).toBe(first)
    expect(cut.at(1)).toBe(second)
    expect(cut.at(2)).toBeUndefined()
    expect(cut.at(99)).toBeUndefined()
    expect([...cut.values()]).toEqual([first, second])
    expect([...cut.values(1, 2)]).toEqual([second])
    expect([...cut.reverseValues()]).toEqual([second, first])
    expect([...cut.reverseValues(0, 1)]).toEqual([first])
    expect([...cut.reverseValuesOf(['turn/start'])]).toEqual([first])
    expect([...cut.reverseValuesOf(['turn/end', 'turn/start'])]).toEqual([second, first])
    expect([...cut.chunkRuns()]).toEqual([])

    const range = cut.materialize(1, 2)
    expect(range).toEqual([second])
    expect(Object.isFrozen(range)).toBe(true)
    expect(() => { (range as SessionEvent[]).push(first) }).toThrow(TypeError)

    session.append('turn/start', { turn: 2 })
    expect(cut.length).toBe(2)
    expect(cut.at(2)).toBeUndefined()
    expect([...cut.values()]).toEqual([first, second])
    expect(session.readLog().length).toBe(3)
  })

  it('captures one immutable surface and matching log cut per revision', () => {
    const session = Session.create(SessionId('surface-cut'))
    session.append('turn/start', { turn: 1 })
    ...
    const before = session.readSurface()
    expectTypeOf(before).toEqualTypeOf<SessionSurfaceCut>()
    expect(session.readSurface()).toBe(before)
    expect(before.logRevision).toBe(3)
    expect(before.log).toBe(session.readLog())
    expect(before.replaceGeneration).toBe(0)
    expect(before.generationSeq).toBeNull()
    expect(before.nodes).toEqual([first.seq, second.seq])
    expect(Object.isFrozen(before)).toBe(true)
    expect(Object.isFrozen(before.nodes)).toBe(true)
    expect(() => { (before.nodes as number[]).push(99) }).toThrow(TypeError)

    session.append('todo/write', { todos: [] })
    const afterLogOnly = session.readSurface()
    expect(afterLogOnly).not.toBe(before)
    expect(afterLogOnly.logRevision).toBe(4)
    expect(afterLogOnly.nodes).toEqual(before.nodes)
    expect(before.log.length).toBe(3)

    const replacement = session.append('user/message', createUserMessage({
      content: [{ type: 'text', text: 'replacement' }],
      source: { kind: 'plugin', plugin: 'surface-cut-test' },
    }), {
      surfaceOp: { op: 'replace', start: first.seq, end: first.seq },
      sourceEventSeqs: [first.seq],
    })
    const afterReplace = session.readSurface()
    expect(afterReplace.logRevision).toBe(5)
    expect(afterReplace.replaceGeneration).toBe(1)
    expect(afterReplace.generationSeq).toBe(replacement.seq)
    expect(afterReplace.nodes).toEqual([replacement.seq, second.seq])
    expect(before.nodes).toEqual([first.seq, second.seq])
    expect(afterLogOnly.nodes).toEqual([first.seq, second.seq])
  })

  it('keeps a captured cut stable while its packed tail row extends', () => {
    const session = Session.create(SessionId('packed-cut'))
    session.append('turn/start', { turn: 1 })
    const chunks = ['a', 'b', 'c'].map(text => session.append('assistant/chunk', {
      turn: 1,
      step: 1,
    ...
    expect([...cut.values()]).toEqual([expect.objectContaining({ type: 'turn/start' }), ...chunks])
    expect([...cut.valuesOf(['turn/start'])]).toEqual([expect.objectContaining({ type: 'turn/start' })])
    expect([...cut.valuesOf(['assistant/chunk'], 2, 4)]).toEqual(chunks.slice(1))
    expect([...cut.chunkRuns(2, 4)]).toEqual([{ from: 2, to: 4, turn: 1, step: 1 }])
    const decoded = cut.at(3)
    expect(decoded).toEqual(chunks[2])
    expect(Object.isFrozen(decoded)).toBe(true)
    expect(Object.isFrozen(decoded?.data)).toBe(true)
  })

  it('rejects invalid log-cut points and ranges without clamping', () => {
    const session = Session.create(SessionId('log-cut-bounds'))
    session.append('turn/start', { turn: 1 })
    const cut = session.readLog()

    for (const seq of [-1, 0.5, Number.NaN, Number.POSITIVE_INFINITY, Number.MAX_SAFE_INTEGER + 1]) {
      expect(() => cut.at(seq)).toThrow(/non-negative safe integer/)
    }
    for (const range of [
      [-1, 0],
      [0.5, 1],
      ...
      expect(() => cut.values(...range)).toThrow(/log range/)
      expect(() => cut.reverseValues(...range)).toThrow(/log range/)
      expect(() => cut.valuesOf(['turn/start'], ...range)).toThrow(/log range/)
      expect(() => cut.reverseValuesOf(['turn/start'], ...range)).toThrow(/log range/)
      expect(() => cut.chunkRuns(...range)).toThrow(/log range/)
      expect(() => cut.materialize(...range)).toThrow(/log range/)
    }
  })

  it('folds delivered events directly and catches only missed ranges', () => {
    const session = Session.create(SessionId('event-fold'))
    const reduced: number[] = []
    const fold = new SessionEventFold(
      () => reduced,
      (state, event) => { state.push(event.seq) },
      ['turn/start', 'turn/end'],
    )

    session.append('turn/start', { turn: 1 })
    expect(fold.read(session)).toEqual([0])

    const second = session.append('turn/end', { turn: 1, reason: { kind: 'completed' } })
    const third = session.append('turn/start', { turn: 2 })
    fold.advance(session, third)
    expect(reduced).toEqual([0, 1, 2])

    fold.advance(session, second)
    expect(reduced).toEqual([0, 1, 2])
  })

  it('leaves a failed fold event retryable', () => {
    const session = Session.create(SessionId('event-fold-retry'))
    ...
    const fold = new SessionEventFold(
      ...
      (state, candidate) => {
        attempts += 1
        if (attempts === 1) throw new Error('injected fold failure')
        state.push(candidate.seq)
      },
      ['turn/start'],
    )

    expect(() => fold.read(session)).toThrow('injected fold failure')
    fold.advance(session, event)
    expect(reduced).toEqual([0])
    expect(attempts).toBe(2)
  })

  it('skips unselected packed event runs while advancing the fold watermark', () => {
    const session = Session.create(SessionId('event-fold-types'))
    const reduced: number[] = []
    const fold = new SessionEventFold(
      () => reduced,
      (state, event) => { state.push(event.seq) },
    ...
    fold.advance(session, end)
    expect(reduced).toEqual([0, 5])
  })
