...
describe('session-log invariants', () => {
  ...
  it('validates packed historical chunk runs against their owning step', async () => {
    const { ctx } = await setup()
    ...
    expect(() => ctx.sessions.create(SessionId('packed-chunks-outside-step'), {
      seed: [
        { type: 'turn/start', seq: 0, time: 1, data: { turn: 1 } },
        ...chunks,
      ],
    })).toThrow(/assistant\/chunk names turn 1\/step 1 but open is turn 1\/step null/)
  })
