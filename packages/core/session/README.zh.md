...
- `Session.fromRestoreRecords()` 接管已验证存储记录的所有权，并在重建逻辑日志时保留打包分片行。`Session.fromRestoreRecordsAsync()` 生成相同的脱离态 Session，但会在有界恢复批次之间让出执行权，使冷加载不会独占宿主事件循环。
...
- `session.surface` 暴露只读 `SessionSurface` 视图，由会话唯一的增量 surface 管理器所有；每次提交重写，`replaceGeneration` 都会变化，`generationSeq` 则标识创建当前代际的替换。
- `session.readLog()` 无需分配完整事件数组即可捕获一个不可变的逻辑前缀。它提供经过校验的点读取、区间读取与反向读取，基于紧凑类型索引的按类型迭代器，分片连续段摘要，以及显式的有界 `materialize()`。
- `session.readSurface()` 捕获不可变的 `SessionSurfaceCut`，包含当前代际、冻结的位置成员关系，以及含有全部节点的准确稳定日志 cut。它只复制表层 seq 列表，并在 append 前缓存。
- `session.events` 是确实需要完整冻结数组的消费方所使用的兼容接口。它惰性实体化，在下次 append 前缓存该展开结果，并且绝不改变之前返回的快照。
...
共享的[存储编解码器](src/chunk-rows.ts)在事件序列与紧凑行之间无损转换。它会逐字保留无法识别的事件，并拒绝形态错误的编码行；是否启用打包写入由持久化后端决定。Session 也会私下使用同一行词汇：符合条件的连续分片 delta 在恢复后保持打包状态，在实时 append 期间也可以被打包；所有公共读取仍保留原始逻辑事件边界、seq、时间戳和深度不可变性。

### 增量事件折叠

`SessionEventFold<State>` 统一实现根据已声明事件类型集合派生的惰性逐会话状态。已经初始化的折叠会直接消费送达的 `session/event`，只回放遗漏的选定类型前缀，忽略已被更早读取消费的通知，并且只在 reducer 成功后推进水位线。会修改状态的 reducer 在异常后必须保持可重试。
...
- 持久化插件：订阅 `session/event`（延后写入），并在 `session/flush`（受等待）及 fiber dispose（资源释放）时排空。持久后端通过 `Session.fromRestoreRecordsAsync()` 移交已验证存储记录，使打包行保持紧凑，并让长时间恢复 CPU 工作主动让出执行权；这类后端会把元数据约定（`SessionHeader`、`session.header`）与日志一同存储。
...
- 压缩与上下文管理：`dsh-compaction-basic` 为摘要检查点追加替换用 `user/message`，`dsh-compaction-tool-result-pruner` 追加仅修改内容的 `tool/result` 替换，`dsh-session-context` 则捕获稳定当前代际，用于有界检查与改写。工具配对策略归 [`dsh-compaction` seam](../../compaction/compaction/README.md) 所有；此包拥有有序成员关系、替换校验、`replaceGeneration` 与 `generationSeq`。
