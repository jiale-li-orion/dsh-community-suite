...
import type { Session, SessionChunkRun, SessionEvent } from '@deepseek-ai/dsh-session'
import { TOOL_NOT_STARTED } from './repair.ts'

const PACKAGE_NAME = '@deepseek-ai/dsh-session'
const STRUCTURAL_REPLAY_TYPES = [
  'turn/start',
  'turn/end',
  'step/start',
  'step/end',
...
/** Validate and advance one payload-free historical chunk interval. */
function applyChunkRun(trace: SessionTrace, run: SessionChunkRun, fail: InvariantFailure): void {
  if (run.from <= trace.lastSeq) {
    fail(`seq must strictly increase: saw chunk run at ${run.from} after ${trace.lastSeq}`)
  }
  requireOpenStep(trace, 'assistant/chunk', run.turn, run.step, fail)
  trace.lastSeq = run.to - 1
}
...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  ...
  const seedSession = (session: Session): SessionTrace => {
    ...
    const events = log.valuesOf(STRUCTURAL_REPLAY_TYPES)[Symbol.iterator]()
    const chunks = log.chunkRuns()[Symbol.iterator]()
    let event = events.next()
    let chunk = chunks.next()
    while (!event.done || !chunk.done) {
      if (!chunk.done && (event.done || chunk.value.from < event.value.seq)) {
        applyChunkRun(trace, chunk.value, fail)
        chunk = chunks.next()
      } else if (!event.done) {
        applyTransition(trace, validateEvent(trace, event.value, fail))
        event = events.next()
      }
    }
    trace.lastSeq = log.length - 1
    return trace
  }
