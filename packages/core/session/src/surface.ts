...
/** Random-access logical event window consumed by the incremental surface fold. */
interface SurfaceEventLog {
...
function createFoldState(): SurfaceFoldState {
  return { nodes: [], replaceGeneration: 0, generationSeq: null }
}
...
export function foldSurface(events: readonly SessionEvent[]): SurfaceFoldResult {
  ...
  return { nodes: [...state.nodes], replacements, generationSeq: state.generationSeq }
}
...
export class SurfaceManager implements SessionSurface {
  ...
  /**
   * Commit the candidate most recently accepted by {@link validateNext} after
   * its exact event has entered the backing log. Session calls this at the
   * append commit point so packed storage never needs to decode that event
   * merely to advance the surface fold.
   * @param event - the exact accepted candidate now at the log tail.
   */
  commitNext(event: SessionEvent): void {
    const pending = this._pendingPlan
    if (pending?.event === event
      && pending.expectedSeq === event.seq
      && this.baseSeq + this.log.length === event.seq + 1) {
      applySurfacePlan(this._state, pending.plan)
      this._lastProcessedSeq = event.seq
  ...
  /**
   * Advance across a just-adopted contiguous range whose record encoding proves
   * that every logical member has one non-surface event type. This avoids
   * decoding packed stream deltas merely to establish that they do not alter
   * the model-visible surface.
   * @param type - shared logical event type represented by the range.
   * @param from - inclusive absolute sequence.
   * @param to - exclusive absolute sequence after adoption into the log.
   */
  commitNonSurfaceRange(type: string, from: number, to: number): void {
    if (isSurfaceEligibleType(type)) {
      throw new Error(`cannot skip surface-eligible event type "${type}"`)
    }
    if (this._pendingPlan !== undefined
      || from !== this._lastProcessedSeq + 1
      || to < from
      || this.baseSeq + this.log.length !== to) {
      throw new Error(`non-surface range [${from}, ${to}) does not match the next committed log range`)
    }
    this._lastProcessedSeq = to - 1
  }
  ...
  /** Seq of the replacement that created the current generation. */
  get generationSeq(): number | null {
    if (this._lastProcessedSeq < this.baseSeq + this.log.length - 1) this._processDelta()
    return this._state.generationSeq
  }
  ...
  private _processDelta(): void {
    ...
    for (let seq = this._lastProcessedSeq + 1; seq <= tailSeq; seq++) {
      ...
      /* v8 ignore next -- the loop bounds are derived from this same logical window. */
      if (event === undefined) throw new Error(`surface log is missing event at seq ${seq}`)
      const pending = this._pendingPlan
