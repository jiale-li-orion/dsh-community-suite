...
const GOAL_REPLAY_TYPES = ['goal/change', 'user/message'] as const
...
function foldChecked(events: Iterable<SessionEvent>, fail: InvariantFailure): FoldedGoal {
  try {
...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  for (const session of ctx.sessions.list()) {
    ...
    for (const event of log.valuesOf(['user/message'])) {
      validateEvent(log.valuesOf(GOAL_REPLAY_TYPES, 0, event.seq), event, fail)
    }
  }
  ...
  ctx.on('internal/dispatch', (_mode, eventName, args) => {
    ...
    validateEvent(session.readLog().valuesOf(GOAL_REPLAY_TYPES), event, fail)
  }, { global: true })
}, { inject: ['sessions'] })
