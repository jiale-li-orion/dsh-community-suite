...
const GOAL_FOLD_EVENT_TYPES = ['goal/change', 'user/message'] as const
...
export class GoalService extends TypertRemoteService {
  ...
  private cache(session: Session): GoalCache {
    ...
    for (const event of session.readLog().valuesOf(GOAL_FOLD_EVENT_TYPES)) applyGoalEvent(state, event)
    cache = {
      state,
      activation: 'disarmed',
  ...
  private sync(session: Session, cache: GoalCache): void {
    ...
    for (const event of log.valuesOf(GOAL_FOLD_EVENT_TYPES, cache.observedSeq)) {
      applyGoalEvent(cache.state, event)
      cache.observedSeq = event.seq + 1
