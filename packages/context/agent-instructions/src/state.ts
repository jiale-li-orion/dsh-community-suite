...
function visibleInstructionChanges(
  ...
  // Preserve the previous raw-log ordering while visiting only current surface
  // nodes; replacements can place a newer seq before an older retained node.
  for (const seq of agent.session.surface.nodes.toSorted((left, right) => left - right)) {
    const event = log.at(seq)
    if (event === undefined) {
      throw new Error(`agent-instructions: surface seq ${String(seq)} has no session event`)
    }
