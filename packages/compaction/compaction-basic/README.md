...
- **Summarization** — a direct `llm/stream` call uses the configured provider/model pair and cap, falling back to the latest logged request target and then the agent target, without running the loop-only `agent/request` extension point. Automatic and `/compact` calls replay the conversation prefix for cache reuse; reviewed middle ranges send a position-local canonical document so cost does not grow with unrelated retained history. Every call sets `GenerateOptions.purpose` to `compaction`. Only returned text enters the checkpoint; image output fails with `UNSUPPORTED_CONTENT`.
- **Reviewed preparation** — the backend resolves the exact summarization model's combined context capacity and maximum output capability, derives an output cap from the selected input, and records the plan before provider work. The complete selected document uses one `direct` call whenever its instruction, input, and output cap fit; only an oversized request forms bounded `map` chunks and `reduce` rounds. Every attempted and completed call is durable, and commit revalidates membership, digest, token price, balance, and shrink before opening the standard bracket.
...
The protected `summarize()` method customizes provider work. A template- or remote-summarizer subclass also overrides `resolvePreparationTarget()` when it does not use `ctx.llm` model metadata; pressure, retention, cited source events, shrink validation, and shadowed-token accounting stay on `ctx.tokenMeter`. The summarize hook returns the safe summary plus the complete provider output, call envelope, and usage when available (`{ summary, rawOutput?, llmStreamCall?, provider, model, maxTokens?, usage? }`); `llmStreamCall: true` means producing that result consumed exactly one call through this context's `ctx.llm.stream()` and requires complete `rawOutput`, while unmarked `rawOutput` does not identify the call path. The transaction preserves those fields on `compaction/summary`.
...
| `maxTokens` | no (default `8192`) | Output cap for automatic and `/compact` summarization; may include reasoning tokens. |
...
| `preparationOutputRatio` | no (default `0.25`) | Selected-input fraction used as the reviewed range's default output target. |
| `preparationMinOutputTokens` | no (default `16000`) | Minimum default output target before model and context caps. |
| `preparationMaxOutputTokens` | no (default `128000`) | Maximum default output target before model and context caps. |
| `preparationConcurrency` | no (default `2`) | Maximum concurrently active provider calls during reviewed range preparation. |
...
### Automatic and `/compact` summarizer request
...
### Reviewed range preparation request

#### What the model sees

Each map call receives one selected canonical context document and each reduce call receives partial summaries. The instruction says that the result replaces only this position, forbids inferring global current work, requires the seven sections `Outcomes`, `Decisions and Constraints`, `Artifacts and State Changes`, `Commands, Errors, and Results`, `Background Facts Still in Force`, `Open Threads Originating Here`, and `Literal Anchors`, and includes the optional preservation brief.

#### Token effect

The selected range first receives the default output target `min(preparationMaxOutputTokens, max(preparationMinOutputTokens, ceil(selectedTokens × preparationOutputRatio)))`, capped by the exact model's declared maximum output. If the complete position-local request plus that output fits the model's combined context window, one `direct` call runs. Otherwise map/reduce uses a convergence-safe cap and capacity-derived input chunks. Unrelated Context is never included, and preparation adds no conversation message until commit.

#### KV Cache effect

Range calls are independent position-local requests and do not promise reuse of the conversation prefix. A later commit preserves the conversation prefix before the selected range and invalidates it from that position.
