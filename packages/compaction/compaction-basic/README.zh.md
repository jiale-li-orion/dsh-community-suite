...
- **摘要**：直接 `llm/stream` 调用使用已配置的提供方／模型对与上限，回退到最新已记录请求目标，然后再回退到 agent（智能体）目标，而不运行仅用于 agent loop 的 `agent/request` 扩展点。自动与 `/compact` 调用会回放对话前缀以复用 cache；评审中的中间范围只发送位置局部规范文档，因此成本不会随无关保留历史增长。每次调用都会把 `GenerateOptions.purpose` 设为 `compaction`。只有返回文本进入检查点；图片输出以 `UNSUPPORTED_CONTENT` 失败。
- **可评审 preparation**：后端解析准确摘要模型的组合上下文容量与最大输出能力，根据所选输入推导输出上限，并在提供方工作前记录计划。只要完整所选文档、指令和输出上限能够放入窗口，就只运行一次 `direct` 调用；只有超窗请求才形成有界 `map` 分片与 `reduce` 轮次。每次尝试和完成的调用都会持久化，Commit 会在打开标准 bracket 前重新校验成员关系、digest、token 价格、平衡与缩减。
...
受保护的 `summarize()` 方法定制提供方工作。不使用 `ctx.llm` 模型元数据的模板摘要器或远程摘要器子类还会覆盖 `resolvePreparationTarget()`；压力、保留、被引用的源事件、缩减验证与已遮蔽 token 计量仍由 `ctx.tokenMeter` 负责。摘要钩子返回安全摘要，以及完整提供方输出、调用 envelope 和可用时的 usage（`{ summary, rawOutput?, llmStreamCall?, provider, model, maxTokens?, usage? }`）；`llmStreamCall: true` 表示生成该结果时恰好通过此上下文的 `ctx.llm.stream()` 发起了一次调用，且必须提供完整的 `rawOutput`；未带标记的 `rawOutput` 并不能判定调用路径。事务会在 `compaction/summary` 上保留这些字段。
...
| `maxTokens` | 否（默认 `8192`） | 自动与 `/compact` 摘要的输出上限；可包含推理 token。 |
| `compactionRetries` | 否（默认 `1`） | 压力仍高于阈值时，在首次尝试后进行的额外尝试次数。 |
| `maxOverflowRetries` | 否（默认 `1`） | 规范上下文窗口溢出后的最大重试次数；`0` 只禁用恢复。 |
| `preparationOutputRatio` | 否（默认 `0.25`） | 用作可评审范围默认输出目标的所选输入比例。 |
| `preparationMinOutputTokens` | 否（默认 `16000`） | 应用模型与上下文上限之前的最小默认输出目标。 |
| `preparationMaxOutputTokens` | 否（默认 `128000`） | 应用模型与上下文上限之前的最大默认输出目标。 |
| `preparationConcurrency` | 否（默认 `2`） | 可评审范围 preparation 期间同时活跃的提供方调用上限。 |
...
### 可评审范围 preparation 请求

#### 模型看到的内容

每次 map 调用接收一份所选规范上下文文档，每次 reduce 调用接收部分摘要。指令说明结果只替换该位置，禁止推断全局当前工作，要求 `Outcomes`、`Decisions and Constraints`、`Artifacts and State Changes`、`Commands, Errors, and Results`、`Background Facts Still in Force`、`Open Threads Originating Here` 与 `Literal Anchors` 七个章节，并携带可选保留说明。

#### Token 影响

所选范围先得到默认输出目标 `min(preparationMaxOutputTokens, max(preparationMinOutputTokens, ceil(selectedTokens × preparationOutputRatio)))`，再受准确模型声明的最大输出限制。如果完整的位置局部请求加上该输出能够放入模型的组合上下文窗口，系统只运行一次 `direct` 调用；否则 map/reduce 使用可收敛的上限与按容量推导的输入分片。系统绝不会包含无关 Context；Commit 前，preparation 不增加任何对话消息。

#### KV Cache 影响

范围调用是彼此独立的位置局部请求，不承诺复用对话前缀。之后 commit 会保留所选范围之前的对话前缀，并从该位置开始使其失效。
...
- **摘要失败会保留最新持久表层**：任何替换前，自动路径会记录警告，并携带完整超预算历史继续。如果剪枝已落地，后续摘要失败会从该持久剪枝表层继续。因达到自动 `maxTokens` 或可评审 preparation 的有效输出上限而发生的摘要截断（隐藏推理 token 可能会耗尽该额度）遵循同一规则。
