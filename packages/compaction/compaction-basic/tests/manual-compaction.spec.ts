...
describe('compactNow transaction and failure classification', () => {
  ...
  it('rejects a selected span whose pricing changed during summarization', async () => {
    ...
    const original = meter.measureRange.bind(meter)
    compact.duringSummary = () => {
      vi.spyOn(meter, 'measureRange').mockImplementationOnce((target, from, to, header) => {
        const measurement = original(target, from, to, header)
        return {
          ...measurement,
