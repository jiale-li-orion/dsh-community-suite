...
All five operations are **abstract** — the backend owns trigger policy, retention, event sequencing, summarization, and reviewed-range preparation. Reusable request measurement is a separate service, [`ctx.tokenMeter`](../../llm/token-meter/README.md), rather than part of this Service Definition.
...
| `prepareRegion(request, agent, signal, onProgress)` | Produce a capacity-derived direct or map/reduce plan, deterministic chunks, durable call attempts and completions, and a generated summary for an already recorded exact span without changing the surface. The caller persists every progress item before review begins. |
| `commitPrepared(preparation, summary, source, admission, agent, signal)` | Revalidate exact membership, digest, token price, tool balance, and summary shrink, then commit the standard bracket under idle maintenance or inside the caller's current turn. |
...
`compactIfNeeded`, `compactNow`, `prepareRegion`, and `commitPrepared` take a required `signal`; `compactRegion`'s is optional. A backend that summarizes via `ctx.llm.stream()` **must** forward it into the call's `GenerateOptions.signal`, so an abort or fiber dispose tears down the in-flight summarization. Automatic and explicit-region brackets recover their numeric owner from the currently open turn. Idle manual and reviewed commits stamp `turn: null`; a model tool may commit a preparation inside its existing numbered turn.

`ManualCompactionError.code` is the closed set `busy | cancelled | changed | summary | commit | persistence`. `changed` and `summary` mean the selected conversation surface was not replaced, but their failed attempt is still recorded in the session log. `commit` is deliberately neutral about partial mutation, and `persistence` means the in-memory bracket closed but its explicit flush failed.

## Provider resolution

Every `CompactionEngine` registers with `ctx.compactions`. A provider mounted inside an Agent preset is keyed to that Agent composition; a root provider becomes the process fallback. `CompactionDirectory.resolve(agent)` fails when neither exists, so Session Context never silently uses another Agent's summarizer.
...
The `compaction/*` events extend `SessionEventMap` (merge-extensible) via declaration merging — they are session events, not cordis `Events`, and every lifecycle or preparation record is log-only (no `surfaceOp`). Preparation records cover request, capacity plan, call attempt, completed call, ready summary, review edit, failure, and discard; `preparationId` correlates a later standard bracket. Per-event payloads and semantics are in the generated [persistence log event catalog](../../../docs/persistence-catalog.md).

## Implementing a backend

Subclass `CompactionEngine`, implement all five operations, and load the subclass as a plugin. Every successful backend creates its replacement user message source with `compactCheckpointSource(compactionId, sourceCommandId?)`; the required `compactionId` correlates the checkpoint with its `compaction/*` transaction, while `isCompactCheckpointSource()` recognizes the marker after persistence or cloning without depending on backend identity. A template- or model-backed implementation can live as a sibling package without changing callers or the shared token meter.
...
- **Two selection surfaces** — `/compact` remains an argument-free human command with backend-owned range selection, while `@deepseek-ai/dsh-tool-session-context` exposes reviewed model-selected ranges. They share the bracket but not selection policy.
