...
const ORPHAN_REPLAY_TYPES = ['compaction/start', 'compaction/end', 'session/end-seed'] as const
const COMPACTION_REPLAY_TYPES = [
  'turn/start',
  'turn/end',
  'session/end-seed',
  'user/message',
...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  ...
  const seed = (session: Session): SessionTrace => {
    ...
    const staleOrphanStartSeqs = inheritedOrphanStartSeqs(log.valuesOf(ORPHAN_REPLAY_TYPES))
    for (const event of log.valuesOf(COMPACTION_REPLAY_TYPES)) {
