...
五个操作都是**抽象方法**：触发策略、保留、事件顺序、摘要与可评审范围 preparation 均属于后端。可复用的请求测量是独立服务 [`ctx.tokenMeter`](../../llm/token-meter/README.md)，而非本 Service Definition 的一部分。
...
| `prepareRegion(request, agent, signal, onProgress)` | 针对已记录的准确范围生成按容量推导的 direct 或 map/reduce 计划、确定性分片、持久调用尝试与完成事实及摘要，但不修改表层。评审开始前，调用方会持久记录每项 progress。 |
| `commitPrepared(preparation, summary, source, admission, agent, signal)` | 重新校验准确成员关系、digest、token 价格、工具平衡与摘要缩减，然后在空闲 maintenance 中或调用方当前轮次内提交标准 bracket。 |
...
`compactIfNeeded`、`compactNow`、`prepareRegion` 与 `commitPrepared` 必须传入 `signal`；`compactRegion` 的该参数可选。通过 `ctx.llm.stream()` 摘要的后端**必须**将它转发到调用的 `GenerateOptions.signal`，因此 abort 或 fiber dispose（资源释放）会停止进行中的摘要。自动和显式范围标记对会从当前打开的轮次恢复其数字形式归属。空闲手动与评审提交标记 `turn: null`；模型工具可以在其既有编号轮次内提交 preparation。

`ManualCompactionError.code` 是封闭集合 `busy | cancelled | changed | summary | commit | persistence`。`changed` 和 `summary` 表示所选会话表层未被替换，但日志仍会记录失败尝试。`commit` 有意不判断是否发生了部分变更；`persistence` 表示内存中的 bracket 已闭合，但显式 flush 失败。

## 提供方解析

每个 `CompactionEngine` 都会向 `ctx.compactions` 注册。挂载在 Agent preset 中的提供方按该 Agent 组合登记；根提供方成为进程 fallback。`CompactionDirectory.resolve(agent)` 在两者都不存在时失败，因此 Session Context 不会静默使用其他 Agent 的摘要器。
...
`compaction/*` 事件通过 declaration merging 扩展 `SessionEventMap`（可合并扩展）：它们是会话事件，不是 cordis `Events`，全部生命周期或 preparation 记录都仅存在于日志（不含 `surfaceOp`）。Preparation 记录包括请求、容量计划、调用尝试、已完成调用、ready 摘要、评审编辑、失败与丢弃；`preparationId` 会关联之后的标准 bracket。各事件 payload 与语义见生成的[持久化日志事件目录](../../../docs/persistence-catalog.md)。

## 实现后端

继承 `CompactionEngine`、实现五项操作，再把子类作为插件加载。每个成功后端都使用 `compactCheckpointSource(compactionId, sourceCommandId?)` 创建替换 user 消息的源；必填的 `compactionId` 将检查点与对应 `compaction/*` 事务关联，而 `isCompactCheckpointSource()` 可在持久化或克隆后识别该标记，无需依赖后端身份。基于模板或模型的实现可以放在同级包中，不需更改调用方或共享 token meter。
...
- **两种选择界面**：`/compact` 仍是无参数人工命令，由后端选择范围；`@deepseek-ai/dsh-tool-session-context` 则暴露模型选择范围的可评审流程。两者共享 bracket，但不共享选择策略。
