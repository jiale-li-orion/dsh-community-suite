...
describe('LlmRuntime', () => {
  ...
  it('rejects an adapter-owned request default above the model output capability', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    const adapter = new class extends ScriptedAdapter {
      override resolveModel(provider: string, model: string): Promise<LlmResolvedModelInfo> {
  ...
  it.each([0, 1.5, Number.MAX_SAFE_INTEGER + 1])(
    'rejects invalid adapter-owned maximum output tokens %s',
    async (maxOutputTokens) => {
      ...
      await expect(ctx.llm.resolveModelInfo('route', 'model'))
        .rejects.toMatchObject({ code: 'INVALID_MODEL_MAX_OUTPUT_TOKENS' })
    },
  )
