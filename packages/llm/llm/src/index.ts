...
export class LlmRuntime extends Service {
  ...
  private async resolveModelInfoFor(
    ...
    if (maxOutputTokens !== undefined
      ...
      throw new LlmError(
        `adapter returned invalid maximum output tokens for provider "${provider}" model "${model}"`,
        'INVALID_MODEL_MAX_OUTPUT_TOKENS',
      )
    }
    ...
    if (defaultMaxTokens !== undefined && maxOutputTokens !== undefined
      && defaultMaxTokens > maxOutputTokens) {
      throw new LlmError(
        `adapter returned default maxTokens above the maximum output for provider "${provider}" model "${model}"`,
        'INVALID_MODEL_MAX_TOKENS',
      )
    }
