...
`ctx.tokenMeter` 直接公开三个操作：

- `measure(session, requestHeader?)` 在同一个已消费日志 revision 上返回请求压力与当前已计价表层。
- `measureRange(session, from, to, requestHeader?)` 返回相同总量，但只复制一个半开的位置表层范围。
- `estimateMessage(message)` 使用固定启发式规则为一条消息计价。

两种测量方法都会同步一次，并返回独立且深度不可变的值。`totalTokens` 是请求与响应压力，`surfaceTokens` 是完整的仅表层启发式总量。`requestHeader` 覆盖只影响压力字段；表层字段仍描述当前会话。`measure()` 会克隆全部位置节点；`measureRange()` 校验 `from..to`、报告 `rangeStart`，并只克隆该切片。
...
- **完整测量会克隆当前表层**：只需要选中位置的消费方必须使用 `measureRange()`；需要比较完整表层的压力策略仍会支付 O(surface) 成本。
