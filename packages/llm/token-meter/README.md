...
- `measureRange(session, from, to, requestHeader?)` returns the same totals while copying only one half-open positional surface range.
- `estimateMessage(message)` prices one message with the fixed heuristic.

Both measurement methods synchronize once and return detached, deeply immutable values. `totalTokens` is request-and-response pressure, while `surfaceTokens` is the complete surface-only heuristic total. A `requestHeader` override affects pressure fields only; the surface fields still describe the current session. `measure()` clones every positional node; `measureRange()` validates `from..to`, reports `rangeStart`, and clones only that slice.
...
- **Complete measurement clones the current surface** — consumers that need only selected positions must use `measureRange()`; pressure policy that compares the complete surface still pays O(surface).
