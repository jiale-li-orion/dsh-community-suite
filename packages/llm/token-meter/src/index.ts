...
import { SessionEventFold, canonicalHeader, headerEquals, isSurfaceEvent } from '@deepseek-ai/dsh-session'
// Type-only: resolves the optional projection registry Context declaration.
import type {} from '@deepseek-ai/dsh-session-projection'
import type {
  TokenMeasurement,
  TokenMeasurementBaseline,
  TokenMeasurementTotals,
  TokenMeterConfig,
...
import { planSurfaceTokens } from './surface-fold.ts'
...
export class TokenMeter extends Service {
  ...
  private readonly fold = new SessionEventFold<ReplayState>(
    () => ({
      header: undefined,
      surface: [],
      surfaceTokens: 0,
    ...
    (state, event, session) => { this._foldEvent(session, state, event) },
    ['request/header', 'step/start', 'step/end', 'user/message', 'assistant/message', 'tool/result'],
  )
  ...
  measure(session: Session, requestHeader?: EpochHeader): TokenMeasurement {
    ...
    return deepFreeze(structuredClone({
      ...this._totals(session, state, requestHeader),
      nodes: state.surface,
    }))
  }
  ...
  measureRange(
    ...
    if (!Number.isSafeInteger(from) || !Number.isSafeInteger(to)
      || from < 0 || to < from || to > state.surface.length) {
      throw new RangeError(`token meter range ${String(from)}-${String(to)} is outside 0-${state.surface.length}`)
    }
    return deepFreeze(structuredClone({
      ...this._totals(session, state, requestHeader),
      rangeStart: from,
      nodes: state.surface.slice(from, to),
    }))
  }

  /** Resolve request-pressure totals without copying positional surface nodes. */
  private _totals(
    session: Session,
    state: ReplayState,
    requestHeader?: EpochHeader,
