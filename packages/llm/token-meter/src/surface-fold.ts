...
/** Validated mutation plan committed only after the caller's remaining fold checks pass. */
export interface SurfaceTokenPlan {
  readonly tokens: number
  readonly deltaTokens: number
  apply(nodes: TokenSurfaceNode[]): void
}

/**
 * Validate and price one surface event without copying or mutating the current nodes.
 * @param nodes - mutable priced surface owned by the replay state.
 * @param event - surface event to plan.
 * @returns a no-throw commit after every fallible lookup and price has succeeded.
 */
export function planSurfaceTokens(
  ...
  if (op === 'append') {
    return {
      tokens,
      deltaTokens: tokens,
      apply: (target) => { target.push({ seq: event.seq, tokens }) },
    }
  }
  ...
  return {
    ...
    apply: (target) => { target.splice(startIdx, endIdx - startIdx + 1, { seq: event.seq, tokens }) },
  }
}
...
export function foldSurfaceTokens(
  ...
  plan.apply(next)
  return { tokens: plan.tokens, nodes: next, deltaTokens: plan.deltaTokens }
}
