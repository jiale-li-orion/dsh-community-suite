...
function appendUnchecked(session: Session, event: SessionEvent): void {
  const log = (session as unknown as { log: { append(event: SessionEvent): void } }).log
  log.append(event)
}
...
describe('TokenMeter pricing', () => {
  ...
  it('returns full pressure totals while copying only a requested positional node range', () => {
    const service = meter()
    const session = Session.create(SessionId('range-measurement'))
    ...
    const complete = service.measure(session)
    const range = service.measureRange(session, 1, 2)
    expect(range).toEqual({
      ...complete,
      rangeStart: 1,
      nodes: complete.nodes.slice(1, 2),
    })
    expect(Object.isFrozen(range)).toBe(true)
    expect(Object.isFrozen(range.nodes)).toBe(true)
    expect(() => service.measureRange(session, -1, 2)).toThrow(RangeError)
    expect(() => service.measureRange(session, 2, 4)).toThrow(RangeError)
  })
...
describe('malformed replay and listener lifecycle', () => {
  ...
  it('advances an attached live stream without materializing the complete log', async () => {
    const ctx = new Context()
    const sessionFiber = await ctx.plugin(SessionStore)
    const meterFiber = await ctx.plugin(TokenMeter)
    const session = ctx.sessions.create(SessionId('incremental-live'), {
      seed: Array.from({ length: 1_000 }, (_, seq): SessionEvent<'turn/start'> => ({
        type: 'turn/start',
        seq,
        time: 1,
        data: { turn: seq + 1 },
    ...
    const eventsGetter = vi.spyOn(session, 'events', 'get')

    appendSuccessfulCall(session, header('deepseek-v4-flash'), {
      turn: 1_001,
      usage: { inputTokens: 100, outputTokens: 20 },
      providerText: 'streamed provider output',
    })

    const measurement = ctx.tokenMeter.measure(session)
    expect(measurement.logRevision).toBe(session.seq)
    expect(measurement.baseline).toMatchObject({ kind: 'usage', tokens: 120 })
    expect(eventsGetter).not.toHaveBeenCalled()
    eventsGetter.mockRestore()
    await meterFiber.dispose()
    await sessionFiber.dispose()
  })

  it('retries a delivered event after one fold failure without advancing its watermark', async () => {
    const ctx = new Context()
    const sessionFiber = await ctx.plugin(SessionStore)
    const meterFiber = await ctx.plugin(TokenMeter)
    const service = ctx.tokenMeter
    const session = ctx.sessions.create(SessionId('retry-delivered-event'))
    service.measure(session)

    type FoldEvent = (session: Session, state: unknown, event: SessionEvent) => void
    const internals = service as unknown as { _foldEvent: FoldEvent }
    const original = internals._foldEvent.bind(service)
    let attempts = 0
    internals._foldEvent = (subject, state, event) => {
      if (event.type === 'request/header') {
        attempts += 1
        if (attempts === 1) throw new Error('injected token fold failure')
      }
      original(subject, state, event)
    }

    appendHeader(session, header('retry-model'))
    const measurement = service.measure(session)
    expect(attempts).toBe(2)
    expect(measurement.logRevision).toBe(1)
