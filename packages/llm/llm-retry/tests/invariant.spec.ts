...
import SessionStore, { sessionLogCutFromEvents, SessionId, type Session } from '@deepseek-ai/dsh-session'
...
describe('llm-retry invariants', () => {
  it('has no provider without the requested open step or a route marker', () => {
    expect(providerForOpenStep(sessionLogCutFromEvents([]), 1, 1)).toBeUndefined()
    expect(providerForOpenStep(sessionLogCutFromEvents([{
      type: 'step/start',
      seq: 0,
      time: 1,
      data: { turn: 1, step: 1 },
