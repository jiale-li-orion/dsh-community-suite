...
export class SessionCorpus {
  ...
  async load(sessionId: SessionId, signal?: AbortSignal): Promise<LogicalSession> {
    ...
    const snapshot = {
      ...
      events: Array.from(loaded.log.values(), event => structuredClone(event)),
    }
    signal?.throwIfAborted()
    return snapshot
  }
...
function sourceLive(session: Session): LogicalSessionSource {
  return { header: session.header, events: session.readLog().materialize() }
}
...
function snapshotLive(session: Session): LogicalSession {
  return {
    header: structuredClone(session.header),
    events: Array.from(session.readLog().values(), event => structuredClone(event)),
  }
}
