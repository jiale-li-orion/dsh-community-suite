...
import SessionStore, { SESSION_FORMAT_VERSION, SessionId, sessionLogCutFromEvents } from '@deepseek-ai/dsh-session'
...
class TestPersistence extends SessionPersistence {
  ...
  inspect(
    ...
    if (TestPersistence.inspectOverride !== undefined) {
      return TestPersistence.inspectOverride(id, signal).then(inspectionFromEntry)
    }
    if (TestPersistence.inspectFailure !== undefined) return rejectUnknown(TestPersistence.inspectFailure)
    const entry = TestPersistence.entries.get(id)
    if (entry === undefined) return Promise.reject(new Error('missing test session'))
    const result = inspectionFromEntry(entry)
...
function inspectionFromEntry(entry: { meta: SessionHeader; events: SessionEvent[] }): SessionInspection {
  const snapshot = structuredClone(entry)
  const log = sessionLogCutFromEvents(snapshot.events)
  return { meta: snapshot.meta, log, events: log.materialize() }
}
...
describe('session-query exact reads', () => {
  it('returns a detached replay-valid full log and rejects a corrupt persisted seed', async () => {
    ...
    await expect(ctx.sessionQuery.readSession(corrupt.id)).rejects.toThrow('session log event at index 0 has seq 1')
  })
