      ...
      if (persistence !== undefined) {
        try {
          ...
          for (const entry of persisted.values()) {
            ...
            entry.loaded = observeSession(loaded.meta, loaded.log.materialize())
          }
          assertNotAborted(signal)
...
function observeLive(session: Session): ObservedSession {
  return observeSession(session.header, session.readLog().values())
}

function observeSession(header: SessionHeader, events: Iterable<SessionEvent>): ObservedSession {
  const detachedHeader = structuredClone(header)
  const detachedEvents = Array.from(events, event => structuredClone(event))
  return {
