...
本包还导出两条「发实体」路由共用的响应编码器：`sendEncoded(req, res, status, body, headers)` 在请求声明接受压缩、内容类型为文本、且实体超过 `COMPRESSION_MIN_BYTES`（1024 字节）时做 gzip 压缩，否则原样发送；`selectEncoding(acceptEncoding, contentType, size)` 是它背后的纯决策函数。所有被覆盖的响应都带 `vary: accept-encoding`（包括原样发送的那些），这样共享缓存不会把压缩体交给没要求压缩的客户端；只提供 gzip 这一种编码，因为 brotli 在每次请求上要多花数倍 CPU，却只再多省约十分之一的字节。
