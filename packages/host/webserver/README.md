...
The package also exports the response encoder the two body-serving routes share: `sendEncoded(req, res, status, body, headers)` gzip-compresses a body when the request accepts it, the content type is text, and the body clears `COMPRESSION_MIN_BYTES` (1024), and otherwise sends it unchanged. `selectEncoding(acceptEncoding, contentType, size)` is the pure decision behind it. Every covered response carries `vary: accept-encoding`, including the unchanged ones, so a shared cache never hands a compressed body to a client that did not ask for one; gzip is the only coding offered because brotli would save roughly another tenth of the bytes at several times the CPU on every request.
