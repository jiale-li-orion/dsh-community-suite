...
`session.history` 会从已附加 Session 捕获一个稳定日志切面，或通过持久化检查冷日志，而不会恢复或发布 agent，再通过反向扫描追加来源的 `user/message` 与 `assistant/message` 事件定位请求页。`maxMessages` 统计以追加方式进入 surface 的消息，因此仅供模型使用的替换副本不占用配额。网关只实体化选中的连续持久区间，从而让压缩（compaction）的仅日志 `compaction/summary` 记录与引用它的替换留在同一页。

协议页是该区间的 seq 跨度保持投影。已完成的 Assistant 流只保留首个携带 token 的 delta、usage 分片与最终 `assistant/message`；未完成的 delta 连续段会合并相邻片段，同时保留首 token 与首个非空白内容的计时边界。`HistoryEntry.event.seq` 是该项表示的最后一个持久 seq；当一项覆盖更宽区间时，可选的 `firstSeq` 记录第一个 seq。客户端使用这些区间进行向后分页，并把投影历史接到未经修改的实时尾部，而不会把省略的传输分片误判为缺口。
