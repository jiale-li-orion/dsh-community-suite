...
/**
 * One history page entry: a history projection event plus optional transport
 * metadata. Ordinary entries carry one durable event unchanged. A compacted
 * streamed-delta entry carries one equivalent `assistant/chunk` projection at
 * the last represented seq; {@link firstSeq} preserves the complete durable
 * sequence span for backward pagination and page-continuity checks. A completed
 * `assistant/message` projection may omit chunk provenance already represented
 * by the same page.
 */
export interface HistoryEntry {
  /** First durable seq represented by this entry; omitted when equal to `event.seq`. */
  firstSeq?: number
  /** Last represented seq and the event value consumed by client projections. */
  event: SessionEvent
  /** Host-computed tool render intent; never persisted. */
  view?: ToolEventView
}
