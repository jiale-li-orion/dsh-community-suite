...
/** One session.history item with its represented seq span and optional tool view. */
...
}).refine(entry => entry.firstSeq === undefined || entry.firstSeq <= entry.event.seq, {
  message: 'history entry firstSeq must not exceed event.seq',
...
export const imageLimitsProjectionSchema = z.object({
  ...
  maxImageEdgePixels: z.number().int().positive(),
