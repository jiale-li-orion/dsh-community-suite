...
import type {} from '@deepseek-ai/dsh-workbench/types'
...
/** CPU interval before a worst-case attachment authorization scan yields. */
const ATTACHMENT_SCAN_YIELD_INTERVAL_MS = 10

/** Known image-bearing event types whose sparse indexes cover normal Web reads. */
const INDEXED_ATTACHMENT_EVENT_TYPES = [
  'user/message',
  'assistant/message',
  'tool/result',
  'agent/inbox/spliced',
...
const indexedAttachmentEventTypes = new Set<SessionEventType>([
  ...INDEXED_ATTACHMENT_EVENT_TYPES,
  'assistant/chunk',
])

/** Per-cut authorization results; a later append produces a different cut. */
const attachmentReferences = new WeakMap<SessionLogCut, Map<string, Promise<ImageAttachmentRef | undefined>>>()
...
/** Search one event iterator while periodically returning control to the host. */
async function findReferencedImage(
  events: Iterable<SessionEvent>,
  attachmentId: string,
  skipTypes?: ReadonlySet<SessionEventType>,
): Promise<ImageAttachmentRef | undefined> {
  let checked = 0
  let yieldDeadline = performance.now() + ATTACHMENT_SCAN_YIELD_INTERVAL_MS
  for (const event of events) {
    checked += 1
    if (skipTypes?.has(event.type) !== true) {
      const found = imageInEvent(event, ref => String(ref.attachmentId) === attachmentId)
      if (found !== undefined) return found
    }
    if ((checked & 0xFF) === 0 && performance.now() >= yieldDeadline) {
      await scheduler.yield()
      yieldDeadline = performance.now() + ATTACHMENT_SCAN_YIELD_INTERVAL_MS
    }
  }
  return undefined
}
...
/**
 * Resolve one session-authorized image without expanding the complete log.
 * Sparse known carriers cover ordinary messages, queued input, nested code
 * dispatches, and compaction replacements. The final generic pass preserves
 * support for plugin-defined event carriers and yields during a negative scan.
 */
async function scanReferencedImage(log: SessionLogCut, attachmentId: string): Promise<ImageAttachmentRef | undefined> {
  const indexed = await findReferencedImage(
    log.reverseValuesOf(INDEXED_ATTACHMENT_EVENT_TYPES),
    attachmentId,
  )
  if (indexed !== undefined) return indexed

  const streamed = await findReferencedImage(
    log.reverseValuesOf(['assistant/chunk']),
    attachmentId,
  )
  if (streamed !== undefined) return streamed

  return findReferencedImage(log.reverseValues(), attachmentId, indexedAttachmentEventTypes)
}

/** Share one stable-cut authorization scan among duplicate image reads. */
function referencedImage(log: SessionLogCut, attachmentId: string): Promise<ImageAttachmentRef | undefined> {
  let references = attachmentReferences.get(log)
  if (references === undefined) {
    references = new Map()
    attachmentReferences.set(log, references)
  }
  const existing = references.get(attachmentId)
  if (existing !== undefined) return existing
  const pending = scanReferencedImage(log, attachmentId)
  references.set(attachmentId, pending)
  return pending
}
 ...
 * alone — so they consume no quota; the selected source stays one contiguous raw range,
 ...
 */
type HistoryLog = Pick<SessionLogCut, 'length' | 'reverseValuesOf' | 'materialize'>

function paginate(
  log: HistoryLog,
  beforeSeq: number | undefined,
  maxMessages: number,
): { events: readonly SessionEvent[]; hasMore: boolean } {
  const end = beforeSeq === undefined ? log.length : Math.min(beforeSeq, log.length)
  let count = 0
  let cut = 0
  for (const event of log.reverseValuesOf(['user/message', 'assistant/message'], 0, end)) {
  ...
  return { events: log.materialize(cut, end), hasMore: cut > 0 }
}

interface ProjectedHistoryEvent {
  readonly firstSeq?: number
  readonly event: SessionEvent
}

/** Timestamp of one member in a packed delta row. */
function chunkMemberTime(row: ChunkRow, index: number): number {
  let time = row.time0
  for (let current = 0; current < index; current += 1) {
    // Row validation guarantees one gap between every adjacent member.
    // oxlint-disable-next-line typescript/no-non-null-assertion
    time += row.data.dt[current]!
  }
  return time
}

/** Merge one contiguous member range into an equivalent history projection event. */
function mergedChunkEvent(
  row: ChunkRow,
  start: number,
  end: number,
  timeIndex: number,
): SessionEvent<'assistant/chunk'> {
  const seq = row.seq0 + end
  const time = chunkMemberTime(row, timeIndex)
  switch (row.type) {
    case 'text-chunks':
      return {
        ...
        data: {
          turn: row.data.turn,
          step: row.data.step,
          chunk: { type: 'text-delta', index: row.data.index, text: row.data.texts.slice(start, end + 1).join('') },
        },
      }
    case 'reasoning-chunks':
      return {
        ...
        data: {
          turn: row.data.turn,
          step: row.data.step,
          chunk: { type: 'reasoning-delta', index: row.data.index, text: row.data.texts.slice(start, end + 1).join('') },
        },
      }
      ...
      return {
        ...
        data: {
          ...
          chunk: {
            ...
            argumentsDelta: row.data.args.slice(start, end + 1).join(''),
          },
        },
      }
  }
...
/**
 * Compact one unfinished delta run while retaining exact first-token and
 * first-visible boundaries used by client timing and partial-state folds.
 */
function projectChunkRow(row: ChunkRow): SessionEvent<'assistant/chunk'>[] {
  const members = row.type === 'tool-call-chunks' ? row.data.args : row.data.texts
  let tokenIndex: number
  let visibleIndex = -1
  if (row.type === 'tool-call-chunks') {
    tokenIndex = row.data.name === undefined
      ? row.data.args.findIndex(fragment => fragment !== '')
      : 0
  } else {
    tokenIndex = row.data.texts.findIndex(text => text !== '')
    visibleIndex = row.data.texts.findIndex(text => text.trim() !== '')
  }
  const boundaries = [...new Set([tokenIndex, visibleIndex].filter(index => index >= 0))]
    .sort((left, right) => left - right)
  const projected: SessionEvent<'assistant/chunk'>[] = []
  let start = 0
  for (const boundary of boundaries) {
    projected.push(mergedChunkEvent(row, start, boundary, boundary))
    start = boundary + 1
  }
  if (start < members.length) {
    projected.push(mergedChunkEvent(row, start, members.length - 1, start))
  }
  return projected
}

/** Remove provenance from a completed Assistant event after the cited chunks are projected. */
function withoutSourceSeqs(event: SessionEvent): SessionEvent {
  const raw = event as SessionEvent & { sourceEventSeqs?: readonly number[] }
  const { sourceEventSeqs: _sourceEventSeqs, ...projected } = raw
  return projected
}

/**
 * Project one contiguous raw page into a smaller sequence-span-preserving page.
 * Completed streams retain only their first token-bearing delta and usage chunks;
 * uncited delta runs merge without changing their accumulated content.
 */
function projectHistoryEvents(events: readonly SessionEvent[]): ProjectedHistoryEvent[] {
  const first = events[0]
  if (first === undefined) return []
  const bySeq = new Map(events.map(event => [event.seq, event]))
  const omitted = new Set<number>()
  const compactedMessages = new Set<number>()
  for (const event of events) {
    if (event.type !== 'assistant/message') continue
    const sourceEventSeqs = event.sourceEventSeqs
    if (sourceEventSeqs === undefined || sourceEventSeqs.length === 0) continue
    const chunks: Array<SessionEvent<'assistant/chunk'>> = []
    let complete = true
    for (const seq of sourceEventSeqs) {
      const source = bySeq.get(seq)
      if (source?.type !== 'assistant/chunk'
        || source.data.turn !== event.data.turn
        || source.data.step !== event.data.step) {
        complete = false
        break
      }
      chunks.push(source)
    ...
    const firstToken = chunks.find(source => isTokenDelta(source.data.chunk))
    for (const source of chunks) {
      if (source.seq !== firstToken?.seq && source.data.chunk.type !== 'usage') {
        omitted.add(source.seq)
      }
    }
    compactedMessages.add(event.seq)
  }

  const retained = events.filter(event => !omitted.has(event.seq))
  const records = packChunkRuns(retained)
  const projected: ProjectedHistoryEvent[] = []
  let firstSeq = first.seq
  for (const record of records) {
    const output = record.type === 'text-chunks'
      || record.type === 'reasoning-chunks'
      || record.type === 'tool-call-chunks'
      ? projectChunkRow(record)
      : [compactedMessages.has(record.seq) ? withoutSourceSeqs(record) : record]
    for (const event of output) {
      projected.push({
        ...firstSeq === event.seq ? {} : { firstSeq },
        event,
      })
      firstSeq = event.seq + 1
    }
  }
  return projected
}
...
function sessionBlank(session: Session): boolean {
  return liveSessionListMetadata.read(session).value.blank
}
...
interface LiveSessionListMetadata {
  value: SessionListMetadata
}

const liveSessionListMetadata = new SessionEventFold<LiveSessionListMetadata>(
  () => ({ value: { blank: true, lastPromptAt: null } }),
  (state, event) => { state.value = applySessionListMetadata(state.value, event) },
...
function sessionListFields(header: SessionHeader, session?: PresetBearingSession): {
  ...
  const agentPreset = session === undefined ? header.agentPreset : resolveSessionPreset(session)
  return {
...
function summarize(session: Session, running: boolean): SessionSummary {
  const metadata = liveSessionListMetadata.read(session).value
  return {
    sessionId: session.id,
    updatedAt: sessionListUpdatedAt(session.header, metadata),
    running,
    blank: metadata.blank,
    ...sessionListFields(session.header, session),
  }
}
 ...
 */
function backscanArgs(eventsNewestFirst: Iterable<SessionEvent>, callId: string): { name: string; args: unknown } | undefined {
...
/** Iterate one detached page newest-first without copying it. */
function reversePage(events: readonly SessionEvent[]): Iterable<SessionEvent> {
  return (function* (): Generator<SessionEvent> {
    for (let index = events.length - 1; index >= 0; index -= 1) {
      // Loop bounds prove the page entry exists.
      // oxlint-disable-next-line typescript/no-non-null-assertion
      yield events[index]!
    }
  })()
...
function historyPage(
  ...
  const page = paginate(log, beforeSeq, maxMessages ?? DEFAULT_MAX_MESSAGES)
  const projected = projectHistoryEvents(page.events)
  const projectedEvents = projected.map(entry => entry.event)
  return {
    events: projected.map(({ event, firstSeq }) => {
      const view = viewFor(ctx, event, callId => backscanArgs(reversePage(projectedEvents), callId), scope)
      return {
...
type HistorySource =
  ...
  | { readonly kind: 'detached'; readonly header: SessionHeader; readonly log: SessionLogCut }
...
function detachedProjectionsFor(
  ...
  if (cache !== undefined) return cache.snapshotLog(meta, log)
  const registry = ctx.get('sessionProjections')
  if (registry === undefined) return undefined
  return registry.restoreLog({}, log).snapshot
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  ctx.on('session/event', (session, event) => {
    liveSessionListMetadata.advance(session, event)
  })
  ...
  const inspectServable = (sessionId: SessionId): Promise<SessionInspection> =>
  ...
  const agentFor = createApiRemoteAgentResolver(ctx, {
    agentOptions,
    setup: async ({ meta, log }) =>
      (await composeAgent(resolveSessionPreset({ header: meta, log }))).setup,
  })
  ...
  ctx.inject(['sessionProjections'], (projectionCtx) => {
    projectionCtx.sessionProjections.register<'sessionListMetadata', SessionListMetadata>({
      ...
      eventTypes: ['turn/start', 'user/message'],
  ...
  if (ctx.get('approval') !== undefined) {
    ...
    ctx.on('approval/request', (req, next) => {
      ...
      for (const event of req.agent.session.readLog().reverseValuesOf(['approval/asked', 'approval/decided'])) {
   ...
   * @returns the attached session, or the inspected detached header and log cut.
  ...
  async function historySourceFor(sessionId: SessionId): Promise<HistorySource> {
    ...
    const inspected = await inspectServable(sessionId)
    return { kind: 'detached', header: inspected.meta, log: inspected.log }
  }

  /**
   * The header and log {@link presenterScopeFor} reads to decide which
   * composition a transcript ran under.
   * @param source - the live or detached session this read is served from.
   * @returns that session's creation header and stable log cut.
   */
  function sourceSession(source: HistorySource): PresetBearingSession {
    if (source.kind === 'detached') return { header: source.header, log: source.log }
  ...
  ): { log: HistoryLog; projections?: SessionProjectionsBlock } {
    if (source.kind === 'detached') {
      const projections = includeProjections ? detachedProjectionsFor(ctx, source.header, source.log) : undefined
      return { log: source.log, ...projections === undefined ? {} : { projections } }
    }
    const log = source.session.readLog()
    const projections = includeProjections ? projectionsFor(ctx, source.session) : undefined
    return { log, ...projections === undefined ? {} : { projections } }
  }
    ...
    if (creation === undefined) {
      creation = (async () => {
        ...
        if (persistence !== undefined && stored !== undefined) {
          ...
          const storedPreset = resolveSessionPreset({ header: inspected.meta, log: inspected.log })
  ...
  return {
    sessions: {
      ...
      async history(request) {
        ...
        try {
          ...
          const page = historyPage(ctx, cut.log, beforeSeq, maxMessages, scope)
          return ok(request, {
            events: page.events,
            hasMore: page.hasMore,
      ...
      async fork(request) {
        ...
        const lastSeq = source.log.length - 1
        let boundary: SessionEvent<'turn/end'> | undefined
        if (atSeq !== undefined && atSeq <= lastSeq) {
          for (const event of source.log.valuesOf(['turn/end'], atSeq)) {
            boundary = event
            break
          }
        } else {
          for (const event of source.log.reverseValuesOf(['turn/end'])) {
            boundary = event
            break
          }
        }
        ...
        let cut = source.log.length
        for (const event of source.log.valuesOf(['turn/start'], boundary.seq + 1)) {
          cut = event.seq
          break
        }
        let workspace: Workspace | undefined
        ...
        const forkComposition = await composeAgent(resolveSessionPreset({ header: source.header, log: source.log }))
        const seed = source.log.materialize(0, cut)
        try {
      ...
      async attachment(request) {
        ...
        const ref = await referencedImage(state.log, String(attachmentId))
        if (ref === undefined) {
          return err(request, {
            code: 'attachment-error',
    ...
    subagents: {
      ...
      async history(request, signal) {
          ...
          try {
            ...
            projections = beforeSeq === undefined
              ? subagentHistoryProjections(
                ctx,
                childSessionId,
                () => detachedProjectionsFor(ctx, inspected.meta, inspected.log),
              )
              : undefined
          } catch (error: unknown) {
            if (signal?.aborted) {
        ...
        const page = historyPage(ctx, log, beforeSeq, maxMessages)
          ...
          ctx.on('session/event', (session: Session, event: SessionEvent) => {
            ...
            const view = viewFor(
              ...
              callId => openCalls.get(session.id)?.get(callId)
                ?? backscanArgs(session.readLog().reverseValuesOf(['tool/call']), callId),
