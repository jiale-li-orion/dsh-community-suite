...
import { z } from 'zod'
import {
  contentBlockSchema, historyEntrySchema, sessionCancelRequestSchema, sessionCancelValueSchema, sessionCreateRequestSchema,
...
describe('rpcErrorSchema', () => {
  it('accepts every code branch with its required details', () => {
    ...
    expect(rpcErrorSchema.parse({ code: 'invalid-client-device', message: 'm', details: { value: 'tablet' } }).code).toBe('invalid-client-device')
