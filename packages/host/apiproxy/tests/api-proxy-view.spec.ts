...
import type { Session, SessionEvent, SessionId, SessionLogCut } from '@deepseek-ai/dsh-session'
...
describe('mux live view computation', () => {
  ...
  it('reads and materializes only the selected live history range', async () => {
    const { ctx } = await harness()
    const api = createApiProxy(ctx, { defaultModelSelection: () => ({ provider: 'p', model: 'm' }), cwd: '/tmp' })
    const session = ctx.sessions.create()
    ctx.agents.register({ id: session.id, session, status: 'idle', ctx } as Agent)
    ...
    session.append('turn/start', { turn: 201 })
    const prompt = appendUserText(session, 'bounded page')

    const base = session.readLog()
    const materialized: Array<readonly [number | undefined, number | undefined]> = []
    const tracked: SessionLogCut = {
      length: base.length,
      at: seq => base.at(seq),
      values: (from, to) => base.values(from, to),
      reverseValues: (from, to) => base.reverseValues(from, to),
      valuesOf: (types, from, to) => base.valuesOf(types, from, to),
      reverseValuesOf: (types, from, to) => base.reverseValuesOf(types, from, to),
      chunkRuns: (from, to) => base.chunkRuns(from, to),
      materialize(from, to) {
        materialized.push([from, to])
        return base.materialize(from, to)
      },
    }
    vi.spyOn(session, 'readLog').mockReturnValue(tracked)
    const eventsSnapshot = vi.spyOn(session, 'events', 'get')

    const response = await api.sessions.history({
      rpcId: RpcId('t-hist-bounded'),
      payload: { sessionId: session.id, maxMessages: 1 },
    })
    if (!response.result.ok) throw new Error('unreachable')
    expect(response.result.value.events.map(entry => entry.event.seq)).toEqual([prompt.seq])
    expect(materialized).toEqual([[prompt.seq, base.length]])
    expect(eventsSnapshot).not.toHaveBeenCalled()
  })

  it('compacts completed and partial streamed deltas while preserving seq spans', async () => {
    const { ctx } = await harness()
    const api = createApiProxy(ctx, { defaultModelSelection: () => ({ provider: 'p', model: 'm' }), cwd: '/tmp' })
    const completed = ctx.sessions.create()
    ctx.agents.register({ id: completed.id, session: completed, status: 'idle', ctx } as Agent)
    completed.append('turn/start', { turn: 1 })
    completed.append('step/start', { turn: 1, step: 1 })
    const sources = [completed.append('assistant/chunk', {
      turn: 1, step: 1, chunk: { type: 'block-start', index: 0, blockType: 'text' },
    }).seq]
    for (let index = 0; index < 1_000; index += 1) {
      sources.push(completed.append('assistant/chunk', {
    ...
    sources.push(completed.append('assistant/chunk', {
      turn: 1,
      step: 1,
      chunk: { type: 'usage', usage: { inputTokens: 10, outputTokens: 1_000 } },
    }).seq)
    const final = completed.append('assistant/message', {
      turn: 1,
      step: 1,
      message: createMessage({
        role: 'assistant',
      ...
      usage: { inputTokens: 10, outputTokens: 1_000 },
    }, { surfaceOp: 'append', sourceEventSeqs: sources })

    const response = await api.sessions.history({
      rpcId: RpcId('t-hist-stream'),
      payload: { sessionId: completed.id, maxMessages: 1 },
    })
    if (!response.result.ok) throw new Error('unreachable')
    const rows = response.result.value.events
    expect(rows).toHaveLength(3)
    expect(rows[0]).toMatchObject({ firstSeq: sources[0], event: { type: 'assistant/chunk', seq: sources[1] } })
    expect(rows[1]?.event).toMatchObject({ type: 'assistant/chunk', data: { chunk: { type: 'usage' } } })
    expect(rows[2]?.event).toMatchObject({ type: 'assistant/message', seq: final.seq })
    expect(rows[2]?.event).not.toHaveProperty('sourceEventSeqs')
    expect(JSON.stringify(rows).length).toBeLessThan(3_000)

    const partial = ctx.sessions.create()
    ctx.agents.register({ id: partial.id, session: partial, status: 'idle', ctx } as Agent)
    partial.append('turn/start', { turn: 1 })
    partial.append('step/start', { turn: 1, step: 1 })
    const emptyDelta = partial.append('assistant/chunk', {
      turn: 1, step: 1, chunk: { type: 'text-delta', index: 0, text: '' },
    })
    const firstToken = partial.append('assistant/chunk', {
      turn: 1, step: 1, chunk: { type: 'text-delta', index: 0, text: ' ' },
    })
    const firstVisible = partial.append('assistant/chunk', {
      turn: 1, step: 1, chunk: { type: 'text-delta', index: 0, text: 'a' },
    })
    let lastDelta = firstVisible
    for (let index = 3; index < 100; index += 1) {
      lastDelta = partial.append('assistant/chunk', {
        turn: 1, step: 1, chunk: { type: 'text-delta', index: 0, text: 'a' },
      })
    }
    const partialResponse = await api.sessions.history({
      rpcId: RpcId('t-hist-partial'),
      payload: { sessionId: partial.id, maxMessages: 1 },
    })
    if (!partialResponse.result.ok) throw new Error('unreachable')
    const deltas = partialResponse.result.value.events.filter(entry =>
      entry.event.type === 'assistant/chunk'
      && entry.event.data.chunk.type === 'text-delta')
    expect(deltas).toHaveLength(3)
    expect(deltas[0]).toMatchObject({
      firstSeq: emptyDelta.seq,
      event: { seq: firstToken.seq, time: firstToken.time, data: { chunk: { text: ' ' } } },
    })
    expect(deltas[1]).toMatchObject({
      event: { seq: firstVisible.seq, time: firstVisible.time, data: { chunk: { text: 'a' } } },
    })
    expect(deltas[2]).toMatchObject({
      firstSeq: firstVisible.seq + 1,
      event: { seq: lastDelta.seq, data: { chunk: { text: 'a'.repeat(97) } } },
    })
    expect(deltas.map(entry => (entry.event as SessionEvent<'assistant/chunk'>).data.chunk)
      .map(chunk => chunk.type === 'text-delta' ? chunk.text : '').join(''))
      .toBe(` ${'a'.repeat(98)}`)
  })
  ...
  it('paginates a message with many provenance sources without variadic argument expansion', async () => {
    ...
    try {
      ...
      if (!response.result.ok) throw new Error('unreachable')
      expect(response.result.value.events).toHaveLength(2)
      expect(response.result.value.events[0]?.event).toMatchObject({
        type: 'assistant/chunk',
        seq: sources[0],
      })
      expect(response.result.value.events[1]?.event.seq).toBe(message.seq)
      expect(response.result.value.hasMore).toBe(true)
    } finally {
      min.mockRestore()
    }
