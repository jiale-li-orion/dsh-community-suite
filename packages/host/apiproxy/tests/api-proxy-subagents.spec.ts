...
import { Session, sessionLogCutFromEvents } from '@deepseek-ai/dsh-session'
...
function bench(options: {
  ...
  const childEvents: SessionEvent[] = [{
    type: 'user/message', seq: 0, time: 1, data: childMessage, surfaceOp: 'append',
  }]
  const liveSession = Session.create(CHILD, undefined, childHeader)
  liveSession.append('user/message', childMessage, { surfaceOp: 'append' })
  const inspect = vi.fn(() => Promise.resolve(inspection(childHeader, childEvents)))
