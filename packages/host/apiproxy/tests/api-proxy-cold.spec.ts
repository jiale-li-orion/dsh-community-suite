...
import SessionStore, { sessionLogCutFromEvents } from '@deepseek-ai/dsh-session'
...
function inspection(meta: SessionHeader, events: readonly SessionEvent[]) {
  const log = sessionLogCutFromEvents(events)
  return { meta, log, events: log.materialize() }
}
...
describe('cold history recovery view', () => {
  it('paginates a stable log cut without accessing the complete inspection view', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(UserQuestionService)
    const sessionId = sid('session-cut-only-history')
    const meta = header(sessionId, 1000)
    const message = createUserMessage({
      content: [{ type: 'text', text: 'bounded history' }],
      source: { kind: 'user' },
    })
    const log = sessionLogCutFromEvents([{
      type: 'user/message', seq: 0, time: 1, data: message, surfaceOp: 'append',
    }])
    ctx.provide('sessionPersistence', {
      list: () => Promise.resolve([meta]),
      inspect: () => Promise.resolve({
        ...
        get events(): readonly SessionEvent[] {
          throw new Error('complete event materialization is forbidden in cold history')
        },
      }),
      locate: () => undefined,
    } as never)
    ...
    const history = await api.sessions.history(request({ sessionId, maxMessages: 10 }))

    expect(history.result).toMatchObject({
      ok: true,
      value: { events: [{ event: { type: 'user/message', seq: 0 } }] },
    })
    await ctx.fiber.dispose()
  })

  it('restores the tail projection baseline through the persisted cache over the same log cut', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(SessionProjectionRegistry)
    await ctx.plugin(UserQuestionService)
    const sessionId = sid('session-cached-history')
    const meta = header(sessionId, 1000)
    const log = sessionLogCutFromEvents([{
      type: 'user/message', seq: 0, time: 1,
      data: createUserMessage({
        content: [{ type: 'text', text: 'cached history' }],
        source: { kind: 'user' },
      }),
      surfaceOp: 'append',
    }])
    ctx.provide('sessionPersistence', {
      list: () => Promise.resolve([meta]),
      inspect: () => Promise.resolve({ meta, log, events: log.materialize() }),
      locate: () => undefined,
    } as never)
    const snapshotLog = vi.fn(() => ({ asOfSeq: 0, values: {} }))
    ctx.provide('sessionProjectionCache', { snapshotLog } as never)
    const registryRestore = vi.spyOn(ctx.sessionProjections, 'restoreLog')
    const api = createApiProxy(ctx, {
      defaultModelSelection: () => ({ provider: 'p', model: 'm' }),
      cwd: '/tmp',
    })
    ...
    expect(history.result).toMatchObject({
      ok: true,
      value: { projections: { asOfSeq: 0, values: {} } },
    })
    expect(snapshotLog).toHaveBeenCalledExactlyOnceWith(meta, log)
    expect(registryRestore).not.toHaveBeenCalled()
    await ctx.fiber.dispose()
  })
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const meta = header(sessionId, 1000)
    const inspect = vi.fn(() => Promise.resolve(inspection(meta, [])))
    ctx.provide('sessionPersistence', {
      list: () => Promise.resolve([meta]),
      inspect,
      locate: () => undefined,
  ...
  it('preserves the subagent ownership fence for cold and live Remote lookups', async () => {
    ...
    const inspect = vi.fn(() => Promise.resolve(inspection(coldMeta, [])))
    ctx.provide('sessionPersistence', {
...
describe('subagent ownership fence', () => {
  it('reads a cold child without an Agent and rejects generic resume or adoption', async () => {
    ...
    const inspect = vi.fn(() => Promise.resolve(inspection(meta, events)))
    ctx.provide('sessionPersistence', {
      list: () => Promise.resolve([meta]),
      inspect,
      locate: () => undefined,
  ...
  it('no longer treats a descriptor-only cold child without origin as subagent-owned', async () => {
    ...
    ctx.provide('sessionPersistence', {
      list: () => Promise.resolve([meta]),
      inspect: () => Promise.resolve(inspection(meta, events)),
      locate: () => undefined,
    } as never)
...
describe('sessions.prompt synchronous rejection', () => {
  ...
  it('classifies a raced cold-resume ID collision as agent-busy', async () => {
    ...
    ctx.provide('sessionPersistence', {
      list: () => Promise.resolve([meta]),
      inspect: () => Promise.resolve(inspection(meta, [])),
      locate: () => undefined,
    } as never)
