...
import SessionStore, { SessionId, sessionLogCutFromEvents, type Session } from '@deepseek-ai/dsh-session'
...
describe('session.history presenter scope', () => {
  ...
  it('resolves a switched session from the LOG, not its creation header', async () => {
    ...
    const meta = { version: 0, id: SessionId('p4'), createdAt: 1, cwd: '/tmp/p4', agentPreset: 'standard' }
    const { api } = await harness(['standard', 'minimal'], {
  ...
  it('serves a COLD transcript whose standing mount is no longer usable', async () => {
    ...
    const meta = { version: 0, id: SessionId('p3'), createdAt: 1, cwd: '/tmp/p3', agentPreset: 'standard' }
    const { api } = await harness(['standard'], {
      list: () => Promise.resolve([meta]),
      inspect: () => Promise.resolve(inspection(meta, [])),
    })
