...
import type { SessionId, SessionLogCut } from '@deepseek-ai/dsh-session'
...
describe('Web session model selection', () => {
  ...
  it('authorizes attachment bytes only when the session event stream references the id', async () => {
    ...
    const base = agent.session.readLog()
    const reverseValues = vi.fn((from?: number, to?: number) => base.reverseValues(from, to))
    ...
    vi.spyOn(agent.session, 'readLog').mockReturnValue(tracked)
    const eventsSnapshot = vi.spyOn(agent.session, 'events', 'get')
    ...
    expect(reverseValues).not.toHaveBeenCalled()
    ...
    expect(reverseValues).toHaveBeenCalledOnce()
