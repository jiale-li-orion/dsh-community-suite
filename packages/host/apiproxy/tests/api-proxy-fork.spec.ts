...
describe('sessions.fork', () => {
  ...
  it('materializes only the selected fork prefix', async () => {
    const ctx = await composed()
    const source = liveAgent(ctx, 'session-bounded-fork', 100)
    const base = source.readLog()
    ...
    vi.spyOn(source, 'readLog').mockReturnValue(tracked)
    const eventsSnapshot = vi.spyOn(source, 'events', 'get')

    const response = await api(ctx).sessions.fork(request({ sessionId: source.id, atSeq: 1 }))
    expect(response.result.ok).toBe(true)
    expect(materialized).toEqual([[0, 3]])
  ...
  it('forks a persisted subagent without resuming its Agent', async () => {
    ...
    ctx.provide('sessionPersistence', {
      ...
      inspect: () => Promise.resolve({ meta: header, log, events }),
    } as never)
    ctx.provide('sessionQuery', {
