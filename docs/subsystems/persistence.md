...
interface SessionInspection {
  ...
  readonly meta: SessionHeader
  /** Stable validated logical log cut. */
  readonly log: SessionLogCut
  /**
   * Complete validated logical event log. Access may materialize every event;
   * bounded and selected-type consumers use {@link log} instead.
   */
  readonly events: readonly SessionEvent[]
}
```

`log` is the primary read face: it captures one immutable logical prefix and supports point, range, reverse, and selected-type iteration without requiring a complete event array. `events` remains a lazy compatibility view for consumers whose operation is intrinsically whole-log, such as export and full-text indexing.
...
Source: [`packages/session/session-persistence/src/index.ts:92`](../../packages/session/session-persistence/src/index.ts)
