...
终端 spec 完全指定 argv、cwd、环境覆盖、尺寸、清理宽限期与可选的分配取消。其句柄公开 `pid`、有序输出、`done`、`write`、`inspectForeground`、`signalForeground` 和须等待的 `terminate`；确切的公共形状生成到 [`ctx.subprocess` 服务目录](#ctxsubprocess--subprocessruntime-abstract-seam)中。前台检查默认要求精确；重复轮询可以显式允许复用近期执行基底观测。

```ts type-equiv
/** Freshness permitted for one terminal foreground observation. */
interface SubprocessTerminalInspectOptions {
  /** Maximum age in milliseconds of provider substrate data; zero requires a fresh observation. */
