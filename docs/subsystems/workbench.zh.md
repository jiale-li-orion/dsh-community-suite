...
工作台还提供一个 `wallpaper` 面板：它浏览当前会话的工作区（走与文件面板同一道受围栏保护的列举），列出每个目录的子目录与图片文件，设置某一张就会把它画在会话背后。绘制目标是 ui-layout 的 `shell.background` 列表座位——一层全幅、可穿透点击、位于所有栏位之后的图层——因此背景图从不触碰其他栏位的 DOM，栏位始终画在它之上。选择属于浏览器本地并按浏览器持久化；会话已消失的 URL 只会加载失败，此时图层自行隐藏。
