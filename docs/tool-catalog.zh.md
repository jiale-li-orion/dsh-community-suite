...
| `@deepseek-ai/dsh-tool-session-context` | `context_commit`、`context_discard_preparation`、`context_edit_preparation`、`context_inspect`、`context_prepare`、`context_read`、`history_read`、`history_search` | `ctx.tools`、`ctx.systemPrompt`、`ctx.sessionContext`、`a calling Agent and a selected compaction provider for mutation tools` | `tool/call`、`compaction/preparation/* during reviewed range preparation`、`compaction/* plus one replacement user/message when a preparation commits`、`tool/result` | - | 检查与检查点召回都是有界读取。范围压缩始终按 prepare、评审或编辑、再 commit 的顺序执行；所有原文都保留在仅追加 Session 日志中。 |
...
| `@deepseek-ai/dsh-plugin-catalog-tools` | `plugin_install`、`plugin_search` | `ctx.tools`、`ctx.pluginCatalog`、`ctx.pluginInstall`、`ctx.approval and a calling Agent (plugin_install only)` | `tool/call`、`tool/result`、`approval/asked`、`approval/decided` | - | 搜索工具只返回目录数据，绝不修改任何东西；安装工具解析某次搜索返回的那条确切条目、校验该条目自带的目标，并且只在获得审批后执行它——目录里的命令文本从不被执行，也不会由字段拼装出命令。 |
...
在当前轮次内提交一个已评审的 preparation。它会替换未来模型 Context 中的确切范围；原文仍可从仅追加日志中召回。
...
```json
{
  ...
  "properties": {
    "preparation_id": {
      "type": "string",
      "description": "Ready preparation id."
    }
  },
  "required": [
    "preparation_id"
...
来源：[`packages/session/tool-session-context/src/index.ts`](../packages/session/tool-session-context/src/index.ts)

### `context_discard_preparation`

丢弃一个 ready 或 failed preparation，不改变当前 Context。
...
```json
{
  ...
  "properties": {
    "preparation_id": {
      "type": "string",
      "description": "Preparation id to discard."
    }
  },
  "required": [
    "preparation_id"
...
替换一个 ready preparation 的评审摘要。这项操作仍不改变当前 Context。
...
```json
{
  ...
  "properties": {
    "preparation_id": {
      "type": "string",
      "description": "Opaque preparation id from context_prepare or context_inspect."
    },
    "summary": {
      "type": "string",
      "description": "Complete replacement checkpoint Markdown."
    }
  },
  "required": [
    "preparation_id",
...
把模型当前确切可见的 Context 检查为平衡单元、token 价格、代际、检查点和待评审 preparation。
...
```json
{
  ...
  "properties": {
    "before_index": {
      "type": "integer",
      "description": "Exclusive unit index for an earlier page. Omit to read the current tail."
    },
    "limit": {
      "type": "integer",
      "description": "Maximum units to return, capped at 100."
    }
  }
}
```
...
为一段连续且平衡的单元范围生成并持久记录可评审摘要，不改变当前 Context。
...
```json
{
  ...
  "properties": {
    "start_unit_id": {
      "type": "string",
      "description": "First unit id from context_inspect."
    },
    "end_unit_id": {
      "type": "string",
      "description": "Last unit id from context_inspect."
    },
    "preservation_brief": {
      "type": "string",
      "description": "Exact facts, literals, or artifacts the summary must preserve."
    }
  },
  "required": [
    "start_unit_id",
...
读取 `context_inspect` 返回的一个 `unit_id` 所代表的完整当前内容。
...
```json
{
  ...
  "properties": {
    "unit_id": {
      "type": "string",
      "description": "Opaque current Context unit id."
    }
  },
  "required": [
    "unit_id"
...
读取一个检查点所遮蔽确切原文的有界页面，并可选择递归展开嵌套检查点。
...
```json
{
  ...
  "properties": {
    "checkpoint_id": {
      "type": "string",
      "description": "Checkpoint id from context_inspect."
    },
    "cursor": {
      "type": "string",
      "description": "Opaque nextCursor from the previous history_read page."
    },
    "recursive": {
      "type": "boolean",
      "description": "Expand nested checkpoints. Defaults to true."
    }
  },
  "required": [
    "checkpoint_id"
...
```json
{
  ...
  "properties": {
    ...
    "cursor": {
      "type": "string",
      "description": "Opaque nextCursor from the previous search page."
    },
    "case_sensitive": {
      "type": "boolean",
      "description": "Use exact case. Defaults to false."
    },
    "recursive": {
      "type": "boolean",
      "description": "Search nested checkpoints. Defaults to true."
    }
  },
  "required": [
    "checkpoint_id",
...
检查与检查点召回都是有界读取。范围压缩始终按 prepare、评审或编辑、再 commit 的顺序执行；所有原文都保留在仅追加 Session 日志中。
...
来源：[`packages/workbench/tool-workbench/src/index.ts`](../packages/workbench/tool-workbench/src/index.ts)

### `workbench_open`

打开共享工作台栏，可选按 id 选中一个面板。人会在同一个面板上看到该栏打开；此调用不启动其他任何东西。
...
```json
{
  ...
  "properties": {
    "panel": {
      "type": "string",
      "description": "Panel id to select. Omit to keep the current selection."
    }
  }
}
```
...
这三个工具驱动浏览器渲染的共享工作台视图；它们不写入会话状态，因此结果是点名已提交视图的通知。
