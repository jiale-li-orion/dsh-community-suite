...
import CompactionDirectory from '@deepseek-ai/dsh-compaction/directory'
import SessionContextService from '@deepseek-ai/dsh-session-context'
import * as ToolSessionContext from '@deepseek-ai/dsh-tool-session-context'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  {
    pkg: '@deepseek-ai/dsh-tool-session-context',
    dir: 'tool-session-context',
    source: 'packages/session/tool-session-context/src/index.ts',
    requires: [
      'ctx.tools',
      'ctx.systemPrompt',
      'ctx.sessionContext',
      'a calling Agent and a selected compaction provider for mutation tools',
    ],
    writes: [
      'tool/call',
      'compaction/preparation/* during reviewed range preparation',
      'compaction/* plus one replacement user/message when a preparation commits',
      'tool/result',
    ],
    async mount(ctx) {
      await ctx.plugin(SessionStore)
      ...
      await ctx.plugin(SessionContextService)
      await ctx.plugin(ToolSessionContext)
    },
    note:
      'Inspection and checkpoint recall are bounded reads. Range compaction remains a prepare, review or edit, then commit operation; every original stays in the append-only Session log.',
  },
  {
