...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  // Product-owned locale/source identifiers use the bare `cordis` id. They
  // are not npm package references and must stay stable across the rescope.
...
function excluded(file: string): boolean {
  ...
  if (file === 'scripts/rescope-vendor.spec.ts') return true // pre/post-state fixtures
...
function rewriteLine(line: string, file: string, all: readonly Pattern[]): string {
  ...
  for (const pattern of all) {
    ...
    out = out.replace(pattern.token, (match, quote: string, subpath: string) => {
      // `cordis/<event>` is the Harness event namespace, not a package
      // subpath. All package imports in this tree use the bare module name.
      if (pattern.upstream === 'cordis' && subpath !== '') return match
      if (
        pattern.upstream === 'cordis'
        && file === 'scripts/gen-cordis-catalog.ts'
        && line.trim() === "'cordis': 'extensions.md',"
      ) return match
...
/** Rewrite one in-memory file using the same rules as the repository codemod. */
export function rewriteRescopeText(text: string, file: string, reverse = false): string {
  return rewrite(text, file, patterns(reverse)).text
}
