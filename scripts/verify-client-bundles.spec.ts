 ...
 * module table cannot answer, a declared graph edge no row can satisfy, a
 * bundle registering under the wrong id, and a registration envelope that
 * throws.
 */
...
describe('checkBundles', () => {
  ...
  it('rejects a declared graph edge that names no client row', () => {
    const violations = checkBundles(
      [bundle('@deepseek-ai/dsh-client-ui-workbench', [], '@deepseek-ai/dsh-client-ui-workbench', ['@deepseek-ai/dsh-plugin-install'])],
      TABLE,
    )
    expect(violations).toHaveLength(1)
    expect(violations[0]?.message).toMatch(/dsh\.client\.inject names "@deepseek-ai\/dsh-plugin-install".*no such row/)
  })

  it('accepts a declared graph edge naming a registered bundle or a seed module', () => {
    expect(checkBundles(
      [bundle('@deepseek-ai/dsh-client-ui-workbench', [], '@deepseek-ai/dsh-client-ui-workbench', ['react', '@deepseek-ai/dsh-client-runtime'])],
      TABLE,
    )).toEqual([])
  })
  ...
  it('rejects an envelope that throws and a bundle with no registration', () => {
    const throwing: ClientBundle = { id: 'a', path: 'packages/x/a/lib/client.js', inject: [], source: 'throw new Error("boom")' }
    const silent: ClientBundle = { id: 'b', path: 'packages/x/b/lib/client.js', inject: [], source: 'window.__ModuleLoader__ = {}' }
