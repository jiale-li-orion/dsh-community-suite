...
携带状态的日志事件必须携带变更后的完整状态，绝不携带裸增量。三个领域现状已然合规：`todo/write` 是整表快照，`plan/mode` 是一个完整布尔值，`goal/change` 元数据是完整的 `GoalSnapshot`（或一个全量值清除墓碑）。该规则让每个领域的状态转移始终足够廉价（框架逐事件推进水位线，并且只应用声明的事件类型），让值在协议层自描述，并让任何消费方都可以把最近推送的值当作最终值——靠 seq 比较获得乱序免疫，且自愈：漏掉的更新会被下一次更新纠正。
...
- **host 是投影唯一的计算地点。** 框架在每个已提交的会话事件上推进所有已注册单元的水位线，并且只对单元声明的 `eventTypes` 调用 `apply`；引用未变（`Object.is`）就不产生任何下游工作。冷回放与惰性回放会合并这些声明，并使用 Session 日志的类型索引，因此只关心元数据的单元不会解码打包的流式分片。[有界 Session 日志读取决策](../../implemented/architecture/2026-08-16-bounded-session-log-reads-and-packed-retention.md)负责稳定切面与索引机制。客户端从不折叠领域事件——它们收到的是成品值（基线块 + 下文的推送帧）。这消除了双重实现陷阱（plan 的双事件折叠只在 host 写一遍），也消除了一切客户端侧领域代码。
...
注册表的读取面已经服务于本 RFC 协议计划之外的一个已交付消费方：[subagent 列表经投影单元读取身份](../../implemented/architecture/2026-08-06-subagent-list-identity-projection.md)注册了 `subagent` 单元——从 `subagent/descriptor` 按 last-wins 折叠出的持久化 mode/label 身份——`SubagentRuntime.listChildren` 对 live child 经 `snapshot()` 读取（水位缓存，零日志读），对 cold child 则在一次持久化检查上调用 `restoreLog({}, inspection.log)` 读取。注册表不提供带类型的领域失败通道：值缺席是普通信号，单元抛错则使该次读取失败，由消费方收容。
...
1. **host 基座**：`dsh-session-projection`（单元约定、按类型过滤的驱动、水位线缓存）+ api-proxy 的 projections 块 + `session/projection` 推送帧。零领域注册也可合入（此时块与帧直接缺席）。
...
- **忙碌会话上的分发扇出**：每个已提交事件仍需检查每个已注册单元以推进其水位线，但 `eventTypes` 会阻止无关的 `apply` 调用，并让回放跳过无关的打包记录。剩余成本与规模很小的已注册单元表成正比；未来可以增加事件类型到注册项的索引，而无需改变单元约定。
