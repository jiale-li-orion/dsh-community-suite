...
export interface ProjectionDefinition<K extends keyof SessionProjectionMap, S> {
  ...
  init(): S
  /** Event types passed to apply, or all events. */
...
- **The host is the only place a projection is computed.** The framework advances every registered unit's watermark on each committed session event and calls `apply` only for the unit's declared `eventTypes`; an unchanged reference (`Object.is`) produces no downstream work. Cold and lazy replay unions those declarations and uses the Session log's type index, so a unit interested only in metadata does not decode packed stream chunks. The [bounded Session-log read decision](../../implemented/architecture/2026-08-16-bounded-session-log-reads-and-packed-retention.md) owns the stable-cut and index mechanism. Clients never fold domain events — they receive finished values (baseline block + push frame below). This removes the double-implementation trap (plan's two-event fold written once, on the host) and any client-side domain code.
...
The registry's read faces already serve a shipped consumer beyond this RFC's wire plan: [subagent list identity via the projection unit](../../implemented/architecture/2026-08-06-subagent-list-identity-projection.md) registers a `subagent` unit — the durable mode/label identity folded last-wins from `subagent/descriptor` — and `SubagentRuntime.listChildren` reads it through `snapshot()` for a live child (the watermark cache, zero log reads) and `restoreLog({}, inspection.log)` over one persistence inspection for a cold one. The registry has no typed domain failure channel: an absent value is the ordinary signal, while a throwing unit fails that read for the consumer to contain.
...
1. **Host base**: `dsh-session-projection` (unit contract, type-filtered drive, watermark cache) + api-proxy projections block + the `session/projection` push frame. Mergeable with zero domains registered (block and frames simply absent).
...
- **Dispatch fan-out on busy sessions**: every committed event still checks every registered unit so its watermark advances, but `eventTypes` prevents unrelated `apply` calls and lets replay skip unrelated packed rows. The remaining cost is proportional to the small registered-unit table; a future event-type-to-registration index can remove that scan without changing the unit contract.
