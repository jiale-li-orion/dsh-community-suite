...
恢复过程是一条从持久化产物进入 `Session.fromRestoreRecordsAsync` 的所有权转移流水线。压缩产物仍作为源缓冲区驻留，但解码与扫描阶段会增量消费上一阶段的输出，不会保留整份日志的明文或展开后的事件副本。JSONL 扫描器保留已验证的存储记录；Session 将其接管到私有日志，并让符合条件的分片行保持打包状态。
...
持久化层把 parser 持有的存储记录转移给 `Session.fromRestoreRecordsAsync`。普通事件记录是已分离且无环的树，因此恢复专用路径使用一次 `for...in` 与 `switch` 校验固定事件信封，按事件判别字段执行当前数据形状检查，并通过显式 `pending` 数组迭代冻结所拥有的对象图，不使用循环跟踪集合。打包行会重新校验编码与逻辑 seq 连续性，再直接接管而不展开其中的分片成员。Session 接管每条记录后会释放 parser 数组中的对应槽位，并在有界记录工作后让出事件循环，因此恢复既不会同时保留 parser 记录与私有日志引用，也不会长期独占宿主事件循环。`surface` 校验会为普通事件记录一次转换计划，并在该事件进入日志时提交；已验证的打包分片区间则作为非 surface 区间提交。
...
- **跳过恢复值的校验或冻结**：不予采纳，因为持久存储属于运行时边界，而 Session 的公开读取承诺已接受历史不可变。优化路径利用更强的所有权事实特化这些操作，而不是将其移除。
...
快速解码器依赖运行时探测的 Node 内部接口，但接口不兼容时会改用公共实现，不会改变正确性。系统会在协作式帧、明文扫描与恢复准入让出点观察取消信号；截止时间并不是单个帧或单条记录解析内部严格的挂钟时间上限。活跃 Session 通过私有打包记录保留完整逻辑日志，并按需公开单个不可变事件。[有界会话日志读取与打包保留决策](2026-08-16-bounded-session-log-reads-and-packed-retention.md)负责该表示，以及完整数组必须显式实体化的要求。

测试会强制执行两种解码器实现，比对帧顺序和损坏处理行为，覆盖协作式取消、明文与撕裂帧扫描、打包记录接管和撕裂尾部恢复，并保留既有会话信封、`surface` 与不可变性约定。
