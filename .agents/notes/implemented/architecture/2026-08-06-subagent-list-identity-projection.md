...
- **Value retrieval is a three-rung compute-and-discard ladder**: a live child reads `sessionProjections.snapshot()` (the registry's existing watermark cache, zero log reads); a cold child first reads the optional `sessionProjectionCache.cachedSnapshot(header)`, using the value directly when a non-null `subagent` identity passing the seq gate (`seq >= seedLength ?? 0`) is among its values; otherwise it pays one `persistence.inspect` read plus one `registry.restoreLog({}, inspection.log)` fold; beyond that, absent is absent — no cache of its own, no write-back, no index.
...
| 3: cold child, fallback | One `persistence.inspect(id)` read + `registry.restoreLog({}, inspection.log).snapshot.values.subagent` | One artifact read; the packed Session cut and projection `eventTypes` skip unrelated stream payloads during the fold |
...
- The cold-read cost, recorded honestly: only with the cache unmounted or missed does a cold child pay one artifact read per listing. JSONL inspection retains packed rows, and the registry folds only each unit's declared event types, so chunk-heavy logical length no longer implies expanding every event; physical parsing and the relevant indexed events remain real work. The settled stance is compute-and-discard, and no cache of its own is built. The read goes through `inspect()` into the [Session preparation](2026-08-05-session-preparation.md) cold path, so short-term repeated reads of the same id can hit its LRU for reuse, but listing does not depend on this. A live child reads zero log throughout.
...
Consuming surfaces: catalog diagnostic handling across wire, tool, and GUI stays unchanged (the `list_agents` description and output schema are untouched; the plugin only narrows its load requirement — `sessionQuery` dropped from inject). On the route segment, `hasSubagentDescriptor()` is deleted and `hasSubagentOwner` looks only at `header.origin`. `subagents.history` shares `session.history`'s current data path: a live child supplies a stable in-memory log cut and watermark snapshot, a cold child supplies one persistence inspection and detached projection fold, and the response carries the same sequence-span `HistoryEntry` projection plus unmodified live-tail stitching as ordinary history.
...
| host/apiproxy | api-proxy.ts | `hasSubagentDescriptor` deleted; the owner check looks only at `header.origin`; `subagents.history` shares `session.history`'s stable log-cut source, sequence-span history projection, and projection baseline paths |
...
| wire/client | api/subagents.ts, runtime sessions/service.ts, GUI | Catalog row shape and diagnostic handling unchanged; history entries may carry `firstSeq` so the client preserves represented seq spans before the raw live tail |
