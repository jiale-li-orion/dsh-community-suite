...
Reading is unconditional and layout-blind. Packed, unpacked, and mixed files expose the same contiguous logical `SessionEvent` stream, so the default does not require a session-format version change or an on-disk runtime migration. JSONL restoration may transfer validated storage records into Session without expanding packed rows; callers still observe only logical events. The [bounded Session-log read decision](2026-08-16-bounded-session-log-reads-and-packed-retention.md) owns that private retention and its explicit materialization API. The option controls newly appended batches only; it never selects a reader mode.

### Logical events and physical rows

Packing stays at the `dsh-session` storage seam through `packChunkRuns()` and the storage-record readers. The encoder recognizes exact delta-event shapes, preserves unrecognized events verbatim, and packs only runs of at least three. A packed row is storage vocabulary, not a `SessionEventMap` member: it is never returned as a public event value or fired through `session/event`, even when Session retains that row privately.
...
Ordinary JSONL writes and committed fixtures use fewer physical rows while preserving the exact logical event stream. Runtime readers accept every existing layout, active Sessions retain eligible rows privately, and operators retain a deliberate unpacked diagnostic mode. Raw files are less convenient for per-token line processing, and external tools that incorrectly treat every post-header row as a `SessionEvent` encounter storage tags more often; supported readers use the storage-record API or explicitly materialize logical events.
