...
- host 判据是日志中不存在任何 `turn/start`；独立命令、设置、标题与其他仅日志事件不会让对话变为非 blank。live 会话通过按索引的增量折叠读取该值。cold 列表行在可用时读持久投影缓存；只有有界小产物探测确认日志仍无轮次时，才信任可能的 `blank: true` 提示，来源缺失、陈旧、过大或不可读时均降级为可见（`blank: false`）。两个第一方后端仍为真 lazy，因此 never-appended 会话仍不会进入 `persistence.list()`。
