...
Restoration is one ownership-transfer pipeline from the persistence artifact into `Session.fromRestoreRecordsAsync`. The compressed artifact remains the source buffer, while each decoding and scanning stage consumes the previous stage's output incrementally without retaining a whole-log plaintext or expanded event copy. The JSONL scanner retains validated storage records; Session adopts them into its private log and keeps eligible chunk rows packed.
...
Persistence transfers parser-owned storage records to `Session.fromRestoreRecordsAsync`. Ordinary event records are detached, acyclic trees, so the restore-only path validates the fixed event envelope with one `for...in` and `switch`, dispatches current-shape checks by event discriminant, and iteratively freezes the owned graph with an explicit `pending` array and no cycle-tracking set. Packed rows are revalidated for encoding and logical sequence continuity, then adopted without expanding their chunk members. Session releases each parser-array slot after adoption and yields after bounded record work, so restoration does not retain both the parser record and private-log reference or monopolize the host event loop. Surface validation records one transition plan and commits that plan when the exact ordinary event enters the log; packed chunk ranges commit as validated non-surface intervals.
...
- **Skip validation or freezing for restored values** — rejected because durable storage is a runtime boundary and public Session reads promise immutable accepted history. The optimized path specializes those operations around stronger ownership facts instead of removing them.
...
The fast decoder depends on runtime-probed Node internals, but incompatibility selects the public implementation rather than changing correctness. Cancellation is observed around cooperative frame, plaintext-scan, and restore-admission yields; the deadline is not a hard wall-clock bound inside one frame or record parse. The active Session retains the complete logical log through private packed records and exposes individual immutable events on demand. The [bounded session-log reads and packed retention decision](2026-08-16-bounded-session-log-reads-and-packed-retention.md) owns that representation and the requirement that complete arrays be explicitly materialized.

Tests force both decoder implementations, compare their frame order and corruption behavior, exercise cooperative cancellation, plaintext and torn-frame scanning, packed-record adoption, and torn-tail recovery, and retain the existing session envelope, surface, and immutability contracts.
