...
mode 与 label 由新的 `subagent` projection unit（纯身份两臂）折叠，unit 是折叠规则的唯一权威；`listChildren` 不再依赖 session-query——枚举是 subagent 自管的 live-preferred 合并，取值走三级「算完即止」阶梯：live child 同步读注册表的既有水位缓存（零日志读）；cold child 先问可选的 `sessionProjectionCache` checkpoint，取到过 seq 门的身份即定值；否则一次 `persistence.inspect` 读取加一次按索引的 `registry.restoreLog` 折叠。无自建索引、不自建缓存、无回写。
...
- **取值三级「算完即止」阶梯**：live child 读 `sessionProjections.snapshot()`（注册表既有水位缓存，零日志读）；cold child 先读可选 `sessionProjectionCache.cachedSnapshot(header)`，values 含非 null 且过 seq 门（`seq >= seedLength ?? 0`）的 `subagent` 身份即直接用；否则一次 `persistence.inspect` 读取加 `registry.restoreLog({}, inspection.log)` 折叠；再没有就没有——不自建缓存、无回写、无索引。
...
| 3：cold child，兜底 | 一次 `persistence.inspect(id)` 读取 + `registry.restoreLog({}, inspection.log).snapshot.values.subagent` | 一次产物读取；打包 Session 切面与投影 `eventTypes` 会在折叠时跳过无关流式载荷 |
...
- 冷读成本如实记录：只有在 cache 未挂载或未命中时，cold child 每次列表才付一次产物读取。JSONL 检查会保留打包行，注册表只折叠各 unit 声明的事件类型，因此分片密集的逻辑长度不再意味着展开每个事件；物理解析和相关索引事件仍是真实工作。定案「算完即止」，不自建缓存。该读取经 `inspect()` 走 [Session 准备阶段](2026-08-05-session-preparation.md)的冷路径，同 id 短期重复读取可命中其 LRU 复用，但列表不依赖此。live child 全程零日志读。
...
消费面：catalog diagnostic 在 wire、tool 和 GUI 中的处理保持不变（`list_agents` 的 description 与 output schema 未动；该插件仅加载要求收窄——inject 去掉 `sessionQuery`）。路由段的 `hasSubagentDescriptor()` 已删除，`hasSubagentOwner` 只看 `header.origin`。`subagents.history` 与 `session.history` 共享当前数据路径：live child 提供稳定内存日志切面与水位快照，cold child 提供一次持久化检查与 detached 投影折叠，响应则携带与普通历史相同的 seq 跨度 `HistoryEntry` 投影，并拼接未经修改的实时尾部。
...
| host/apiproxy | api-proxy.ts | 删 `hasSubagentDescriptor`，属主判定只看 `header.origin`；`subagents.history` 与 `session.history` 共享稳定日志切面来源、seq 跨度历史投影和投影基线路径 |
| tool | tool-subagent-control/list-agents.ts | 加载要求收窄（inject 去 `sessionQuery`）；model-visible schema、描述与渲染零改动 |
| wire/client | api/subagents.ts、runtime sessions/service.ts、GUI | catalog 行形状与 diagnostic 处理不变；history entry 可携带 `firstSeq`，使客户端在原样实时尾部之前保留所代表的 seq 跨度 |
