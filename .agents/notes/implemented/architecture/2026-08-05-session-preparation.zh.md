...
使用协调器的持久化实现会将一个冷源加载为准备完成的 Session。后端转移新鲜、彼此无别名的元数据，以及逻辑事件或已验证的存储记录，并附带标识这些精确值的来源限定 revision；Session 恢复路径直接验证并接管所拥有的表示，不再复制，并在私有实现中保留符合条件的打包分片行。协调器计算中断轮次的 closer，并且只构造一次精确的未发布 Session。其不可变 header 与稳定且已配平的 `SessionLogCut` 构成读取方借用的主要 `SessionInspection`；完整 `events` 数组是惰性兼容视图，revision 则保留在持久化内部。[有界 Session 日志读取决策](2026-08-16-bounded-session-log-reads-and-packed-retention.md)负责切面与打包保留语义。
...
首次冷检查需要承担完整产物验证与 Session 构造成本，并可能保留该未发布 Session 直至淘汰，但有界读取方不会额外展开每个打包事件。持久化层必须协调预留、append、修复和发布；调用方必须把检查结果视为借用的不可变状态。依赖默认 `prepare()` 的后端仍然正确，但无法获得复用优化。
