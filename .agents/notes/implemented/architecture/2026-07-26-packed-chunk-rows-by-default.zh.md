...
读取始终不受选项控制且与布局无关。打包、非打包和混合文件都会公开相同且连续的逻辑 `SessionEvent` 流，因此更改默认值不需要变更会话格式版本，也不需要对磁盘数据执行运行时迁移。JSONL 恢复可以把已验证的存储记录转移给 Session，而不展开打包行；调用方仍然只观察逻辑事件。[有界 Session 日志读取决策](2026-08-16-bounded-session-log-reads-and-packed-retention.md)负责该私有保留及其显式实体化 API。该选项只控制新追加的批次，绝不会选择读取器模式。

### 逻辑事件与物理行

打包保留在 `dsh-session` 的存储 seam，并通过 `packChunkRuns()` 与存储记录读取 API 实现。编码器识别精确的增量事件形态，原样保留无法识别的事件，并且只打包至少包含 3 个事件的连续段。打包行属于存储词汇，不是 `SessionEventMap` 成员：即使 Session 在内部保留该行，它也绝不会作为公开事件值返回或通过 `session/event` 触发。
...
常规 JSONL 写入与签入仓库的 fixture 使用更少的物理行，同时精确保留逻辑事件流。运行时读取器接受所有现有布局，活跃 Session 在私有实现中保留符合条件的打包行，操作方也保留显式的非打包诊断模式。按 token 逐行处理原始文件较为不便；错误地将 header 后每一行都视为 `SessionEvent` 的外部工具会更频繁地遇到存储标签，受支持的读取器则会使用存储记录 API 或显式实体化逻辑事件。
