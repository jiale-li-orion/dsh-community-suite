...
- The host criterion is the absence of any `turn/start`; standalone commands, settings, titles, and other log-only events do not make a conversation non-blank. A live session reads this through an indexed incremental fold. A cold list row uses the persisted projection cache when available; a possible `blank: true` hint is trusted only after a bounded small-artifact probe confirms the log still has no turn, while a missing, stale, large, or unreadable source degrades to visible (`blank: false`). A never-appended session still never enters `persistence.list()` because both first-party backends remain truly lazy.
