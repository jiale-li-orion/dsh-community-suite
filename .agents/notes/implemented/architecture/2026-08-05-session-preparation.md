...
A coordinator-backed persistence implementation loads one cold source into a prepared Session. The backend transfers fresh, mutually unaliased metadata and either logical events or validated storage records together with the source-qualified revision that identifies those exact values; the Session restore path validates and adopts the owned representation instead of cloning it, retaining eligible chunk rows privately. The coordinator computes interrupted-turn closers and constructs the exact unpublished Session once. Its immutable header and stable balanced `SessionLogCut` form the primary `SessionInspection` borrowed by readers; the complete `events` array is a lazy compatibility view, while the revision remains internal to persistence. The [bounded Session-log read decision](2026-08-16-bounded-session-log-reads-and-packed-retention.md) owns the cut and packed-retention semantics.
...
The first cold inspection pays complete artifact validation and Session-construction cost and may retain that unpublished Session until eviction, but bounded readers do not also expand every packed event. Persistence must coordinate reservation, append, repair, and publication, and callers must treat inspection values as immutable borrowed state. Backends that rely on the default `prepare()` remain correct but do not receive the reuse optimization.
