...
- 子进程 fixture（测试前置数据）覆盖非 leader 与非主线程的 stdin 等待、僵尸进程完全停稳、不可读进程状态、受支持的 syscall 表、不支持的架构、误报拒绝、有界快照复用与过期、精确刷新、缓存候选重新校验，以及共享的本地检查器；同一单元测试套件通过注入覆盖 macOS 检查器逻辑。
