...
- Subprocess process fixtures cover non-leader and non-main-thread stdin waits, zombie quiescence, unreadable process state, supported syscall tables, unsupported architectures, false-positive rejection, bounded snapshot reuse and expiry, exact refresh, cached-candidate revalidation, and one shared local inspector; macOS inspector logic is injected into the same unit suite.
