...
一张壁纸现在只需一条声明的座位、一个面板和一个小 store；删掉 bundle 行就同时删掉图层与面板，没有条目注册时框架毫无变化。遮罩保证任何图片上方的会话文字都可读。代价是 `ui-layout` 多了一条核心侧座位（与工作台栏一样是增量的），以及面板目前只列工作区根目录——嵌套图片仍需文件面板的导航。
