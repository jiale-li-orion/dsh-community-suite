...
**准入时把图片缩到路由的尺寸上限以内。** 视觉路由公布的限额是「单边最大 `8192 px`，当一次请求包含 15 张及以上图片时降为 `4096 px`」（[DeepSeek 视觉指南：Limits](https://github.com/thevibeworks/deepseek-docs/blob/main/content/en/guides/vision.md)），而本 harness 允许每条消息 20 张。超过该上限的图片**不被拒绝**：`attachment-local` 会在入库时按比例缩到 `maxImageEdgePixels`（默认 4096，即任何时候都成立的那个界），保持原格式（含动画），并存储缩放后的字节，因此存下来的引用永远描述一张路由接受的图片。拒绝会让长截图读不了——而"读长截图"正是 agent 检查页面的方式——何况路由在推理前会把每张图缩到大约 800x800 的像素预算，缩到该长边并不损失保真度。
