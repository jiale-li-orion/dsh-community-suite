...
### 22:30 · 修用户实测到的面板 bug（缺桶即整页空白）

用户报："当前会话 uploads 下面已经有了东西，就没归进去。其他倒是一切正常。"

- **根因（我的 bug）**：面板先列 `uploads/` 取得桶，再**逐个桶**去列；而**某个桶不存在时该次 `list` 会抛错**，导致**整个读取失败**、直接落到空状态 —— 当前会话的文件只在 `desktop-browser/` 里，列 `mobile-app/` 就抛了 ✗。
- **修法**：① **缺桶按正常跳过**（"某个来源还没人传过文件"是最常见的情况，不该让整页空白）；② **`uploads/` 根下的文件也列出**——用户最早两张（证件照、51 页 PDF）是**分来源之前**传的，**没有来源记录**，因此如实标为"来源未知"，**不编造来源**。
- **测试**：新增 2 条正是对着这次线上失败写的——"某个桶不存在时仍显示其它桶的文件"（且断言**不出现空状态**）、"根下文件列为无记录来源"；`ui-workbench` **121 测试**通过、`UploadsPanel.tsx` 覆盖率 100%、lint 0。
- **教训（第四条同源）**：这都是**"把缺失当成错误"**的同一类问题——之前是"没有类别就不注入"（正确 ✓），这次是"没有桶就整页空白"（错误 ✗）。**缺什么就说缺什么，不要让一处缺失抹掉全部已知。**

### 22:40 · 用户验收上传面板通过；计划文件补完成度标签

- 用户：**"可以了，就这样吧"** → 上传链路（选择/落盘/来源分桶/面板/空状态插画）验收通过。
- **计划文件补标签**（用户授权修改 `阶段性开发计划.spec.md`）：E0 清单 4/5、E1 清单 6/6、E2 清单 **1 完成 + 3 部分 + 2 未做 ≈ 42%**；E2 的六条改为**就地标注**，每条后面写明证据或缺口（内容接入服务只做了一半、`ingest_id`/幂等/Share Target/MIME 校验/过期策略未做）。
- **两次自我纠正（都值得记）**：① 我最初写"13/16""6/7"这类数字是**编的**——改成用脚本**按文件实际点算**；② 我的第一次编辑把**原清单与我的标注块同时留下**（清单变成 12 条），改成**就地改标记**而不是另加一份。**编造数字和留下重复清单，都是"看起来完成了"的假象。**

### 23:00 · E2 下一步：接入记录与幂等（计划第 3、4 条的剩余部分）

**先说明一处计划与现实不符**：计划 E2 第 5 条写的是 **Web Share Target**（浏览器分享目标），它**需要 Service Worker + 已安装的 PWA**；而用户手机用的是 **APK**，那条在该设备上**根本不会触发** ✗。Android 上的等价物是**意图过滤器（`ACTION_SEND`）**——我把它记在计划里，作为该条的等价替换，等你确认后实现（它需要真机才能验证，我不想谎报）。

**这一步做的是可自证的那半**：

- **`ingestId`**：上传请求可带一个不透明令牌（`[A-Za-z0-9._-]{1,128}`，越界 400）；**同一 id 重复提交返回第一次的结果**（第二次连请求体都不用读 ✓），**同名文件不会被复制成第二份**；**不同次选择即使字节相同也各存一份**（同一个 id 才算同一次）✓。
- **接入记录**：写 `uploads/.dsh/ingest.json`（**每个会话的上传根一份索引**，不是每个来源桶一份——重复请求可能落在不同桶的语境里 ✗），每条记 `path / bytes / sha256 / device / receivedAt`；**摘要用于把"重复"与"同内容的新文件"区分开**。索引放**点目录**里：文件面板默认不显示 ✓、上传面板只列文件不列目录 ✓ 所以它不会冒出来冒充一个上传 ✓。
- **索引坏掉不拖垮上传**：内容不是对象时按空处理并重建 ✓（**文件本身才是"收到了什么"的权威** ✓）。
- **客户端**：每个文件在**选中时**生成一次 id 并经上传动作送出 → 重试同一次选择会命中同一条记录 ✓；新的一次选择是新 id ✓。
- **验证**：`workbench-bytes` **44 测试**通过、`index.ts` **覆盖率 100%**；`ui-workbench` 121 测试通过；lint 0；全仓 typecheck 通过；两面产物已重建（**宿主需重启生效**）。

## 2026-09-11

### 21:57 · 排查电脑端 39 项插件 pending；WSL 故障后服务恢复

- 用户报告电脑浏览器显示 `web boot: 39 entries did not activate`，随后补充当时 WSL 出现“灾难性故障”。缺少故障时的浏览器控制台和 WSL 事件记录，尚不能确认该页面错误的直接根因。
- 检查开始时，Windows 与 WSL 的 3080 端口均不可连接；之后已有另一进程启动源码版 Web 服务。本次未停止该进程，也未删除插件或放宽依赖检查。
- 验证：独立 Chromium 正常进入工作台，无启动控制台异常；另做连续 6 次页面加载，均无 `Failed to load plugins` 或未捕获页面异常。实际请求启动清单中的 45 份插件，内容 SHA-1 短哈希全部与各自 `rev` 一致。这些证据说明当前实例正常，不证明故障发生时的页面或缓存正常。
...
**更正一条部署认知（实测推翻）**：我一直以为"bundle 的 rev 是宿主启动时算出来的" ✗ —— 实测首页 HTML 里的 rev 与"当前 bundle 内容的 sha1 前 12 位"完全一致（`160d4909a139`）✓，说明 rev **按内容实时计算** ✓。因此重建客户端包后，浏览器的 HTTP 缓存与 APK 的本地缓存都会自动失效并取到新字节 ✓，**不需要重启宿主** ✓（这条以前会让我白白要求一次重启 ✗）。

**仍未验（不假装已完成）**：E0 真机项（断网重连后再读回结果、停 PC 后手机显示不可达）、E1「添加到主屏幕」、连接横幅的真机触发；上传的并发幂等与崩溃恢复仍是缺口。

### 23:50 · 上传恢复语义：一次尝试、原子索引、MIME，外加四盏我留下的红灯

**主改动在 `packages/workbench/workbench-bytes` 的上传路由**：

- **路径改成"抢占"而不是"先查后写"** ✗→✓：以前用 `existsSync` 判断空闲再交出路径，两个同时到达的上传会选中同一个名字、把正文交错写进同一个文件 ✗。现在用 `openSync(candidate, 'wx')` 独占创建，只有名字被占（`EEXIST`）才顺延编号 ✓——两个同名并发上传各得一个完整文件 ✓。
- **同一 ingestId 的并发请求共享同一次尝试** ✓：`apply` 持有一张在途表（键=会话+id ✓，属**注册期状态** ✓，随路由存亡 ✓）。后到的请求等待前一次尝试并按它的结果作答 ✓；尝试一结束就出表 ✓，所以**失败不会被永久缓存** ✓，之后的重试是全新尝试 ✓。
- **索引提交改为"文件锁 + 原子替换"** ✓：以前是普通 `writeFileSync`，并发会互相丢记录 ✗、崩溃会留下半截文件 ✗——而读取端把坏索引当作空索引 ✗，于是**一次中断丢掉全部记录** ✗。现在 `withFileLock` + `writeFileAtomic` ✓，并发不再丢记录 ✓、读者永远看到完整的旧文件或完整的新文件 ✓。
- **摘要随正文流式计算** ✓，不再为了记录而回读几十 MB 的文件 ✓。
- **记录带上媒体类型** ✓（读取时可缺省 ✓，旧记录继续有效 ✓，不编造 ✓）。
- **顺序是有意的：先文件、后记录** ✓。两者之间崩溃只会留下**孤儿文件** ✓（看一眼目录就能发现 ✓），反过来则会留下"指向不存在文件的记录" ✗，重试还会把它当成已完成回放 ✗。代价是一份重复副本，选它而不是选一句假话 ✓。

**测试 56 → 63 项** ✓：新增"同名两个上传同时到达"（各自正文完整 ✓、索引两条记录都在 ✓）、"重试命中在途尝试"✓、"被拒绝的尝试不留记录也不留文件" ✓、"不是重名冲突的写入失败上报为服务端失败" ✓（用只读目录制造 ✓，带 `win32` 守卫 ✓，与 `pwsh-sandbox` 的既有先例一致 ✓）；记录校验矩阵按覆盖率报告补了 3 例（闭集外设备 ✓、桶内嵌套路径 ✓、空媒体类型 ✓）。改动文件的每文件覆盖率达标 ✓。

**一处被我推翻的错误认知** ✗：我一度给 `workbench-bytes` 加 `dsh-llm` / `dsh-atomic-write` 的 package.json 依赖 ✓ —— 实测仓库里 **0 个包**声明它们 ✓：这类包靠 `tsconfig.base.json` 路径映射与 tsconfig references 解析 ✓。**已撤回** ✓（tsconfig 引用保留 ✓）。

**hygiene 第一次跑出来的四盏红灯，全是我早前留下的** ✗：client-origin 缺 invariant 伴生（一次报 7 项 ✓）、`ui-connection-status` 缺 `tsconfig.base.json` 路径映射 ✓、knip 缺"只有 .tsx 测试"的项目条目 ✓、`constraints` 连同一个**真缺陷** ✗——我声明了 `./origin → lib/origin.js`，而 tsdown 根本不构建该文件 ✗。已全部修好 ✓：补伴生文件与身份断言 ✓、导出/发布清单 ✓、tsdown 入口 ✓、tsconfig 引用 ✓、路径映射 ✓、knip 条目 ✓；那个**没有消费者的 `/origin` 子路径导出**按"抽象必须有当前需要"删掉 ✓。

**验证（全部串行跑，共 21 项改动）**：`pnpm run typecheck` EXIT=0 ✓；`doc-sync` **28/28** ✓；`knip`、`constraints`、`verify-cordis-config`、`verify-package-invariants`、`verify-runtime-closure`、`verify-node-next-types` 全 PASS ✓；`workbench` 全组 128 项 ✓；`workbench-bytes` 63 项 ✓（改动文件覆盖率达标 ✓）；**真实组装场景** `upload-ingest.e2e.ts` ✓；`client-origin` 20 项 ✓；`lint` 0 错 0 警 ✓。

**`publint` 那盏红灯也是我的** ✗（我先判成"与我无关"，随后自己推翻）：

- 我先看到 `./src/*` 与 CJS/ESM 这两类消息 ✗，**误以为**它们是全仓性的、与改动无关 ✗ —— 其实它们各出现 222 次与 42 次，是**警告** ✓。
- 把完整的 wrapper 输出按"消息频次"统计后，**只出现一次**的那条就露出来了 ✓：`pkg.exports["./origin"].default is ./lib/origin.js but the file does not exist.` ✗ —— 正是我这个包里那个**没有任何构建会产出的 `lib/origin.js`** ✗，也就是本轮我已删掉的那个无消费者子路径导出 ✓。
- 删掉后：`pnpm run publint` **EXIT=0** ✓，`pnpm run hygiene` **EXIT=0** ✓（整个聚合全绿 ✓）。
- **教训（又一次）**：不要因为一条消息"看起来很普遍"就归因为既有问题 ✗ —— **按频次把它挑出来**，罕见的那条才是真凶 ✓；而且"我判断与我无关"本身就该被验证 ✗。

**E2 进度对照**：上传恢复从"1 完成 / 4 部分 / 1 未做"推进为——**并发幂等 ✓ 完成**、**崩溃安全 ✓ 完成**（文件与索引仍不是单事务 ✓，但失败模式已从"假记录"变成"孤儿文件" ✓）、**MIME ✓ 完成**；剩余：记录不回头校验文件是否被移动/修改 ✓、过期清理 ✗、粘贴/分享入口 ✗（等价物是 Android `ACTION_SEND` ✓，需真机）。

**仍未验（不假装已完成）**：E0 真机项（用户已明确说不重要 ✓，暂缓 ✓）、连接横幅与 gzip 的真机确认 ✗。

### 00:20 · 下一项开工前的实测发现：粘贴非图片文件现在会被拒

**发现（读代码得到的，不是猜的）**：输入框已有粘贴处理 ✓（`InputBar.tsx` 的 `onPaste` ✓），但它把剪贴板里的 **file** 一律交给 `intakeImages` → `addImages` ✓，而那条链路只接受图片 ✓ —— 粘一个 PDF 进去，只会收到"不是图片"的拒绝提示 ✗，进不了上传路由 ✓。计划里 E2 那条"Web Share Target 前提不成立 → 需提供页面内上传、**粘贴**的同等入口"正是这个缺口 ✓。

**拟定接法（下一轮执行）**：给输入框加一个**链式插槽** ✓，形状照抄已被验证的 `workbench.viewer`（按注册顺序解析、第一个非空 `select` 胜出 ✓）：非图片文件在输入框里沿该插槽找一个认领者 ✓，`ui-workbench` 注册的条目用**现有的上传动作**接住它 ✓（同一份 `ingestId` 生成与草稿追加逻辑 ✓，不新建第二条上传路径 ✓）；**图片继续走既有 image attachment** ✓（保持不变 ✓，避免一条内容两种归属 ✗）。没有认领者时保持今天的行为 ✓（给出明确拒绝 ✓）。这属于跨包契约新增 ✓ → 需要同 PR 的 Agent Note ✓ 与两侧测试 ✓，以及真实组装场景 ✓。

**顺带把 `publint` 红灯查清了（结论：是我的，且已修好）**：我在上一条里判成"与我无关" ✗ —— **错了** ✗。把 wrapper 的完整输出按消息**频次**统计后，只出现一次的那条是真凶 ✓：`pkg.exports["./origin"].default is ./lib/origin.js but the file does not exist.` —— 正是本包里那个没有任何构建会产出的 `lib/origin.js` ✓，已随无消费者的子路径导出一起删掉 ✓。现在 `pnpm run publint` 与 `pnpm run hygiene` 均 **EXIT=0** ✓。

**仍未验（不假装已完成）**：宿主**未重启** ✗（用户明确说后续一起测 ✓），所以本轮上传恢复的新语义（独占占名、在途幂等、原子索引、`mediaType`）**尚未在运行中的服务上生效** ✓ —— 代码已推送 ✓，重启后即生效 ✓。
